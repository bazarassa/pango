/* Vogue
 * gen-script-for-lang.c: Utility program to generate vogue-script-lang-table.h
 *
...
#include <vogue/vogue-enum-types.h>
...
static const char *get_script_name (VogueScript script)
{
	 ...
	 const FcChar8 *lang)
{
  ...
  info.lang = vogue_language_from_string ((const char *)lang);
	      ...
	      gconstpointer b)
{
  ...
  return strcmp (vogue_language_to_string (info_a->lang),
		 vogue_language_to_string (info_b->lang));
}
...
int main (void)
{
  ...
  for (i = 0; i < script_array->len; i++)
    {
      ...
      max_lang_len = MAX (max_lang_len,
			  (int)strlen (vogue_language_to_string (info->lang)));
  ...
  g_print ("/* vogue-script-lang-table.h:\n"
	   " * \n"
	   " * Generated by %s\n"
	   " * Date: %s\n"
  ...
  g_print ("typedef struct _VogueScriptForLang {\n"
	   "  const char lang[%d];\n"
	   "  GUnicodeScript scripts[%d];\n"
	   "} VogueScriptForLang;\n"
	   "\n"
	   "static const VogueScriptForLang vogue_script_for_lang[] = {\n",
	   max_lang_len + 1,
	   max_script_len);
  ...
  for (i = 0; i < script_array->len; i++)
    {
      ...
      g_print ("  { \"%s\", %*s{ ",
	       vogue_language_to_string (info->lang),
	       (int)(max_lang_len - strlen (vogue_language_to_string (info->lang))), "");
      for (j = 0; j < MAX_SCRIPTS; j++)
	{
	  if (!info->scripts[j].freq)
	    {

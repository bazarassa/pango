/* Vogue
 * vogue-glyph-item.c: Pair of VogueItem and a glyph string
 *
 ...
 * zero-length item).
 *
 * This function is similar in function to vogue_item_split() (and uses
 * it internally.)
 *
 * Return value: the newly allocated item representing text before
 *               @split_index, which should be freed
 *               with vogue_glyph_item_free().
 *
 * Since: 1.2
 **/
VogueGlyphItem *
vogue_glyph_item_split (VogueGlyphItem *orig,
			const char     *text,
			int             split_index)
{
  VogueGlyphItem *new;
  ...
  new = g_slice_new (VogueGlyphItem);
  split_offset = g_utf8_pointer_to_offset (text + orig->item->offset,
					   text + orig->item->offset + split_index);
  new->item = vogue_item_split (orig->item, split_index, split_offset);

  new->glyphs = vogue_glyph_string_new ();
  vogue_glyph_string_set_size (new->glyphs, num_glyphs);

  if (LTR (orig))
    {
      memcpy (new->glyphs->glyphs, orig->glyphs->glyphs, num_glyphs * sizeof (VogueGlyphInfo));
      ...
      memmove (orig->glyphs->glyphs, orig->glyphs->glyphs + num_glyphs,
	       num_remaining * sizeof (VogueGlyphInfo));
      for (i = num_glyphs; i < orig->glyphs->num_glyphs; i++)
	orig->glyphs->log_clusters[i - num_glyphs] = orig->glyphs->log_clusters[i] - split_index;
    }
  else
    {
      memcpy (new->glyphs->glyphs, orig->glyphs->glyphs + num_remaining, num_glyphs * sizeof (VogueGlyphInfo));
  ...
  vogue_glyph_string_set_size (orig->glyphs, orig->glyphs->num_glyphs - num_glyphs);

  return new;
}

/**
 * vogue_glyph_item_copy:
 * @orig: (nullable): a #VogueGlyphItem, may be %NULL
 *
 * Make a deep copy of an existing #VogueGlyphItem structure.
 *
 * Return value: (nullable): the newly allocated #VogueGlyphItem, which should
 *               be freed with vogue_glyph_item_free(), or %NULL
 *               if @orig was %NULL.
 *
 * Since: 1.20
 **/
VogueGlyphItem *
vogue_glyph_item_copy  (VogueGlyphItem *orig)
{
  VogueGlyphItem *result;

  if (orig == NULL)
    return NULL;

  result = g_slice_new (VogueGlyphItem);

  result->item = vogue_item_copy (orig->item);
  result->glyphs = vogue_glyph_string_copy (orig->glyphs);

  return result;
}

/**
 * vogue_glyph_item_free:
 * @glyph_item: (nullable): a #VogueGlyphItem, may be %NULL
 *
 * Frees a #VogueGlyphItem and resources to which it points.
 *
 * Since: 1.6
 **/
void
vogue_glyph_item_free  (VogueGlyphItem *glyph_item)
{
  if (glyph_item == NULL)
    return;

  if (glyph_item->item)
    vogue_item_free (glyph_item->item);
  if (glyph_item->glyphs)
    vogue_glyph_string_free (glyph_item->glyphs);

  g_slice_free (VogueGlyphItem, glyph_item);
}

G_DEFINE_BOXED_TYPE (VogueGlyphItem, vogue_glyph_item,
                     vogue_glyph_item_copy,
                     vogue_glyph_item_free);


/**
 * vogue_glyph_item_iter_copy:
 * @orig: (nullable): a #VogueGlyphItemIter, may be %NULL
 *
 * Make a shallow copy of an existing #VogueGlyphItemIter structure.
 *
 * Return value: (nullable): the newly allocated #VogueGlyphItemIter, which should
 *               be freed with vogue_glyph_item_iter_free(), or %NULL
 *               if @orig was %NULL.
 *
 * Since: 1.22
 **/
VogueGlyphItemIter *
vogue_glyph_item_iter_copy  (VogueGlyphItemIter *orig)
{
  VogueGlyphItemIter *result;

  if (orig == NULL)
    return NULL;

  result = g_slice_new (VogueGlyphItemIter);
...
/**
 * vogue_glyph_item_iter_free:
 * @iter: (nullable): a #VogueGlyphItemIter, may be %NULL
 *
 * Frees a #VogueGlyphItemIter created by vogue_glyph_item_iter_copy().
 *
 * Since: 1.22
 **/
void
vogue_glyph_item_iter_free  (VogueGlyphItemIter *iter)
{
  if (iter == NULL)
    return;

  g_slice_free (VogueGlyphItemIter, iter);
}

G_DEFINE_BOXED_TYPE (VogueGlyphItemIter, vogue_glyph_item_iter,
                     vogue_glyph_item_iter_copy,
                     vogue_glyph_item_iter_free)

/**
 * vogue_glyph_item_iter_next_cluster:
 * @iter: a #VogueGlyphItemIter
 *
 * Advances the iterator to the next cluster in the glyph item.
 * See #VogueGlyphItemIter for details of cluster orders.
 *
 ...
 **/
gboolean
vogue_glyph_item_iter_next_cluster (VogueGlyphItemIter *iter)
{
  int glyph_index = iter->end_glyph;
  VogueGlyphString *glyphs = iter->glyph_item->glyphs;
  int cluster;
  VogueItem *item = iter->glyph_item->item;
	  ...
	  if (glyphs->log_clusters[glyph_index] > cluster)
	    {
	      ...
	      iter->end_char += vogue_utf8_strlen (iter->text + iter->start_index,
...
/**
 * vogue_glyph_item_iter_prev_cluster:
 * @iter: a #VogueGlyphItemIter
 *
 ...
 **/
gboolean
vogue_glyph_item_iter_prev_cluster (VogueGlyphItemIter *iter)
{
	  ...
	  if (glyphs->log_clusters[glyph_index] < cluster)
	    {
	      ...
	      iter->start_char -= vogue_utf8_strlen (iter->text + iter->start_index,
						 iter->end_index - iter->start_index);
	      break;
	    }
	}
...
/**
 * vogue_glyph_item_iter_init_start:
 * @iter: a #VogueGlyphItemIter
 * @glyph_item: the glyph item to iterate over
 * @text: text corresponding to the glyph item
 *
 * Initializes a #VogueGlyphItemIter structure to point to the
 ...
 **/
gboolean
vogue_glyph_item_iter_init_start (VogueGlyphItemIter  *iter,
				  ...
				  const char          *text)
{
  ...
  return vogue_glyph_item_iter_next_cluster (iter);
}

/**
 * vogue_glyph_item_iter_init_end:
 * @iter: a #VogueGlyphItemIter
 * @glyph_item: the glyph item to iterate over
 * @text: text corresponding to the glyph item
 *
 ...
 **/
gboolean
vogue_glyph_item_iter_init_end (VogueGlyphItemIter  *iter,
				VogueGlyphItem      *glyph_item,
				const char          *text)
{
  iter->glyph_item = glyph_item;
  ...
  /* Advance onto the first cluster of the glyph item */
  return vogue_glyph_item_iter_prev_cluster (iter);
}
 ...
 */
static void
append_attrs (VogueGlyphItem *glyph_item,
	      GSList         *attrs)
{
...
/* Make a deep copy of a #GSList of VogueAttribute
 */
static GSList *
attr_slist_copy (GSList *attrs)
{
  ...
  for (tmp_list = new_attrs; tmp_list; tmp_list = tmp_list->next)
    tmp_list->data = vogue_attribute_copy (tmp_list->data);
 ...
 */
static VogueGlyphItem *
split_before_cluster_start (ApplyAttrsState *state)
{
  VogueGlyphItem *split_item;
  int split_len = state->iter.start_index - state->iter.glyph_item->item->offset;

  split_item = vogue_glyph_item_split (state->iter.glyph_item, state->iter.text, split_len);
...
/**
 * vogue_glyph_item_apply_attrs:
 * @glyph_item: a shaped item
 * @text: text that @list applies to
 * @list: a #VogueAttrList
 *
 * Splits a shaped item (VogueGlyphItem) into multiple items based
 * on an attribute list. The idea is that if you have attributes
 * that don't affect shaping, such as color or underline, to avoid
 * affecting shaping, you filter them out (vogue_attr_list_filter()),
 ...
 *
 * Return value: (transfer full) (element-type Vogue.GlyphItem): a
 *   list of glyph items resulting from splitting @glyph_item. Free
 *   the elements using vogue_glyph_item_free(), the list using
 *   g_slist_free().
 *
 * Since: 1.2
 **/
GSList *
vogue_glyph_item_apply_attrs (VogueGlyphItem   *glyph_item,
			      const char       *text,
			      VogueAttrList    *list)
{
  VogueAttrIterator *iter = vogue_attr_list_get_iterator (list);
  GSList *result = NULL;
  ...
  do
    {
      vogue_attr_iterator_range (iter, &range_start, &range_end);
      if (range_end > glyph_item->item->offset)
	break;
    }
  while (vogue_attr_iterator_next (iter));

  state.segment_attrs = vogue_attr_iterator_get_attrs (iter);
  ...
  for (have_cluster = vogue_glyph_item_iter_init_start (&state.iter, glyph_item, text);
       have_cluster;
       have_cluster = vogue_glyph_item_iter_next_cluster (&state.iter))
    {
      gboolean have_next;
      ...
      do
	{
	  ...
	  have_next = vogue_attr_iterator_next (iter);
	  vogue_attr_iterator_range (iter, &range_start, &range_end);
	  ...
	  state.segment_attrs = g_slist_concat (state.segment_attrs,
						vogue_attr_iterator_get_attrs (iter));
	}
      while (have_next);
    }
 ...
 *   in Vogue units. May be negative, though too large
 ...
 **/
void
vogue_glyph_item_letter_space (VogueGlyphItem *glyph_item,
			       const char     *text,
			       VogueLogAttr   *log_attrs,
			       int             letter_spacing)
{
  VogueGlyphItemIter iter;
  VogueGlyphInfo *glyphs = glyph_item->glyphs->glyphs;
  gboolean have_cluster;
  ...
  for (have_cluster = vogue_glyph_item_iter_init_start (&iter, glyph_item, text);
       have_cluster;
       have_cluster = vogue_glyph_item_iter_next_cluster (&iter))
    {
...
/**
 * vogue_glyph_item_get_logical_widths:
 * @glyph_item: a #VogueGlyphItem
 * @text: text that @glyph_item corresponds to
 *   (glyph_item->item->offset is an offset from the
 *    start of @text)
 ...
 *
 * Given a #VogueGlyphItem and the corresponding
 * text, determine the screen width corresponding to each character. When
 * multiple characters compose a single cluster, the width of the entire
 * cluster is divided equally among the characters.
 *
 * See also vogue_glyph_string_get_logical_widths().
 *
 * Since: 1.26
 **/
void
vogue_glyph_item_get_logical_widths (VogueGlyphItem *glyph_item,
				     const char     *text,
				     int            *logical_widths)
{
  VogueGlyphItemIter iter;
  ...
  for (has_cluster = vogue_glyph_item_iter_init_start (&iter, glyph_item, text);
       has_cluster;
       has_cluster = vogue_glyph_item_iter_next_cluster (&iter))
    {

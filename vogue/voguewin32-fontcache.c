/* Vogue
 * voguewin32-fontcache.c: Cache of HFONTs by LOGFONTW
 *
 ...
 * VogueWin32FontCache:
 *
 * A #VogueWin32FontCache caches HFONTs by their LOGFONT descriptions.
 */
struct _VogueWin32FontCache
{
  GHashTable *forward;
  GHashTable *back;
...
/**
 * vogue_win32_font_cache_free:
 * @cache: a #VogueWin32FontCache
 *
 * Frees a #VogueWin32FontCache and all associated memory. All fonts loaded
 * through this font cache will be freed along with the cache.
 **/
void
vogue_win32_font_cache_free (VogueWin32FontCache *cache)
{
  ...
  g_slice_free (VogueWin32FontCache, cache);
}
 ...
 **/
VogueWin32FontCache *
vogue_win32_font_cache_new (void)
{
  VogueWin32FontCache *cache;

  cache = g_slice_new (VogueWin32FontCache);
...
static void
cache_entry_unref (VogueWin32FontCache *cache,
		   CacheEntry          *entry)
{
 ...
 * vogue_win32_font_cache_unload().
 **/
HFONT
vogue_win32_font_cache_load (VogueWin32FontCache *cache,
			     const LOGFONTA      *lfp)
{
  LOGFONTW lf;
  ...
  *(LOGFONTA *)&lf = *lfp;
  ...
  return vogue_win32_font_cache_loadw (cache, &lf);
}

/**
 * vogue_win32_font_cache_loadw:
 * @cache: a #VogueWin32FontCache
 ...
 **/
HFONT
vogue_win32_font_cache_loadw (VogueWin32FontCache *cache,
			      const LOGFONTW      *lfp)
{
  CacheEntry *entry;
  LOGFONTW lf;
 ...
 *
 * Frees a font structure previously loaded with vogue_win32_font_cache_load().
 **/
void
vogue_win32_font_cache_unload (VogueWin32FontCache *cache,
			       HFONT		    hfont)
{
  CacheEntry *entry;

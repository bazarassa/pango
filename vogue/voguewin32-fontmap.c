/* Vogue
 * voguewin32-fontmap.c: Win32 font handling
 *
...
typedef struct _VogueWin32Family VogueWin32Family;
typedef VogueFontFamilyClass VogueWin32FamilyClass;

struct _VogueWin32Family
{
...
#define PANGO_WIN32_TYPE_FAMILY              (vogue_win32_family_get_type ())
#define PANGO_WIN32_FAMILY(object)           (G_TYPE_CHECK_INSTANCE_CAST ((object), PANGO_WIN32_TYPE_FAMILY, VogueWin32Family))
#define PANGO_WIN32_IS_FAMILY (object)       (G_TYPE_CHECK_INSTANCE_TYPE ((object), PANGO_WIN32_TYPE_FAMILY))
#define PANGO_WIN32_FAMILY_CLASS (klass)     (G_TYPE_CHECK_CLASS_CAST (klass), PANGO_WIN32_TYPE_FAMILY, VogueWin32Family))
#define PANGO_WIN32_IS_FAMILY_CLASS (klass)  (G_TYPE_CHECK_CLASS_CAST (klass), PANGO_WIN32_TYPE_FAMILY))
#define PANGO_WIN32_FAMILY_GET_CLASS(object) (G_TYPE_INSTANCE_GET_CLASS((object), PANGO_WIN32_FAMILY, VogueWin32FamilyClass))

#define PANGO_WIN32_TYPE_FACE              (vogue_win32_face_get_type ())
#define PANGO_WIN32_FACE(object)           (G_TYPE_CHECK_INSTANCE_CAST ((object), PANGO_WIN32_TYPE_FACE, VogueWin32Face))
#define PANGO_WIN32_IS_FACE(object)        (G_TYPE_CHECK_INSTANCE_TYPE ((object), PANGO_WIN32_TYPE_FACE))
#define PANGO_WIN32_FACE_CLASS (klass)     (G_TYPE_CHECK_CLASS_CAST (klass), PANGO_WIN32_TYPE_FACE, VogueWin32Face))
#define PANGO_WIN32_IS_FACE_CLASS (klass)  (G_TYPE_CHECK_CLASS_CAST (klass), PANGO_WIN32_TYPE_FACE))
#define PANGO_WIN32_FACE_GET_CLASS(object) (G_TYPE_INSTANCE_GET_CLASS((object), PANGO_WIN32_FACE, VogueWin32FaceClass))

static GType      vogue_win32_face_get_type          (void);

static GType      vogue_win32_family_get_type        (void);

static void       vogue_win32_face_list_sizes        (VogueFontFace  *face,
                                                      int           **sizes,
                                                      int            *n_sizes);

static void       vogue_win32_font_map_finalize      (GObject                      *object);
static VogueFont *vogue_win32_font_map_load_font     (VogueFontMap                 *fontmap,
                                                      VogueContext                 *context,
                                                      const VogueFontDescription   *description);
static VogueFontset *vogue_win32_font_map_load_fontset (VogueFontMap               *fontmap,
                                                        VogueContext                 *context,
                                                        const VogueFontDescription   *desc,
                                                        VogueLanguage                *language);
static void       vogue_win32_font_map_list_families (VogueFontMap                 *fontmap,
                                                      VogueFontFamily            ***families,
                                                      int                          *n_families);

static VogueFont *vogue_win32_font_map_real_find_font (VogueWin32FontMap           *win32fontmap,
                                                       VogueContext                *context,
                                                       VogueWin32Face              *face,
                                                       const VogueFontDescription  *description);

static void       vogue_win32_fontmap_cache_clear    (VogueWin32FontMap            *win32fontmap);

static void       vogue_win32_insert_font            (VogueWin32FontMap            *fontmap,
                                                      LOGFONTW                     *lfp,
                                                      gboolean                      is_synthetic);

static VogueWin32Family *vogue_win32_get_font_family (VogueWin32FontMap            *win32fontmap,
                                                      const char                   *family_name);

static const char *vogue_win32_face_get_face_name    (VogueFontFace *face);

static VogueWin32FontMap *default_fontmap = NULL; /* MT-safe */

G_DEFINE_TYPE (VogueWin32FontMap, _vogue_win32_font_map, PANGO_TYPE_FONT_MAP)
...
static int CALLBACK
vogue_win32_inner_enum_proc (LOGFONTW    *lfp,
                             TEXTMETRICW *metrics,
                             DWORD        fontType,
                             LPARAM       lParam)
{
  VogueWin32FontMap *win32fontmap = (VogueWin32FontMap *)lParam;

  /* Windows generates synthetic vertical writing versions of East
   * Asian fonts with @ prepended to their name, ignore them.
   */
  if (lfp->lfFaceName[0] != '@')
    vogue_win32_insert_font (win32fontmap, lfp, FALSE);

  return 1;
}

static int CALLBACK
vogue_win32_enum_proc (LOGFONTW       *lfp,
  ...
  if (fontType == TRUETYPE_FONTTYPE || ((metrics->ntmFlags & NTM_PS_OPENTYPE) || (metrics->ntmFlags & NTM_TYPE1)))
    {
      ...
      EnumFontFamiliesExW (_vogue_win32_hdc, &lf,
                           (FONTENUMPROCW) vogue_win32_inner_enum_proc,
                           lParam, 0);
    }
...
synthesize_foreach (gpointer key,
                    ...
                    gpointer user_data)
{
  VogueWin32Family *win32family = value;
  VogueWin32FontMap *win32fontmap = user_data;

  enum { NORMAL, BOLDER, SLANTED };
  VogueWin32Face *variant[4] = { NULL, NULL, NULL, NULL };
  ...
  while (p)
    {
      VogueWin32Face *win32face = p->data;
      ...
      if (vogue_font_description_get_weight (win32face->description) == PANGO_WEIGHT_NORMAL &&
          vogue_font_description_get_style (win32face->description) == PANGO_STYLE_NORMAL)
      ...
      if (vogue_font_description_get_weight (win32face->description) == PANGO_WEIGHT_NORMAL &&
          vogue_font_description_get_style (win32face->description) >= PANGO_STYLE_OBLIQUE)
  ...
  if (variant[NORMAL] != NULL && variant[BOLDER] == NULL)
    {
      ...
      vogue_win32_insert_font (win32fontmap, &lf, TRUE);
    }
...
static guint
alias_hash (struct VogueAlias *alias)
{
  return g_str_hash (alias->alias);
}

static gboolean
alias_equal (struct VogueAlias *alias1,
             struct VogueAlias *alias2)
{
...
static void
alias_free (struct VogueAlias *alias)
{
  int i;
  g_free (alias->alias);
  ...
  g_slice_free (struct VogueAlias, alias);
}
                   ...
                   GHashTable *ht_aliases)
{
  ...
  GString *tmp_buffer2;
  const char *pos;
  struct VogueAlias alias_key;
  struct VogueAlias *alias;
  ...
  pos = line_buffer->str;
  if (!vogue_skip_space (&pos))
    return;

  if (!vogue_scan_string (&pos, tmp_buffer1) ||
      !vogue_skip_space (&pos))
    {
  ...
  if (!vogue_scan_string (&pos, tmp_buffer2))
    {
  ...
  if (!alias)
    {
      alias = g_slice_new0 (struct VogueAlias);
...
static void
create_standard_family (VogueWin32FontMap *win32fontmap,
                        const char        *standard_family_name)
{
  ...
  for (i = 0; i < n_aliases; i++)
    {
      VogueWin32Family *existing_family = g_hash_table_lookup (win32fontmap->families, aliases[i]);

      if (existing_family)
        {
          VogueWin32Family *new_family = vogue_win32_get_font_family (win32fontmap, standard_family_name);
          ...
          while (p)
            {
              const VogueWin32Face *old_face = p->data;
              VogueWin32Face *new_face = g_object_new (PANGO_WIN32_TYPE_FACE, NULL);

              new_face->logfontw = old_face->logfontw;
              new_face->description = vogue_font_description_copy_static (old_face->description);
              vogue_font_description_set_family_static (new_face->description, standard_family_name);

              if (old_face->coverage != NULL)
                new_face->coverage = vogue_coverage_ref (old_face->coverage);
              else
...
static void
_vogue_win32_font_map_init (VogueWin32FontMap *win32fontmap)
{
  LOGFONTW logfont;
  ...
  win32fontmap->font_cache = vogue_win32_font_cache_new ();
  ...
  EnumFontFamiliesExW (_vogue_win32_hdc, &logfont,
                       (FONTENUMPROCW) vogue_win32_enum_proc,
  ...
  win32fontmap->resolution = (PANGO_SCALE / (double) GetDeviceCaps (_vogue_win32_hdc, LOGPIXELSY)) * 72.0;
}

static void
vogue_win32_font_map_fontset_add_fonts (VogueFontMap          *fontmap,
                                        VogueContext          *context,
                                        VogueFontsetSimple *fonts,
                                        VogueFontDescription  *desc,
                                        const char            *family)
{
  /* Mostly use the "old" vogue_font_map_fontset_add_fonts() */
  ...
  if (n_aliases)
  {
    ...
    {
      vogue_font_description_set_family_static (desc, aliases[j]);
      font = vogue_win32_font_map_load_font (fontmap, context, desc);
      if (font)
...
static void
_vogue_win32_font_map_class_init (VogueWin32FontMapClass *class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);
  VogueFontMapClass *fontmap_class = PANGO_FONT_MAP_CLASS (class);

  class->find_font = vogue_win32_font_map_real_find_font;
  object_class->finalize = vogue_win32_font_map_finalize;
  fontmap_class->load_font = vogue_win32_font_map_load_font;
  /* we now need a load_fontset implementation for the Win32 backend */
  fontmap_class->load_fontset = vogue_win32_font_map_load_fontset;
  fontmap_class->list_families = vogue_win32_font_map_list_families;
...
/**
 * vogue_win32_font_map_for_display:
 *
 * Returns a <type>VogueWin32FontMap</type>. Font maps are cached and should
 * not be freed. If the font map is no longer needed, it can
 * be released with vogue_win32_shutdown_display().
 *
 * Return value: a #VogueFontMap.
 **/
VogueFontMap *
vogue_win32_font_map_for_display (void)
{
...
/**
 * vogue_win32_shutdown_display:
 *
 * Free cached resources.
 **/
void
vogue_win32_shutdown_display (void)
{
  if (default_fontmap)
    {
      vogue_win32_fontmap_cache_clear (default_fontmap);
...
static void
vogue_win32_font_map_finalize (GObject *object)
{
  VogueWin32FontMap *win32fontmap = PANGO_WIN32_FONT_MAP (object);

  g_list_foreach (win32fontmap->freed_fonts->head, (GFunc)g_object_unref, NULL);
  g_queue_free (win32fontmap->freed_fonts);

  vogue_win32_font_cache_free (win32fontmap->font_cache);
  ...
  G_OBJECT_CLASS (_vogue_win32_font_map_parent_class)->finalize (object);
}
 ...
 */
static void
vogue_win32_family_list_faces (VogueFontFamily  *family,
                               VogueFontFace  ***faces,
                               int              *n_faces)
{
  VogueWin32Family *win32family = PANGO_WIN32_FAMILY (family);
  GSList *p;
  int n;
...
static const char *
vogue_win32_family_get_name (VogueFontFamily  *family)
{
  VogueWin32Family *win32family = PANGO_WIN32_FAMILY (family);
...
static gboolean
vogue_win32_family_is_monospace (VogueFontFamily *family)
{
  VogueWin32Family *win32family = PANGO_WIN32_FAMILY (family);
...
G_DEFINE_TYPE (VogueWin32Family, vogue_win32_family, PANGO_TYPE_FONT_FAMILY)

static void
vogue_win32_family_finalize (GObject *object)
{
  VogueWin32Family *win32family = PANGO_WIN32_FAMILY (object);

  g_free (win32family->family_name);

  g_slist_free_full (win32family->faces, g_object_unref);

  G_OBJECT_CLASS (vogue_win32_family_parent_class)->finalize (object);
}

static void
vogue_win32_family_class_init (VogueFontFamilyClass *class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);

  object_class->finalize = vogue_win32_family_finalize;
  class->list_faces = vogue_win32_family_list_faces;
  class->get_name = vogue_win32_family_get_name;
  class->is_monospace = vogue_win32_family_is_monospace;
}

static void
vogue_win32_family_init (VogueWin32Family *family)
{
}
...
static void
vogue_win32_font_map_list_families (VogueFontMap      *fontmap,
                                    VogueFontFamily ***families,
                                    int               *n_families)
{
  GSList *family_list = NULL;
  GSList *tmp_list;
  VogueWin32FontMap *win32fontmap = (VogueWin32FontMap *)fontmap;
...
static VogueWin32Family *
vogue_win32_get_font_family (VogueWin32FontMap *win32fontmap,
                             const char        *family_name)
{
  VogueWin32Family *win32family = g_hash_table_lookup (win32fontmap->families, family_name);
  if (!win32family)
    {
...
static VogueFont *
vogue_win32_font_map_load_font (VogueFontMap               *fontmap,
                                VogueContext               *context,
                                const VogueFontDescription *description)
{
  VogueWin32FontMap *win32fontmap = (VogueWin32FontMap *)fontmap;
  VogueWin32Family *win32family;
  VogueFont *result = NULL;
  GSList *tmp_list;
  const char *family;
  ...
  if (win32family)
    {
      VogueWin32Face *best_match = NULL;

      PING (("got win32family"));
      tmp_list = win32family->faces;
      while (tmp_list)
        {
          VogueWin32Face *face = tmp_list->data;

          if (vogue_font_description_better_match (description,
...
static VogueWin32Font *
vogue_win32_font_neww (VogueFontMap   *fontmap,
                       const LOGFONTW *lfp,
                       int             size)
{
  VogueWin32Font *result;

  g_return_val_if_fail (fontmap != NULL, NULL);
  g_return_val_if_fail (lfp != NULL, NULL);

  result = (VogueWin32Font *)g_object_new (PANGO_TYPE_WIN32_FONT, NULL);
  ...
  result->size = size;
  _vogue_win32_make_matching_logfontw (fontmap, lfp, size, &result->logfontw);

  return result;
}

static VogueFont *
vogue_win32_font_map_real_find_font (VogueWin32FontMap          *win32fontmap,
                                     VogueContext               *context,
                                     VogueWin32Face             *face,
                                     const VogueFontDescription *description)
{
  VogueFontMap *fontmap = PANGO_FONT_MAP (win32fontmap);
  VogueWin32Font *win32font;
  GSList *tmp_list = face->cached_fonts;
  int size = vogue_font_description_get_size (description);

  if (vogue_font_description_get_size_is_absolute (description))
      ...
      if (win32font->size == size)
        {
          ...
          if (win32font->in_cache)
            _vogue_win32_fontmap_cache_remove (fontmap, win32font);

          return (VogueFont *)win32font;
        }

      tmp_list = tmp_list->next;
    }

  win32font = vogue_win32_font_neww (fontmap, &face->logfontw, size);
...
static gboolean
_vogue_win32_get_name_header (HDC                 hdc,
...
static gboolean
_vogue_win32_get_name_record (HDC                 hdc,
			      gint                i,
...
get_family_nameA (const LOGFONTA *lfp)
{
  ...
  if ((oldhfont = SelectObject (_vogue_win32_hdc, hfont)) == NULL)
    goto fail1;

  if (!_vogue_win32_get_name_header (_vogue_win32_hdc, &header))
    goto fail2;
  ...
  for (i = 0; i < header.num_records; i++)
    {
      if (!_vogue_win32_get_name_record (_vogue_win32_hdc, i, &record))
        goto fail2;
  ...
  if (!_vogue_win32_get_name_record (_vogue_win32_hdc, name_ix, &record))
    goto fail2;

  string = g_malloc (record.string_length + 1);
  if (GetFontData (_vogue_win32_hdc, NAME,
  ...
  SelectObject (_vogue_win32_hdc, oldhfont);
  DeleteObject (hfont);
...
/**
 * vogue_win32_font_description_from_logfont:
 * @lfp: a LOGFONTA
 *
 * Creates a #VogueFontDescription that matches the specified LOGFONTA.
 *
 * The face name, italicness and weight fields in the LOGFONTA are used
 * to set up the resulting #VogueFontDescription. If the face name in
...
vogue_win32_font_description_from_logfont (const LOGFONT *lfp)
{
  ...
  if (lfp->lfWeight == FW_DONTCARE)
    ...
    /* The VogueWeight values PANGO_WEIGHT_* map exactly to Windows FW_*. */
    weight = (VogueWeight) lfp->lfWeight;

  /* XXX No idea how to figure out the stretch */
  stretch = PANGO_STRETCH_NORMAL;

  description = vogue_font_description_new ();
  vogue_font_description_set_family (description, family);
  g_free(family);
  vogue_font_description_set_style (description, style);
  vogue_font_description_set_weight (description, weight);
  vogue_font_description_set_stretch (description, stretch);
  vogue_font_description_set_variant (description, variant);
...
/**
 * vogue_win32_font_description_from_logfontw:
 * @lfp: a LOGFONTW
 *
 * Creates a #VogueFontDescription that matches the specified LOGFONTW.
 *
 ...
 */
VogueFontDescription *
vogue_win32_font_description_from_logfontw (const LOGFONTW *lfp)
{
  VogueFontDescription *description;
  gchar *family;
  VogueStyle style;
...
static void
vogue_win32_insert_font (VogueWin32FontMap *win32fontmap,
                         LOGFONTW          *lfp,
                         gboolean           is_synthetic)
{
  LOGFONTW *lfp2 = NULL;
  ...
  *lfp2 = *lfp;
  ...
  description = vogue_win32_font_description_from_logfontw (lfp2);
  ...
  if (!vogue_font_description_get_family (description))
    {
      vogue_font_description_free (description);
      return;
    }
  ...
  win32family =
    vogue_win32_get_font_family (win32fontmap,
                                 vogue_font_description_get_family (win32face->description));
 ...
 */
void
_vogue_win32_make_matching_logfontw (VogueFontMap   *fontmap,
                                     const LOGFONTW *lfp,
                                     int             size,
                                     LOGFONTW       *out)
{
...
vogue_win32_face_describe (VogueFontFace *face)
{
  VogueWin32Face *win32face = PANGO_WIN32_FACE (face);

  return vogue_font_description_copy (win32face->description);
}

static const char *
vogue_win32_face_get_face_name (VogueFontFace *face)
{
  VogueWin32Face *win32face = PANGO_WIN32_FACE (face);

  if (!win32face->face_name)
    {
      VogueFontDescription *desc = vogue_font_face_describe (face);

      vogue_font_description_unset_fields (desc,
                                           PANGO_FONT_MASK_FAMILY | PANGO_FONT_MASK_SIZE);

      win32face->face_name = vogue_font_description_to_string (desc);
...
static gboolean
vogue_win32_face_is_synthesized (VogueFontFace *face)
{
  VogueWin32Face *win32face = PANGO_WIN32_FACE (face);
...
G_DEFINE_TYPE (VogueWin32Face, vogue_win32_face, PANGO_TYPE_FONT_FACE)

static void
vogue_win32_face_finalize (GObject *object)
{
  VogueWin32Face *win32face = PANGO_WIN32_FACE (object);

  vogue_font_description_free (win32face->description);

  if (win32face->coverage != NULL)
    vogue_coverage_unref (win32face->coverage);
...
static void
vogue_win32_face_class_init (VogueFontFaceClass *class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);

  object_class->finalize = vogue_win32_face_finalize;
  class->describe = vogue_win32_face_describe;
  class->get_face_name = vogue_win32_face_get_face_name;
  class->list_sizes = vogue_win32_face_list_sizes;
  class->is_synthesized = vogue_win32_face_is_synthesized;
}

static void
vogue_win32_face_init (VogueWin32Face *face)
{
}

static void
vogue_win32_face_list_sizes (VogueFontFace  *face,
                             int           **sizes,
                             int            *n_sizes)
{
  /*
   * for scalable fonts it's simple, and currently we only have such
   * see : vogue_win32_enum_proc(), TRUETYPE_FONTTYPE
   */
  *sizes = NULL;
  *n_sizes = 0;
}
...
/**
 * vogue_win32_font_map_get_font_cache:
 * @font_map: a <type>VogueWin32FontMap</type>.
 *
 * Obtains the font cache associated with the given font map.
 *
 * Return value: the #VogueWin32FontCache of @font_map.
 **/
VogueWin32FontCache *
vogue_win32_font_map_get_font_cache (VogueFontMap *font_map)
{
...
void
_vogue_win32_fontmap_cache_remove (VogueFontMap   *fontmap,
                                   VogueWin32Font *win32font)
{
  VogueWin32FontMap *win32fontmap = PANGO_WIN32_FONT_MAP (fontmap);
...
static void
vogue_win32_fontmap_cache_clear (VogueWin32FontMap *win32fontmap)
{
  g_list_foreach (win32fontmap->freed_fonts->head, (GFunc)g_object_unref, NULL);
  g_queue_free (win32fontmap->freed_fonts);
  win32fontmap->freed_fonts = g_queue_new ();
...
static VogueFontset *
vogue_win32_font_map_load_fontset (VogueFontMap                 *fontmap,
                                   VogueContext                 *context,
                                   const VogueFontDescription   *desc,
                                   VogueLanguage                *language)
{
  ...
  /* (Copied directly from vogue-fontmap.c) */
  VogueFontDescription *tmp_desc = vogue_font_description_copy_static (desc);
  const char *family;
  char **families;
  int i;
  VogueFontsetSimple *fonts;
  VogueWin32FontMap *win32fontmap = NULL;
  ...
  for (i = 0; families[i]; i++)
    vogue_win32_font_map_fontset_add_fonts (fontmap,
                                            context,
                                            fonts,
                                            tmp_desc,
                                            families[i]);

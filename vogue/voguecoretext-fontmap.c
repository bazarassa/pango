...
typedef struct _VogueCoreTextFontset VogueCoreTextFontset;

#define PANGO_TYPE_CORE_TEXT_FAMILY              (vogue_core_text_family_get_type ())
#define PANGO_CORE_TEXT_FAMILY(object)           (G_TYPE_CHECK_INSTANCE_CAST ((object), PANGO_TYPE_CORE_TEXT_FAMILY, VogueCoreTextFamily))
#define PANGO_IS_CORE_TEXT_FAMILY(object)        (G_TYPE_CHECK_INSTANCE_TYPE ((object), PANGO_TYPE_CORE_TEXT_FAMILY))
#define PANGO_CORE_TEXT_FAMILY_CLASS(klass)	(G_TYPE_CHECK_CLASS_CAST ((klass), PANGO_CORE_TEXT_FAMILY, VogueCoreTextFamilyClass))
#define PANGO_IS_CORE_TEXT_FAMILY_CLASS(klass)	(G_TYPE_CHECK_CLASS_TYPE ((klass), PANGO_CORE_TEXT_FAMILY))
#define PANGO_CORE_TEXT_FAMILY_GET_CLASS(object) (G_TYPE_INSTANCE_GET_CLASS((object), PANGO_CORE_TEXT_FAMILY, VogueCoreTextFamilyClass))

#define PANGO_TYPE_CORE_TEXT_FONTSET           (vogue_core_text_fontset_get_type ())
#define PANGO_CORE_TEXT_FONTSET(object)        (G_TYPE_CHECK_INSTANCE_CAST ((object), PANGO_TYPE_CORE_TEXT_FONTSET, VogueCoreTextFontset))
#define PANGO_IS_CORE_TEXT_FONTSET(object)     (G_TYPE_CHECK_INSTANCE_TYPE ((object), PANGO_TYPE_CORE_TEXT_FONTSET))


struct _VogueCoreTextFamily
{
...
struct _VogueCoreTextFamilyClass
{
  VogueFontFamilyClass parent_class;
};

typedef struct _VogueCoreTextFamilyClass VogueCoreTextFamilyClass;

#define PANGO_TYPE_CORE_TEXT_FACE              (vogue_core_text_face_get_type ())
#define PANGO_CORE_TEXT_FACE(object)           (G_TYPE_CHECK_INSTANCE_CAST ((object), PANGO_TYPE_CORE_TEXT_FACE, VogueCoreTextFace))
#define PANGO_IS_CORE_TEXT_FACE(object)        (G_TYPE_CHECK_INSTANCE_TYPE ((object), PANGO_TYPE_CORE_TEXT_FACE))
#define PANGO_CORE_TEXT_FACE_CLASS(klass)	(G_TYPE_CHECK_CLASS_CAST ((klass), PANGO_CORE_TEXT_FACE, VogueCoreTextFaceClass))
#define PANGO_IS_CORE_TEXT_FACE_CLASS(klass)	(G_TYPE_CHECK_CLASS_TYPE ((klass), PANGO_CORE_TEXT_FACE))
#define PANGO_CORE_TEXT_FACE_GET_CLASS(object) (G_TYPE_INSTANCE_GET_CLASS((object), PANGO_CORE_TEXT_FACE, VogueCoreTextFaceClass))

struct _VogueCoreTextFace
{
  VogueFontFace parent_instance;

  VogueCoreTextFamily *family;
...
struct _VogueCoreTextFaceClass
{
  VogueFontFaceClass parent_class;
};

typedef struct _VogueCoreTextFaceClass VogueCoreTextFaceClass;

static GType vogue_core_text_family_get_type (void);
static GType vogue_core_text_face_get_type (void);
static GType vogue_core_text_fontset_get_type (void);

static VogueCoreTextFontset    *vogue_core_text_fontset_new     (VogueCoreTextFontsetKey    *key,
                                                                 const VogueFontDescription *description);
static VogueCoreTextFontsetKey *vogue_core_text_fontset_get_key (VogueCoreTextFontset       *fontset);

/*
 * Helper functions to translate CoreText data to Vogue
 */

typedef struct
{
    float ct_weight;
    VogueWeight vogue_weight;
} VogueCTWeight;
...
static const VogueCTWeight ct_weight_map[] = {
...
ct_font_descriptor_get_weight (CTFontDescriptorRef desc)
{
  ...
  CFNumberRef cf_number;
  CGFloat value;
  VogueWeight weight = PANGO_WEIGHT_NORMAL;
    ...
    if (value < ct_weight_min || value > ct_weight_max)
    ...
    else
      {
        ...
        weight = lerp(value, ct_weight_map[i-1].ct_weight, ct_weight_map[i].ct_weight,
                      ct_weight_map[i-1].vogue_weight, ct_weight_map[i].vogue_weight);
...
vogue_core_text_style_name_is_oblique (const char *style_name)
{
  if (!style_name)
    return FALSE;
...
_vogue_core_text_font_description_from_ct_font_descriptor (CTFontDescriptorRef desc)
{
  SInt64 font_traits;
  char *family_name;
  char *style_name;
  ...
  font_desc = vogue_font_description_new ();
  ...
  vogue_font_description_set_family (font_desc, family_name);
  g_free (family_name);

  /* Weight */
  vogue_font_description_set_weight (font_desc,
  ...
  if ((font_traits & kCTFontItalicTrait) == kCTFontItalicTrait)
    vogue_font_description_set_style (font_desc, PANGO_STYLE_ITALIC);
  else if (vogue_core_text_style_name_is_oblique (style_name))
    vogue_font_description_set_style (font_desc, PANGO_STYLE_OBLIQUE);
  else
    vogue_font_description_set_style (font_desc, PANGO_STYLE_NORMAL);

  if ((font_traits & kCTFontCondensedTrait) == kCTFontCondensedTrait)
    vogue_font_description_set_stretch (font_desc, PANGO_STRETCH_CONDENSED);

  if (ct_font_descriptor_is_small_caps (desc))
    vogue_font_description_set_variant (font_desc, PANGO_VARIANT_SMALL_CAPS);
  else
    vogue_font_description_set_variant (font_desc, PANGO_VARIANT_NORMAL);
...
static inline gboolean
vogue_core_text_face_is_oblique (VogueCoreTextFace *face)
{
  return vogue_core_text_style_name_is_oblique (face->style_name);
}

static void
vogue_core_text_face_make_italic (VogueCoreTextFace *ctface,
...
static inline VogueCoreTextFace *
vogue_core_text_face_copy (const VogueCoreTextFace *old)
{
...
static inline VogueCoreTextFace *
vogue_core_text_face_from_ct_font_descriptor (CTFontDescriptorRef desc)
{
  VogueCoreTextFace *face = g_object_new (PANGO_TYPE_CORE_TEXT_FACE,
                                          NULL);
...
vogue_core_text_face_describe (VogueFontFace *face)
{
  VogueCoreTextFace *ctface = PANGO_CORE_TEXT_FACE (face);

  return _vogue_core_text_font_description_from_ct_font_descriptor (ctface->ctfontdescriptor);
}

static const char *
vogue_core_text_face_get_face_name (VogueFontFace *face)
{
  VogueCoreTextFace *ctface = PANGO_CORE_TEXT_FACE (face);
...
static void
vogue_core_text_face_list_sizes (VogueFontFace  *face,
                                 int           **sizes,
                                 int            *n_sizes)
{
  *n_sizes = 0;
...
G_DEFINE_TYPE (VogueCoreTextFace, vogue_core_text_face, PANGO_TYPE_FONT_FACE);

static void
vogue_core_text_face_init (VogueCoreTextFace *face)
{
  face->family = NULL;
}

static void
vogue_core_text_face_finalize (GObject *object)
{
  VogueCoreTextFace *ctface = PANGO_CORE_TEXT_FACE (object);

  g_free (ctface->style_name);
  CFRelease (ctface->ctfontdescriptor);

  G_OBJECT_CLASS (vogue_core_text_face_parent_class)->finalize (object);
}

static gboolean
vogue_core_text_face_is_synthesized (VogueFontFace *face)
{
  VogueCoreTextFace *cface = PANGO_CORE_TEXT_FACE (face);

  return cface->synthetic_italic;
}

static void
vogue_core_text_face_class_init (VogueCoreTextFaceClass *klass)
{
  GObjectClass *object_class = (GObjectClass *)klass;
  VogueFontFaceClass *pfclass = PANGO_FONT_FACE_CLASS(klass);

  object_class->finalize = vogue_core_text_face_finalize;

  pfclass->describe = vogue_core_text_face_describe;
  pfclass->get_face_name = vogue_core_text_face_get_face_name;
  pfclass->list_sizes = vogue_core_text_face_list_sizes;
  pfclass->is_synthesized = vogue_core_text_face_is_synthesized;
}
...
static void
vogue_core_text_family_list_faces (VogueFontFamily  *family,
                                   VogueFontFace  ***faces,
                                   int              *n_faces)
{
  VogueCoreTextFamily *ctfamily = PANGO_CORE_TEXT_FAMILY (family);
  ...
  if (ctfamily->n_faces < 0)
    {
      ...
      for (l = faces; l; l = l->next)
        {
          VogueCoreTextFace *face = l->data;

          if (!g_hash_table_lookup (italic_faces,
                                    GINT_TO_POINTER ((gint)face->weight)))
            {
              VogueCoreTextFace *italic_face;

              italic_face = vogue_core_text_face_copy (face);

              italic_face->family = ctfamily;
              vogue_core_text_face_make_italic (italic_face, TRUE);
      ...
      ctfamily->faces = g_new (VogueFontFace *, ctfamily->n_faces);
  ...
  if (faces)
    *faces = g_memdup (ctfamily->faces, ctfamily->n_faces * sizeof (VogueFontFace *));
}

static const char *
vogue_core_text_family_get_name (VogueFontFamily *family)
{
  VogueCoreTextFamily *ctfamily = PANGO_CORE_TEXT_FAMILY (family);
...
static gboolean
vogue_core_text_family_is_monospace (VogueFontFamily *family)
{
  VogueCoreTextFamily *ctfamily = PANGO_CORE_TEXT_FAMILY (family);
...
G_DEFINE_TYPE (VogueCoreTextFamily, vogue_core_text_family, PANGO_TYPE_FONT_FAMILY);

static void
vogue_core_text_family_finalize (GObject *object)
{
  VogueCoreTextFamily *family = PANGO_CORE_TEXT_FAMILY (object);
  int i;
  ...
  G_OBJECT_CLASS (vogue_core_text_family_parent_class)->finalize (object);
}


static void
vogue_core_text_family_class_init (VogueCoreTextFamilyClass *klass)
{
  GObjectClass *object_class = (GObjectClass *)klass;
  VogueFontFamilyClass *pfclass = PANGO_FONT_FAMILY_CLASS(klass);

  object_class->finalize = vogue_core_text_family_finalize;

  pfclass->list_faces = vogue_core_text_family_list_faces;
  pfclass->get_name = vogue_core_text_family_get_name;
  pfclass->is_monospace = vogue_core_text_family_is_monospace;
}

static void
vogue_core_text_family_init (VogueCoreTextFamily *family)
{
  family->n_faces = -1;
}
...
static void vogue_core_text_font_map_class_init (VogueCoreTextFontMapClass *class);
static void vogue_core_text_font_map_init (VogueCoreTextFontMap *ctfontmap);


G_DEFINE_TYPE (VogueCoreTextFontMap, vogue_core_text_font_map, PANGO_TYPE_FONT_MAP);

static void
vogue_core_text_font_map_finalize (GObject *object)
{
  VogueCoreTextFontMap *fontmap = PANGO_CORE_TEXT_FONT_MAP (object);

  g_hash_table_destroy (fontmap->fontset_hash);
  g_hash_table_destroy (fontmap->font_hash);
  g_hash_table_destroy (fontmap->families);

  G_OBJECT_CLASS (vogue_core_text_font_map_parent_class)->finalize (object);
}
		    ...
		    VogueMatrix *matrix)
{
  ...
  if (context)
    set_matrix = vogue_context_get_matrix (context);
  else
    set_matrix = NULL;
...
/*
 * Helper functions for VogueCoreTextFontsetKey
 */
static const double ppi = 72.0; /* typographic points per inch */

static double
vogue_core_text_font_map_get_resolution (VogueCoreTextFontMap *fontmap,
...
static int
get_scaled_size (VogueCoreTextFontMap       *fontmap,
                 VogueContext               *context,
                 const VogueFontDescription *desc)
{
  double size = vogue_font_description_get_size (desc);
  VogueMatrix *matrix = vogue_context_get_matrix (context);
  double scale_factor = vogue_matrix_get_font_scale_factor (matrix);
  
  if (!vogue_font_description_get_size_is_absolute(desc))
  {
    double dpi = vogue_core_text_font_map_get_resolution (fontmap, context);
    size *= (dpi/ppi);
  }
...
/*
 * VogueCoreTextFontsetKey
 */
struct _VogueCoreTextFontsetKey
{
...
static void
vogue_core_text_fontset_key_init (VogueCoreTextFontsetKey    *key,
                                  VogueCoreTextFontMap       *fontmap,
                                  VogueContext               *context,
                                  const VogueFontDescription *desc,
                                  VogueLanguage              *language)
{
  ...
  key->resolution = vogue_core_text_font_map_get_resolution (fontmap, context);
  key->gravity = vogue_context_get_gravity (context);
  key->desc = vogue_font_description_copy_static (desc);
  vogue_font_description_unset_fields (key->desc, PANGO_FONT_MASK_SIZE);
...
static VogueCoreTextFontsetKey *
vogue_core_text_fontset_key_copy (const VogueCoreTextFontsetKey *old)
{
  VogueCoreTextFontsetKey *key = g_slice_new (VogueCoreTextFontsetKey);
...
static void
vogue_core_text_fontset_key_free (VogueCoreTextFontsetKey *key)
{
  ...
  g_slice_free (VogueCoreTextFontsetKey, key);
}

static guint
vogue_core_text_fontset_key_hash (const VogueCoreTextFontsetKey *key)
{
...
static gboolean
vogue_core_text_fontset_key_equal (const VogueCoreTextFontsetKey *key_a,
                                   const VogueCoreTextFontsetKey *key_b)
{
...
vogue_core_text_fontset_key_get_language (const VogueCoreTextFontsetKey *key)
{
  return key->language;
}

static const VogueMatrix *
vogue_core_text_fontset_key_get_matrix (const VogueCoreTextFontsetKey *key)
{
  return &key->matrix;
}

static VogueGravity
vogue_core_text_fontset_key_get_gravity (const VogueCoreTextFontsetKey *key)
{
  return key->gravity;
}

static gpointer
vogue_core_text_fontset_key_get_context_key (const VogueCoreTextFontsetKey *key)
{
  return key->context_key;
}

/*
 * VogueCoreTextFontKey
 */
struct _VogueCoreTextFontKey
{
  VogueCoreTextFontMap *fontmap;
  CTFontDescriptorRef ctfontdescriptor;
  VogueMatrix matrix;
...
static void
vogue_core_text_font_key_init (VogueCoreTextFontKey    *key,
                               VogueCoreTextFontMap    *ctfontmap,
                               VogueCoreTextFontsetKey *fontset_key,
                               CTFontDescriptorRef      ctdescriptor,
                               gboolean                 synthetic_italic)
{
  key->fontmap = ctfontmap;
  key->ctfontdescriptor = ctdescriptor;
  key->matrix = *vogue_core_text_fontset_key_get_matrix (fontset_key);
  key->pointsize = fontset_key->pointsize;
  key->resolution = fontset_key->resolution;
  key->synthetic_italic = synthetic_italic;
  key->gravity = vogue_core_text_fontset_key_get_gravity (fontset_key);
  key->context_key = vogue_core_text_fontset_key_get_context_key (fontset_key);
}

static VogueCoreTextFontKey *
vogue_core_text_font_key_copy (const VogueCoreTextFontKey *old)
{
  VogueCoreTextFontKey *key = g_slice_new (VogueCoreTextFontKey);
...
static void
vogue_core_text_font_key_free (VogueCoreTextFontKey *key)
{
  ...
  g_slice_free (VogueCoreTextFontKey, key);
}

static guint
vogue_core_text_font_key_hash (const VogueCoreTextFontKey *key)
{
  guint32 hash = FNV1_32_INIT;
...
static gboolean
vogue_core_text_font_key_equal (const VogueCoreTextFontKey *key_a,
                                const VogueCoreTextFontKey *key_b)
{
...
int
vogue_core_text_font_key_get_size (const VogueCoreTextFontKey *key)
{
  return key->pointsize;
}

double
vogue_core_text_font_key_get_resolution (const VogueCoreTextFontKey *key)
{
  return key->resolution;
}

gboolean
vogue_core_text_font_key_get_synthetic_italic (const VogueCoreTextFontKey *key)
{
  return key->synthetic_italic;
}

gpointer
vogue_core_text_font_key_get_context_key (const VogueCoreTextFontKey *key)
{
  return key->context_key;
}

const VogueMatrix *
vogue_core_text_font_key_get_matrix (const VogueCoreTextFontKey *key)
{
  return &key->matrix;
}

VogueGravity
vogue_core_text_font_key_get_gravity (const VogueCoreTextFontKey *key)
{
  return key->gravity;
}

CTFontDescriptorRef
vogue_core_text_font_key_get_ctfontdescriptor (const VogueCoreTextFontKey *key)
{
...
static void
vogue_core_text_font_map_add (VogueCoreTextFontMap *ctfontmap,
                              VogueCoreTextFontKey *key,
                              VogueCoreTextFont    *ctfont)
{
  VogueCoreTextFontKey *key_copy;

  _vogue_core_text_font_set_font_map (ctfont, ctfontmap);

  key_copy = vogue_core_text_font_key_copy (key);
  _vogue_core_text_font_set_font_key (ctfont, key_copy);
  g_hash_table_insert (ctfontmap->font_hash, key_copy, ctfont);
}

static VogueCoreTextFont *
vogue_core_text_font_map_new_font (VogueCoreTextFontMap    *fontmap,
                                   VogueCoreTextFontsetKey *fontset_key,
                                   CTFontDescriptorRef      ctfontdescriptor,
                                   gboolean                 synthetic_italic)
{
  VogueCoreTextFontMapClass *klass;
  VogueCoreTextFont *font;
  VogueCoreTextFontKey key;

  vogue_core_text_font_key_init (&key, fontmap, fontset_key, ctfontdescriptor,
                                 synthetic_italic);
  ...
  vogue_core_text_font_map_add (fontmap, &key, font);

  return font;
}

static gboolean
find_best_match (VogueCoreTextFamily         *font_family,
                 const VogueFontDescription  *description,
                 VogueCoreTextFace          **best_face)
{
  VogueFontDescription *new_desc;
  VogueFontDescription *best_description = NULL;
  int i;
  ...
  *best_face = NULL;
  ...
  for (i = 0; i < font_family->n_faces; i++)
    {
      new_desc = vogue_font_face_describe (font_family->faces[i]);

      if (vogue_font_description_better_match (description, best_description,
                                               new_desc))
	{
	  vogue_font_description_free (best_description);
	  best_description = new_desc;
	  *best_face = (VogueCoreTextFace *)font_family->faces[i];
	}
      else
	vogue_font_description_free (new_desc);
    }
...
static guint
vogue_core_text_font_map_get_serial (VogueFontMap *fontmap)
{
  VogueCoreTextFontMap *ctfontmap = PANGO_CORE_TEXT_FONT_MAP (fontmap);

  return ctfontmap->serial;
}

static void
vogue_core_text_font_map_changed (VogueFontMap *fontmap)
{
  VogueCoreTextFontMap *ctfontmap = PANGO_CORE_TEXT_FONT_MAP (fontmap);
...
static VogueFont *
vogue_core_text_font_map_load_font (VogueFontMap               *fontmap,
                                    VogueContext               *context,
                                    const VogueFontDescription *description)
{
  VogueLanguage *language;
  ...
  fontset = vogue_font_map_load_fontset (fontmap, context,
...
static void
vogue_core_text_font_map_list_families (VogueFontMap      *fontmap,
                                        VogueFontFamily ***families,
                                        int               *n_families)
{
  GSList *family_list = NULL;
  GSList *tmp_list;
  VogueCoreTextFontMap *ctfontmap = (VogueCoreTextFontMap *)fontmap;
  ...
  if (families)
    {
      ...
      *families = g_new (VogueFontFamily *, *n_families);
...
static VogueFontset *
vogue_core_text_font_map_load_fontset (VogueFontMap               *fontmap,
                                       VogueContext               *context,
                                       const VogueFontDescription *desc,
                                       VogueLanguage              *language)
{
  VogueCoreTextFontset *fontset;
  VogueCoreTextFontsetKey key;
  VogueCoreTextFontMap *ctfontmap = PANGO_CORE_TEXT_FONT_MAP (fontmap);
  static gboolean warned_full_fallback = FALSE; /* MT-safe */

  vogue_core_text_fontset_key_init (&key, ctfontmap,
  ...
  if (G_UNLIKELY (!fontset))
    {
      ...
      fontset = vogue_core_text_fontset_new (&key, desc);
      ...
      if (G_UNLIKELY (!fontset))
        {
           ...
           */
          VogueFontDescription *tmp_desc;

          /* Cannot use vogue_core_text_fontset_key_free() here */
          vogue_font_description_free (key.desc);

          tmp_desc = vogue_font_description_copy_static (desc);
          if (strcmp (vogue_font_description_get_family (tmp_desc), "emoji") == 0)
            vogue_font_description_set_family_static (tmp_desc, "Apple Color Emoji");
          else
            vogue_font_description_set_family_static (tmp_desc, "Sans");

          vogue_core_text_fontset_key_init (&key, ctfontmap, context, tmp_desc,
                                            language);
              ...
              if (!warned_full_fallback)
                {
                  ...
                  ctmp = vogue_font_description_to_string (desc);
      ...
      if (insert_in_hash)
        g_hash_table_insert (ctfontmap->fontset_hash,
                             vogue_core_text_fontset_get_key (fontset),
                             fontset);
    }
...
static void
vogue_core_text_font_map_init (VogueCoreTextFontMap *ctfontmap)
{
  VogueCoreTextFamily *family;
  CTFontCollectionRef collection;
  CFArrayRef ctfaces;
  ...
  ctfontmap->font_hash = g_hash_table_new_full ((GHashFunc)vogue_core_text_font_key_hash,
                                                (GEqualFunc)vogue_core_text_font_key_equal,
                                                (GDestroyNotify)vogue_core_text_font_key_free,
                                                NULL);

  ctfontmap->fontset_hash = g_hash_table_new_full ((GHashFunc)vogue_core_text_fontset_key_hash,
                                                   (GEqualFunc)vogue_core_text_fontset_key_equal,
                                                   NULL,
...
static void
vogue_core_text_font_map_class_init (VogueCoreTextFontMapClass *class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);
  VogueFontMapClass *fontmap_class = PANGO_FONT_MAP_CLASS (class);

  object_class->finalize = vogue_core_text_font_map_finalize;

  fontmap_class->load_font = vogue_core_text_font_map_load_font;
  fontmap_class->list_families = vogue_core_text_font_map_list_families;
  fontmap_class->load_fontset = vogue_core_text_font_map_load_fontset;
  fontmap_class->shape_engine_type = PANGO_RENDER_TYPE_CORE_TEXT;
  fontmap_class->get_serial = vogue_core_text_font_map_get_serial;
  fontmap_class->changed = vogue_core_text_font_map_changed;
}

/*
 * VogueCoreTextFontSet
 */

static void              vogue_core_text_fontset_finalize     (GObject                   *object);
static void              vogue_core_text_fontset_init         (VogueCoreTextFontset      *fontset);
static VogueLanguage *   vogue_core_text_fontset_get_language (VogueFontset              *fontset);
static  VogueFont *      vogue_core_text_fontset_get_font     (VogueFontset              *fontset,
                                                               guint                      wc);
static void              vogue_core_text_fontset_foreach      (VogueFontset              *fontset,
                                                               VogueFontsetForeachFunc    func,
                                                               gpointer                   data);

struct _VogueCoreTextFontset
{
  VogueFontset parent_instance;

  const gchar *orig_family;
  VogueFontDescription *orig_description;
...
struct _VogueCoreTextFontsetClass
{
  VogueFontsetClass parent_instance;
};

typedef struct _VogueCoreTextFontsetClass VogueCoreTextFontsetClass;

G_DEFINE_TYPE (VogueCoreTextFontset,
               vogue_core_text_fontset,
               PANGO_TYPE_FONTSET);
...
static VogueCoreTextFontset *
vogue_core_text_fontset_new (VogueCoreTextFontsetKey    *key,
                             const VogueFontDescription *description)
{
  VogueCoreTextFamily *font_family;
  VogueCoreTextFontset *fontset;
  VogueCoreTextFont *best_font = NULL;
  gchar **family_names;
  const gchar *family;
  gchar *name;
  int i;
  ...
  family = vogue_font_description_get_family (description);
  ...
  for (i = 0; family_names[i]; ++i)
    {
      ...
      if (font_family)
        {
          VogueCoreTextFace *family_face;
          VogueCoreTextFont *font;

          /* Force a listing of the available faces */
          vogue_font_family_list_faces ((VogueFontFamily *)font_family, NULL, NULL);

          if (find_best_match (font_family, description, &family_face))
            {
              font = vogue_core_text_font_map_new_font (key->fontmap,
                                                        key,
  ...
  fontset->key = vogue_core_text_fontset_key_copy (key);
  fontset->orig_description = vogue_font_description_copy (description);
  ...
  /* Add the cascade list for this language */
#if defined(MAC_OS_X_VERSION_10_8) && MAC_OS_X_VERSION_MIN_REQUIRED >= MAC_OS_X_VERSION_10_8
    {
      ...
      if (key->language)
        {
          languages[0] = CFStringCreateWithCString (NULL,
                                                    vogue_language_to_string (key->language),
                                                    kCFStringEncodingASCII);
      ...
      fontset->cascade_list = CTFontCopyDefaultCascadeListForLanguages (vogue_core_text_font_get_ctfont (best_font), language_pref_list);
   ...
   */
  fontset->cascade_list = CTFontCopyDefaultCascadeList (vogue_core_text_font_get_ctfont (best_font));
#endif
...
static VogueFont *
vogue_core_text_fontset_load_font (VogueCoreTextFontset *ctfontset,
                                   CTFontDescriptorRef   ctdescriptor)
{
  VogueCoreTextFontsetKey *key;
  VogueCoreTextFont *font;

  key = vogue_core_text_fontset_get_key (ctfontset);

  /* For now, we will default the fallbacks to not have synthetic italic,
   * in the future this may be improved.
   */
  font = vogue_core_text_font_map_new_font (ctfontset->key->fontmap,
                                            ctfontset->key,
                                            ctdescriptor,
                                            FALSE);
...
static VogueFont *
vogue_core_text_fontset_get_font_at (VogueCoreTextFontset *ctfontset,
                                     unsigned int          i)
{
  ...
  if (g_ptr_array_index (ctfontset->fonts, i) == NULL)
    {
      ...
      VogueFont *font = vogue_core_text_fontset_load_font (ctfontset, ctdescriptor);
...
static void
vogue_core_text_fontset_class_init (VogueCoreTextFontsetClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  VogueFontsetClass *fontset_class = PANGO_FONTSET_CLASS (klass);

  object_class->finalize = vogue_core_text_fontset_finalize;

  fontset_class->get_font = vogue_core_text_fontset_get_font;
  fontset_class->get_language = vogue_core_text_fontset_get_language;
  fontset_class->foreach = vogue_core_text_fontset_foreach;
}

static void
vogue_core_text_fontset_init (VogueCoreTextFontset *ctfontset)
{
  ctfontset->key = NULL;
...
static void
vogue_core_text_fontset_finalize (GObject *object)
{
  VogueCoreTextFontset *ctfontset = PANGO_CORE_TEXT_FONTSET (object);
  unsigned int i;

  for (i = 0; i < ctfontset->fonts->len; i++)
    {
      VogueFont *font = g_ptr_array_index (ctfontset->fonts, i);
      if (font)
        g_object_unref (font);
    }
  ...
  for (i = 0; i < ctfontset->coverages->len; i++)
    {
      VogueCoverage *coverage = g_ptr_array_index (ctfontset->coverages, i);
      if (coverage)
  ...
  vogue_font_description_free (ctfontset->orig_description);

  if (ctfontset->key)
    vogue_core_text_fontset_key_free (ctfontset->key);

  G_OBJECT_CLASS (vogue_core_text_fontset_parent_class)->finalize (object);
}

static VogueCoreTextFontsetKey *
vogue_core_text_fontset_get_key (VogueCoreTextFontset *fontset)
{
  return fontset->key;
}

static VogueLanguage *
vogue_core_text_fontset_get_language (VogueFontset *fontset)
{
  VogueCoreTextFontset *ctfontset = PANGO_CORE_TEXT_FONTSET (fontset);

  return vogue_core_text_fontset_key_get_language (vogue_core_text_fontset_get_key (ctfontset));
}

static VogueFont *
vogue_core_text_fontset_get_font (VogueFontset *fontset,
                                  guint         wc)
{
  VogueCoreTextFontset *ctfontset = PANGO_CORE_TEXT_FONTSET (fontset);
  ...
  for (i = 0; i < ctfontset->fonts->len; i++)
    {
      VogueFont *font = vogue_core_text_fontset_get_font_at (ctfontset, i);
      if (!font)
        continue;
      ...
      if (coverage == NULL)
        {
          ...
          coverage = vogue_font_get_coverage (font, ctfontset->key->language);
...
static void
vogue_core_text_fontset_foreach (VogueFontset *fontset,
                                 VogueFontsetForeachFunc func,
                                 gpointer data)
{
  VogueCoreTextFontset *ctfontset = PANGO_CORE_TEXT_FONTSET (fontset);

/* Vogue
 * voguecairo-render.c: Rendering routines to Cairo surfaces
 *
 ...
 * windowing systems. The functions in this section allow using Vogue
 * to render to Cairo surfaces.
 *
 * Using Vogue with Cairo is straightforward. A #VogueContext created
 * with vogue_cairo_font_map_create_context() can be used on any
 ...
 * using vogue_cairo_update_context(). The convenience functions
 * vogue_cairo_create_layout() and vogue_cairo_update_layout() handle
 * the common case where the program doesn't need to manipulate the
 * properties of the #VogueContext.
 *
 * When you get the metrics of a layout or of a piece of a layout using
 * functions such as vogue_layout_get_extents(), the reported metrics
 ...
 * functions in Vogue report results in integer Vogue units. To get
 * to the floating point units used in Cairo divide by %PANGO_SCALE.
 *
 * ## Using Vogue with Cairo ## {#rotated-example}
 *
 * |[<!-- language="C" -->
 * #include <math.h>
 ...
 *
 *   /&ast; Create a VogueLayout, set the font and text &ast;/
 ...
 *
 *       /&ast; Inform Vogue to re-layout the text with the new transformation &ast;/
...
typedef struct _VogueCairoRendererClass VogueCairoRendererClass;

#define PANGO_CAIRO_RENDERER_CLASS(klass)    (G_TYPE_CHECK_CLASS_CAST ((klass), PANGO_TYPE_CAIRO_RENDERER, VogueCairoRendererClass))
#define PANGO_IS_CAIRO_RENDERER_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((klass), PANGO_TYPE_CAIRO_RENDERER))
#define PANGO_CAIRO_RENDERER_GET_CLASS(obj)  (G_TYPE_INSTANCE_GET_CLASS ((obj), PANGO_TYPE_CAIRO_RENDERER, VogueCairoRendererClass))

struct _VogueCairoRenderer
{
...
struct _VogueCairoRendererClass
{
  VogueRendererClass parent_class;
};

G_DEFINE_TYPE (VogueCairoRenderer, vogue_cairo_renderer, PANGO_TYPE_RENDERER)

static void
set_color (VogueCairoRenderer *crenderer,
	   VogueRenderPart     part)
{
  VogueColor *color = vogue_renderer_get_color ((VogueRenderer *) (crenderer), part);
  guint16 a = vogue_renderer_get_alpha ((VogueRenderer *) (crenderer), part);
...
static void
_vogue_cairo_renderer_draw_frame (VogueCairoRenderer *crenderer,
				  double              x,
				  double              y,
				  double              width,
				  double              height,
...
static void
_vogue_cairo_renderer_draw_box_glyph (VogueCairoRenderer *crenderer,
				      VogueGlyphInfo     *gi,
				      double              cx,
				      double              cy,
				      gboolean            invalid)
{
  ...
  _vogue_cairo_renderer_draw_frame (crenderer,
				    cx + 1.5,
...
static void
_vogue_cairo_renderer_draw_unknown_glyph (VogueCairoRenderer *crenderer,
					  VogueFont          *font,
					  VogueGlyphInfo     *gi,
					  double              cx,
					  double              cy)
{
  ...
  hbi = _vogue_cairo_font_get_hex_box_info ((VogueCairoFont *)font);
  if (!hbi || !_vogue_cairo_font_install ((VogueFont *)(hbi->font), crenderer->cr))
    {
      _vogue_cairo_renderer_draw_box_glyph (crenderer, gi, cx, cy, invalid_input);
      goto done;
    }
  ...
  else if ((name = vogue_get_ignorable_size (ch, &rows, &cols)))
    {
...
static void
vogue_cairo_renderer_show_text_glyphs (VogueRenderer        *renderer,
				       const char           *text,
				       int                   text_len,
				       ...
				       int                   y)
{
  VogueCairoRenderer *crenderer = (VogueCairoRenderer *) (renderer);
  ...
  if (!_vogue_cairo_font_install (font, crenderer->cr))
    {
	  ...
	  if (gi->glyph != PANGO_GLYPH_EMPTY)
	    {
	      ...
	      _vogue_cairo_renderer_draw_unknown_glyph (crenderer, font, gi, cx, cy);
	    }
...
static void
vogue_cairo_renderer_draw_glyphs (VogueRenderer     *renderer,
				  VogueFont         *font,
				  VogueGlyphString  *glyphs,
				  int                x,
				  int                y)
{
  vogue_cairo_renderer_show_text_glyphs (renderer,
					 NULL, 0,
					 glyphs,
					 NULL, 0,
					 FALSE,
...
static void
vogue_cairo_renderer_draw_glyph_item (VogueRenderer     *renderer,
				      const char        *text,
...
{
  VogueCairoRenderer *crenderer = (VogueCairoRenderer *) (renderer);
  VogueFont          *font      = glyph_item->item->analysis.font;
  VogueGlyphString   *glyphs    = glyph_item->glyphs;
  VogueItem          *item      = glyph_item->item;
  ...
  num_clusters = 0;
  if (vogue_glyph_item_iter_init_start (&iter, glyph_item, text))
    {
      do {
	...
	if (num_bytes < 1)
	  g_warning ("vogue_cairo_renderer_draw_glyph_item: bad cluster has num_bytes %d", num_bytes);
	if (num_glyphs < 1)
	  g_warning ("vogue_cairo_renderer_draw_glyph_item: bad cluster has num_glyphs %d", num_glyphs);
	...
	for (i = MIN (iter.start_glyph, iter.end_glyph+1);
	     ...
	     i++)
	  {
        ...
        num_clusters++;
      } while (vogue_glyph_item_iter_next_cluster (&iter));
    }
...
static void
vogue_cairo_renderer_draw_rectangle (VogueRenderer     *renderer,
				     VogueRenderPart    part,
				     int                x,
				     int                y,
				     int                width,
...
static void
vogue_cairo_renderer_draw_trapezoid (VogueRenderer     *renderer,
				     VogueRenderPart    part,
				     double             y1_,
				     double             x11,
				     double             x21,
...
static void
vogue_cairo_renderer_draw_error_underline (VogueRenderer *renderer,
					   int            x,
					   int            y,
					   int            width,
					   int            height)
...
static void
vogue_cairo_renderer_draw_shape (VogueRenderer  *renderer,
				 VogueAttrShape *attr,
				 int             x,
				 int             y)
{
  ...
  cairo_t *cr = crenderer->cr;
  VogueLayout *layout;
  VogueCairoShapeRendererFunc shape_renderer;
  gpointer                    shape_renderer_data;
  double base_x, base_y;

  layout = vogue_renderer_get_layout (renderer);

  if (!layout)
  	return;

  shape_renderer = vogue_cairo_context_get_shape_renderer (vogue_layout_get_context (layout),
							   &shape_renderer_data);
...
static void
vogue_cairo_renderer_init (VogueCairoRenderer *renderer G_GNUC_UNUSED)
{
}

static void
vogue_cairo_renderer_class_init (VogueCairoRendererClass *klass)
{
  VogueRendererClass *renderer_class = PANGO_RENDERER_CLASS (klass);

  renderer_class->draw_glyphs = vogue_cairo_renderer_draw_glyphs;
  renderer_class->draw_glyph_item = vogue_cairo_renderer_draw_glyph_item;
  renderer_class->draw_rectangle = vogue_cairo_renderer_draw_rectangle;
  renderer_class->draw_trapezoid = vogue_cairo_renderer_draw_trapezoid;
  renderer_class->draw_error_underline = vogue_cairo_renderer_draw_error_underline;
  renderer_class->draw_shape = vogue_cairo_renderer_draw_shape;
}

static VogueCairoRenderer *cached_renderer = NULL; /* MT-safe */
G_LOCK_DEFINE_STATIC (cached_renderer);

static VogueCairoRenderer *
acquire_renderer (void)
{
  VogueCairoRenderer *renderer;
...
static void
release_renderer (VogueCairoRenderer *renderer)
{
...
static void
save_current_point (VogueCairoRenderer *renderer)
{
...
static void
restore_current_point (VogueCairoRenderer *renderer)
{
...
static void
_vogue_cairo_do_glyph_string (cairo_t          *cr,
			      VogueFont        *font,
			      VogueGlyphString *glyphs,
			      gboolean          do_path)
{
  VogueCairoRenderer *crenderer = acquire_renderer ();
  VogueRenderer *renderer = (VogueRenderer *) crenderer;
  ...
  if (!do_path)
    {
      ...
      vogue_renderer_set_color (renderer, PANGO_RENDER_PART_FOREGROUND, NULL);
      vogue_renderer_set_color (renderer, PANGO_RENDER_PART_BACKGROUND, NULL);
      vogue_renderer_set_color (renderer, PANGO_RENDER_PART_UNDERLINE, NULL);
      vogue_renderer_set_color (renderer, PANGO_RENDER_PART_STRIKETHROUGH, NULL);
    }

  vogue_renderer_draw_glyphs (renderer, font, glyphs, 0, 0);
...
static void
_vogue_cairo_do_glyph_item (cairo_t          *cr,
			    const char       *text,
			    VogueGlyphItem   *glyph_item,
			    gboolean          do_path)
{
  ...
  vogue_renderer_draw_glyph_item (renderer, text, glyph_item, 0, 0);
...
static void
_vogue_cairo_do_layout_line (cairo_t          *cr,
			     ...
			     gboolean          do_path)
{
  ...
  vogue_renderer_draw_layout_line (renderer, line, 0, 0);
...
static void
_vogue_cairo_do_layout (cairo_t     *cr,
			VogueLayout *layout,
			gboolean     do_path)
{
  VogueCairoRenderer *crenderer = acquire_renderer ();
  ...
  vogue_renderer_draw_layout (renderer, layout, 0, 0);
...
static void
_vogue_cairo_do_error_underline (cairo_t *cr,
				 double   x,
				 double   y,
				 double   width,
				 double   height,
 ...
 **/
void
vogue_cairo_show_glyph_string (cairo_t          *cr,
			       VogueFont        *font,
			       VogueGlyphString *glyphs)
{
  ...
  _vogue_cairo_do_glyph_string (cr, font, glyphs, FALSE);
}


/**
 * vogue_cairo_show_glyph_item:
 * @cr: a Cairo context
 ...
 * similar to vogue_cairo_show_glyph_string().
 *
 * The origin of the glyphs (the left edge of the baseline) will
 * be drawn at the current point of the cairo context.
 *
 ...
 **/
void
vogue_cairo_show_glyph_item (cairo_t          *cr,
			     const char       *text,
			     VogueGlyphItem   *glyph_item)
{
  g_return_if_fail (cr != NULL);
  ...
  _vogue_cairo_do_glyph_item (cr, text, glyph_item, FALSE);
}

/**
 * vogue_cairo_show_layout_line:
 * @cr: a Cairo context
 * @line: a #VogueLayoutLine
 *
 * Draws a #VogueLayoutLine in the specified cairo context.
 ...
 **/
void
vogue_cairo_show_layout_line (cairo_t          *cr,
			      VogueLayoutLine  *line)
{
  g_return_if_fail (cr != NULL);
  g_return_if_fail (line != NULL);

  _vogue_cairo_do_layout_line (cr, line, FALSE);
}

/**
 * vogue_cairo_show_layout:
 * @cr: a Cairo context
 * @layout: a Vogue layout
 *
 * Draws a #VogueLayout in the specified cairo context.
 * The top-left corner of the #VogueLayout will be drawn
 * at the current point of the cairo context.
 *
 * Since: 1.10
 **/
void
vogue_cairo_show_layout (cairo_t     *cr,
			 VogueLayout *layout)
{
  g_return_if_fail (cr != NULL);
  g_return_if_fail (PANGO_IS_LAYOUT (layout));

  _vogue_cairo_do_layout (cr, layout, FALSE);
}

/**
 * vogue_cairo_show_error_underline:
 * @cr: a Cairo context
 ...
 **/
void
vogue_cairo_show_error_underline (cairo_t *cr,
				  double  x,
				  double  y,
				  double  width,
				  double  height)
{
  ...
  _vogue_cairo_do_error_underline (cr, x, y, width, height, FALSE);
}

/**
 * vogue_cairo_glyph_string_path:
 * @cr: a Cairo context
 * @font: a #VogueFont from a #VogueCairoFontMap
 * @glyphs: a #VogueGlyphString
 *
 ...
 **/
void
vogue_cairo_glyph_string_path (cairo_t          *cr,
			       VogueFont        *font,
			       VogueGlyphString *glyphs)
{
  g_return_if_fail (cr != NULL);
  ...
  _vogue_cairo_do_glyph_string (cr, font, glyphs, TRUE);
}

/**
 * vogue_cairo_layout_line_path:
 * @cr: a Cairo context
 * @line: a #VogueLayoutLine
 *
 * Adds the text in #VogueLayoutLine to the current path in the
 ...
 **/
void
vogue_cairo_layout_line_path (cairo_t          *cr,
			      VogueLayoutLine  *line)
{
  g_return_if_fail (cr != NULL);
  g_return_if_fail (line != NULL);

  _vogue_cairo_do_layout_line (cr, line, TRUE);
}

/**
 * vogue_cairo_layout_path:
 * @cr: a Cairo context
 * @layout: a Vogue layout
 *
 * Adds the text in a #VogueLayout to the current path in the
 * specified cairo context.  The top-left corner of the #VogueLayout
 * will be at the current point of the cairo context.
 *
 * Since: 1.10
 **/
void
vogue_cairo_layout_path (cairo_t     *cr,
			 VogueLayout *layout)
{
  g_return_if_fail (cr != NULL);
  g_return_if_fail (PANGO_IS_LAYOUT (layout));

  _vogue_cairo_do_layout (cr, layout, TRUE);
}

/**
 * vogue_cairo_error_underline_path:
 * @cr: a Cairo context
 * @x: The X coordinate of one corner of the rectangle
 * @y: The Y coordinate of one corner of the rectangle
 * @width: Non-negative width of the rectangle
 ...
 **/
void
vogue_cairo_error_underline_path (cairo_t *cr,
				  double   x,
				  double   y,
				  double   width,
				  double   height)
{
  ...
  _vogue_cairo_do_error_underline (cr, x, y, width, height, TRUE);
}

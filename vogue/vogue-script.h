/* Vogue
 * vogue-script.h: Script tag handling
 *
 ...
 * VogueScriptIter:
 *
 * A #VogueScriptIter is used to iterate through a string
 * and identify ranges in different scripts.
 **/
typedef struct _VogueScriptIter VogueScriptIter;

/**
 * VogueScript:
 * @PANGO_SCRIPT_INVALID_CODE: a value never returned from vogue_script_for_unichar()
 ...
 *
 * The #VogueScript enumeration identifies different writing
 ...
 * whose values are interchangeable with VogueScript.
 */
...
VogueScript vogue_script_for_unichar         (gunichar             ch) G_GNUC_CONST;

PANGO_AVAILABLE_IN_1_44
GType            vogue_script_iter_get_type  (void) G_GNUC_CONST;

PANGO_AVAILABLE_IN_1_4
VogueScriptIter *vogue_script_iter_new       (const char          *text,
					      int                  length);
PANGO_AVAILABLE_IN_1_4
void             vogue_script_iter_get_range (VogueScriptIter     *iter,
                                              const char         **start,
                                              const char         **end,
                                              VogueScript         *script);
PANGO_AVAILABLE_IN_1_4
gboolean         vogue_script_iter_next      (VogueScriptIter     *iter);
PANGO_AVAILABLE_IN_1_4
void             vogue_script_iter_free      (VogueScriptIter     *iter);

#include <vogue/vogue-language.h>

PANGO_AVAILABLE_IN_1_4
VogueLanguage *vogue_script_get_sample_language (VogueScript    script) G_GNUC_PURE;

...
G_DEFINE_BOXED_TYPE (VogueColor, vogue_color,
                     vogue_color_copy,
                     vogue_color_free);
 ...
 * vogue_color_free(). Primarily used by language bindings,
 * not that useful otherwise (since colors can just be copied
 * by assignment in C).
 *
 * Return value: (nullable): the newly allocated #VogueColor, which
 *               should be freed with vogue_color_free(), or %NULL if
 *               @src was %NULL.
 **/
VogueColor*
vogue_color_copy (const VogueColor *src)
{
  VogueColor *ret;
...
/**
 * vogue_color_free:
 * @color: (nullable): an allocated #VogueColor, may be %NULL
 *
 * Frees a color allocated by vogue_color_copy().
 **/
void
vogue_color_free (VogueColor *color)
{
  if (color == NULL)
    return;

  g_slice_free (VogueColor, color);
}

/**
 * vogue_color_to_string:
 * @color: a #VogueColor
 *
 ...
 **/
gchar *
vogue_color_to_string (const VogueColor *color)
{
...
/* Like vogue_color_parse, but allow strings of the form
 ...
 */
gboolean
_vogue_color_parse_with_alpha (VogueColor *color,
                               guint16    *alpha,
		               const char *spec)
{
...
/**
 * vogue_color_parse:
 * @color: (nullable): a #VogueColor structure in which to store the
 *   result, or %NULL
 ...
 **/
gboolean
vogue_color_parse (VogueColor *color,
		   const char *spec)
{
  return _vogue_color_parse_with_alpha (color, NULL, spec);
}

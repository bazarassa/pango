/* Vogue
 * vogue-layout-private.h: Internal structures of VogueLayout
 *
...
struct _VogueLayout
{
  ...
  /* If you add fields to VogueLayout be sure to update _copy()
  ...
  guint auto_dir : 1;
  guint wrap : 2;		/* VogueWrapMode */
  guint is_wrapped : 1;		/* Whether the layout has any wrapped lines */
  guint ellipsize : 2;		/* VogueEllipsizeMode */
  ...
  VogueLogAttr *log_attrs;	/* Logical attributes for layout's text */
  GSList *lines;
...
struct _VogueLayoutIter
{
  ...
  VogueLayoutRun *run; /* FIXME nuke this, just keep the link */
  int index;
...
gboolean _vogue_layout_line_ellipsize (VogueLayoutLine *line,
				       VogueAttrList   *attrs,
				       int              goal_width);

void     _vogue_layout_get_iter (VogueLayout     *layout,
                                 VogueLayoutIter *iter);

void     _vogue_layout_iter_destroy (VogueLayoutIter *iter);

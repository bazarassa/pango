/* Vogue
 * vogue-layout.c: High-level layout driver
 *
 ...
 *
 * While complete access to the layout capabilities of Vogue is provided
 ...
 * VogueLayout:
 *
 * The #VogueLayout structure represents an entire paragraph
 * of text. It is initialized with a #VogueContext, UTF-8 string
 ...
 * of a #VogueLayout, which are illustrated in <xref linkend="parameters"/>.
 * It is possible, as well, to ignore the 2-D setup, and simply
 * treat the results of a #VogueLayout as a list of lines.
 *
 * <figure id="parameters">
 * <title>Adjustable parameters (on the left) and font metrics (on the right) for a VogueLayout</title>
 * <graphic fileref="layout.png" format="PNG"></graphic>
 * </figure>
 *
 * The #VogueLayout structure is opaque, and has no user-visible
 * fields.
 */
 ...
 * VogueLayoutIter:
 *
 * A #VogueLayoutIter structure can be used to
 * iterate over the visual extents of a #VogueLayout.
 *
 * The #VogueLayoutIter structure is opaque, and
 * has no user-visible fields.
 */

#include "config.h"
#include "vogue-glyph.h"		/* For vogue_shape() */
#include "vogue-break.h"
#include "vogue-item.h"
#include "vogue-engine.h"
#include "vogue-impl-utils.h"
#include "vogue-glyph-item.h"
#include <string.h>
...
struct _ItemProperties
{
  ...
  gint            letter_spacing;
  gboolean        shape_set;
  VogueRectangle *shape_ink_rect;
  VogueRectangle *shape_logical_rect;
};

typedef struct _VogueLayoutLinePrivate VogueLayoutLinePrivate;

struct _VogueLayoutLinePrivate
{
...
static gboolean
check_invalid (VogueLayoutIter *iter,
	       const char      *loc)
{
  if (iter->line->layout == NULL)
    {
      g_warning ("%s: VogueLayout changed since VogueLayoutIter was created, iterator invalid", loc);
      return TRUE;
    }
  else
    {
...
static void check_context_changed  (VogueLayout *layout);
static void layout_changed  (VogueLayout *layout);

static void vogue_layout_clear_lines (VogueLayout *layout);
static void vogue_layout_check_lines (VogueLayout *layout);

static VogueAttrList *vogue_layout_get_effective_attributes (VogueLayout *layout);

static VogueLayoutLine * vogue_layout_line_new         (VogueLayout     *layout);
static void              vogue_layout_line_postprocess (VogueLayoutLine *line,
							ParaBreakState  *state,
							gboolean         wrapped);

static int *vogue_layout_line_get_log2vis_map (VogueLayoutLine  *line,
					       gboolean          strong);
static int *vogue_layout_line_get_vis2log_map (VogueLayoutLine  *line,
					       gboolean          strong);
static void vogue_layout_line_leaked (VogueLayoutLine *line);

/* doesn't leak line */
static VogueLayoutLine* _vogue_layout_iter_get_line (VogueLayoutIter *iter);

static void vogue_layout_get_item_properties (VogueItem      *item,
					      ItemProperties *properties);

static void vogue_layout_get_empty_extents_at_index (VogueLayout    *layout,
						     int             index,
						     VogueRectangle *logical_rect);

static void vogue_layout_finalize    (GObject          *object);

G_DEFINE_TYPE (VogueLayout, vogue_layout, G_TYPE_OBJECT)

static void
vogue_layout_init (VogueLayout *layout)
{
  layout->serial = 1;
  layout->attrs = NULL;
...
static void
vogue_layout_class_init (VogueLayoutClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = vogue_layout_finalize;
}

static void
vogue_layout_finalize (GObject *object)
{
  VogueLayout *layout;

  layout = PANGO_LAYOUT (object);

  vogue_layout_clear_lines (layout);

  if (layout->context)
    g_object_unref (layout->context);

  if (layout->attrs)
    vogue_attr_list_unref (layout->attrs);

  g_free (layout->text);

  if (layout->font_desc)
    vogue_font_description_free (layout->font_desc);

  if (layout->tabs)
    vogue_tab_array_free (layout->tabs);

  G_OBJECT_CLASS (vogue_layout_parent_class)->finalize (object);
}
 ...
 *
 * Create a new #VogueLayout object with attributes initialized to
 * default values for a particular #VogueContext.
 *
 * Return value: the newly allocated #VogueLayout, with a reference
 *               count of one, which should be freed with
 *               g_object_unref().
 **/
VogueLayout *
vogue_layout_new (VogueContext *context)
{
  VogueLayout *layout;
  ...
  layout->context_serial = vogue_context_get_serial (context);
 ...
 * value.
 *
 * Return value: (transfer full): the newly allocated #VogueLayout,
 *               with a reference count of one, which should be freed
 *               with g_object_unref().
 **/
VogueLayout*
vogue_layout_copy (VogueLayout *src)
{
  VogueLayout *layout;
  ...
  layout = vogue_layout_new (src->context);
  if (src->attrs)
    layout->attrs = vogue_attr_list_copy (src->attrs);
  if (src->font_desc)
    layout->font_desc = vogue_font_description_copy (src->font_desc);
  if (src->tabs)
    layout->tabs = vogue_tab_array_copy (src->tabs);
  ...
  memcpy (&layout->copy_begin, &src->copy_begin,
	  G_STRUCT_OFFSET (VogueLayout, copy_end) - G_STRUCT_OFFSET (VogueLayout, copy_begin));

  return layout;
}

/**
 * vogue_layout_get_context:
 * @layout: a #VogueLayout
 *
 * Retrieves the #VogueContext used for this layout.
 *
 * Return value: (transfer none): the #VogueContext for the layout.
 ...
 **/
VogueContext *
vogue_layout_get_context (VogueLayout *layout)
{
...
/**
 * vogue_layout_set_width:
 * @layout: a #VogueLayout.
 * @width: the desired width in Vogue units, or -1 to indicate that no
 *         wrapping or ellipsization should be performed.
 *
 * Sets the width to which the lines of the #VogueLayout should wrap or
 * ellipsized.  The default value is -1: no width set.
 **/
void
vogue_layout_set_width (VogueLayout *layout,
			int          width)
{
...
/**
 * vogue_layout_get_width:
 * @layout: a #VogueLayout
 *
 * Gets the width to which the lines of the #VogueLayout should wrap.
 *
 * Return value: the width in Vogue units, or -1 if no width set.
 **/
int
vogue_layout_get_width (VogueLayout    *layout)
{
...
/**
 * vogue_layout_set_height:
 * @layout: a #VogueLayout.
 * @height: the desired height of the layout in Vogue units if positive,
 *          or desired number of lines if negative.
 *
 * Sets the height to which the #VogueLayout should be ellipsized at.  There
 ...
 * paragraph.  File a bug against vogue at <ulink
 ...
 **/
void
vogue_layout_set_height (VogueLayout *layout,
			 int          height)
{
  g_return_if_fail (layout != NULL);
...
/**
 * vogue_layout_get_height:
 * @layout: a #VogueLayout
 *
 * Gets the height of layout used for ellipsization.  See
 * vogue_layout_set_height() for details.
 *
 * Return value: the height, in Vogue units if positive, or
 * number of lines if negative.
 *
 * Since: 1.20
 **/
int
vogue_layout_get_height (VogueLayout    *layout)
{
  g_return_val_if_fail (layout != NULL, 0);
...
/**
 * vogue_layout_set_wrap:
 * @layout: a #VogueLayout
 * @wrap: the wrap mode
 *
 * Sets the wrap mode; the wrap mode only has effect if a width
 * is set on the layout with vogue_layout_set_width().
 * To turn off wrapping, set the width to -1.
 **/
void
vogue_layout_set_wrap (VogueLayout  *layout,
		       VogueWrapMode wrap)
{
...
/**
 * vogue_layout_get_wrap:
 * @layout: a #VogueLayout
 *
 * Gets the wrap mode for the layout.
 *
 * Use vogue_layout_is_wrapped() to query whether any paragraphs
 * were actually wrapped.
 *
 * Return value: active wrap mode.
 **/
VogueWrapMode
vogue_layout_get_wrap (VogueLayout *layout)
{
 ...
 */
gboolean
vogue_layout_is_wrapped (VogueLayout *layout)
{
  g_return_val_if_fail (layout != NULL, FALSE);

  vogue_layout_check_lines (layout);

  return layout->is_wrapped;
}

/**
 * vogue_layout_set_indent:
 * @layout: a #VogueLayout.
 * @indent: the amount by which to indent.
 *
 * Sets the width in Vogue units to indent each paragraph. A negative value
 ...
 **/
void
vogue_layout_set_indent (VogueLayout *layout,
			 int          indent)
{
  g_return_if_fail (layout != NULL);
...
/**
 * vogue_layout_get_indent:
 * @layout: a #VogueLayout
 *
 * Gets the paragraph indent width in Vogue units. A negative value
 * indicates a hanging indentation.
 *
 * Return value: the indent in Vogue units.
 **/
int
vogue_layout_get_indent (VogueLayout *layout)
{
  g_return_val_if_fail (layout != NULL, 0);
...
/**
 * vogue_layout_set_spacing:
 * @layout: a #VogueLayout.
 * @spacing: the amount of spacing
 *
 * Sets the amount of spacing in Vogue unit between
 * the lines of the layout. When placing lines with
 * spacing, Vogue arranges things so that
 *
 * line2.top = line1.bottom + spacing
 *
 * Note: Since 1.44, Vogue defaults to using the
 ...
 * set to zero with vogue_layout_set_line_spacing().
 **/
void
vogue_layout_set_spacing (VogueLayout *layout,
			  int          spacing)
{
  g_return_if_fail (layout != NULL);
...
/**
 * vogue_layout_get_spacing:
 * @layout: a #VogueLayout
 *
 * Gets the amount of spacing between the lines of the layout.
 *
 * Return value: the spacing in Vogue units.
 **/
int
vogue_layout_get_spacing (VogueLayout *layout)
{
  g_return_val_if_fail (layout != NULL, 0);
 ...
 * vogue_layout_set_spacing() is ignored.
 *
 ...
 */
void
vogue_layout_set_line_spacing (VogueLayout *layout,
			       float        factor)
{
  g_return_if_fail (layout != NULL);
...
/**
 * vogue_layout_get_line_spacing:
 * @layout: a #VogueLayout
 *
 * Gets the value that has been
 * set with vogue_layout_set_line_spacing().
 *
 * Since: 1.44
 */
float
vogue_layout_get_line_spacing (VogueLayout *layout)
{
...
/**
 * vogue_layout_set_attributes:
 * @layout: a #VogueLayout
 * @attrs: (allow-none) (transfer none): a #VogueAttrList, can be %NULL
 *
 * Sets the text attributes for a layout object.
 * References @attrs, so the caller can unref its reference.
 **/
void
vogue_layout_set_attributes (VogueLayout   *layout,
			     VogueAttrList *attrs)
{
  VogueAttrList *old_attrs;
  g_return_if_fail (layout != NULL);
  ...
  layout->attrs = attrs;
  if (layout->attrs)
    vogue_attr_list_ref (layout->attrs);

  layout_changed (layout);

  if (old_attrs)
    vogue_attr_list_unref (old_attrs);
  layout->tab_width = -1;
}

/**
 * vogue_layout_get_attributes:
 * @layout: a #VogueLayout
 *
 * Gets the attribute list for the layout, if any.
 *
 * Return value: (transfer none): a #VogueAttrList.
 **/
VogueAttrList*
vogue_layout_get_attributes (VogueLayout *layout)
{
...
/**
 * vogue_layout_set_font_description:
 * @layout: a #VogueLayout
 * @desc: (allow-none): the new #VogueFontDescription, or %NULL to unset the
 ...
 **/
void
vogue_layout_set_font_description (VogueLayout                 *layout,
				    const VogueFontDescription *desc)
{
  g_return_if_fail (layout != NULL);

  if (desc != layout->font_desc &&
      (!desc || !layout->font_desc || !vogue_font_description_equal(desc, layout->font_desc)))
    {
      if (layout->font_desc)
	vogue_font_description_free (layout->font_desc);

      layout->font_desc = desc ? vogue_font_description_copy (desc) : NULL;
...
/**
 * vogue_layout_get_font_description:
 * @layout: a #VogueLayout
 *
...
vogue_layout_get_font_description (VogueLayout *layout)
{
  g_return_val_if_fail (PANGO_IS_LAYOUT (layout), NULL);
 ...
 * the characters.
 *
 * Note that this setting is not implemented and so is ignored in Vogue
 * older than 1.18.
 **/
void
vogue_layout_set_justify (VogueLayout *layout,
			  gboolean     justify)
{
  g_return_if_fail (layout != NULL);
 ...
 **/
gboolean
vogue_layout_get_justify (VogueLayout *layout)
{
  g_return_val_if_fail (layout != NULL, FALSE);
 ...
 * of the layout's #VogueContext. (See vogue_context_set_base_dir()).
 *
 ...
 **/
void
vogue_layout_set_auto_dir (VogueLayout *layout,
			   gboolean     auto_dir)
{
  g_return_if_fail (PANGO_IS_LAYOUT (layout));
...
/**
 * vogue_layout_get_auto_dir:
 * @layout: a #VogueLayout
 *
 * Gets whether to calculate the bidirectional base direction
 * for the layout according to the contents of the layout.
 * See vogue_layout_set_auto_dir().
 *
 ...
 **/
gboolean
vogue_layout_get_auto_dir (VogueLayout *layout)
{
 ...
 **/
void
vogue_layout_set_alignment (VogueLayout   *layout,
 ...
 **/
VogueAlignment
vogue_layout_get_alignment (VogueLayout *layout)
{
...
/**
 * vogue_layout_set_tabs:
 * @layout: a #VogueLayout
 * @tabs: (allow-none): a #VogueTabArray, or %NULL
 *
 ...
 **/
void
vogue_layout_set_tabs (VogueLayout   *layout,
		       VogueTabArray *tabs)
{
  g_return_if_fail (PANGO_IS_LAYOUT (layout));
  ...
  if (tabs != layout->tabs)
	...
	vogue_tab_array_free (layout->tabs);
      ...
      layout->tabs = tabs ? vogue_tab_array_copy (tabs) : NULL;
...
/**
 * vogue_layout_get_tabs:
 * @layout: a #VogueLayout
 *
 * Gets the current #VogueTabArray used by this layout. If no
 * #VogueTabArray has been set, then the default tabs are in use
 * and %NULL is returned. Default tabs are every 8 spaces.
 * The return value should be freed with vogue_tab_array_free().
 *
 * Return value: (nullable): a copy of the tabs for this layout, or
 * %NULL.
 **/
VogueTabArray*
vogue_layout_get_tabs (VogueLayout *layout)
{
  g_return_val_if_fail (PANGO_IS_LAYOUT (layout), NULL);

  if (layout->tabs)
    return vogue_tab_array_copy (layout->tabs);
  else
    return NULL;
}

/**
 * vogue_layout_set_single_paragraph_mode:
 * @layout: a #VogueLayout
 * @setting: new setting
 *
 ...
 **/
void
vogue_layout_set_single_paragraph_mode (VogueLayout *layout,
					gboolean     setting)
{
  g_return_if_fail (PANGO_IS_LAYOUT (layout));
...
/**
 * vogue_layout_get_single_paragraph_mode:
 * @layout: a #VogueLayout
 *
 * Obtains the value set by vogue_layout_set_single_paragraph_mode().
 *
 * Return value: %TRUE if the layout does not break paragraphs at
 * paragraph separator characters, %FALSE otherwise.
 **/
gboolean
vogue_layout_get_single_paragraph_mode (VogueLayout *layout)
{
  g_return_val_if_fail (PANGO_IS_LAYOUT (layout), FALSE);
 ...
 * vogue_layout_set_width() and vogue_layout_set_height().
 *
 * If the layout contains characters such as newlines that
 * force it to be layed out in multiple paragraphs, then whether
 * each paragraph is ellipsized separately or the entire layout
 * is ellipsized as a whole depends on the set height of the layout.
 * See vogue_layout_set_height() for details.
 *
 * Since: 1.6
 **/
void
vogue_layout_set_ellipsize (VogueLayout        *layout,
...
/**
 * vogue_layout_get_ellipsize:
 * @layout: a #VogueLayout
 *
 * Gets the type of ellipsization being performed for @layout.
 * See vogue_layout_set_ellipsize()
 *
 * Return value: the current ellipsization mode for @layout.
 *
 * Use vogue_layout_is_ellipsized() to query whether any paragraphs
 * were actually ellipsized.
 *
 * Since: 1.6
 **/
VogueEllipsizeMode
vogue_layout_get_ellipsize (VogueLayout *layout)
{
 ...
 */
gboolean
vogue_layout_is_ellipsized (VogueLayout *layout)
{
  g_return_val_if_fail (layout != NULL, FALSE);
 ...
 *
 * Note that if you have used vogue_layout_set_markup() or
 * vogue_layout_set_markup_with_accel() on @layout before, you may
 * want to call vogue_layout_set_attributes() to clear the attributes
 * set on the layout from the markup as this function does not clear
 * attributes.
 **/
void
vogue_layout_set_text (VogueLayout *layout,
		       const char  *text,
		       int          length)
{
  ...
  for (;;) {
     ...
     * ((VogueGlyph) -1) by PANGO_GET_UNKNOWN_GLYPH(-1),
  ...
  if (start != layout->text)
    ...
    g_warning ("Invalid UTF-8 string passed to vogue_layout_set_text()");

  layout->n_chars = vogue_utf8_strlen (layout->text, -1);
 ...
 **/
const char*
vogue_layout_get_text (VogueLayout *layout)
{
  g_return_val_if_fail (PANGO_IS_LAYOUT (layout), NULL);
...
/**
 * vogue_layout_get_character_count:
 * @layout: a #VogueLayout
 *
 ...
 */
gint
vogue_layout_get_character_count (VogueLayout *layout)
{
  g_return_val_if_fail (PANGO_IS_LAYOUT (layout), 0);
...
/**
 * vogue_layout_set_markup:
 * @layout: a #VogueLayout
 * @markup: marked-up text
 * @length: length of marked-up text in bytes, or -1 if @markup is
 *          null-terminated
 *
 * Same as vogue_layout_set_markup_with_accel(), but
 * the markup text isn't scanned for accelerators.
 *
 **/
void
vogue_layout_set_markup (VogueLayout *layout,
			 const char  *markup,
			 int          length)
{
  vogue_layout_set_markup_with_accel (layout, markup, length, 0, NULL);
}

/**
 * vogue_layout_set_markup_with_accel:
 * @layout: a #VogueLayout
 * @markup: marked-up text
 * (see <link linkend="VogueMarkupFormat">markup format</link>)
 * @length: length of marked-up text in bytes, or -1 if @markup is
 *          null-terminated
 ...
 * <link linkend="VogueMarkupFormat">markup format</link>). Replaces
 ...
 **/
void
vogue_layout_set_markup_with_accel (VogueLayout    *layout,
				    const char     *markup,
				    int             length,
				    gunichar        accel_marker,
				    gunichar       *accel_char)
{
  VogueAttrList *list = NULL;
  char *text = NULL;
  GError *error;
  ...
  error = NULL;
  if (!vogue_parse_markup (markup, length,
			   accel_marker,
			   &list, &text,
			   accel_char,
			   &error))
    {
      g_warning ("vogue_layout_set_markup_with_accel: %s", error->message);
      g_error_free (error);
      return;
    }

  vogue_layout_set_text (layout, text, -1);
  vogue_layout_set_attributes (layout, list);
  vogue_attr_list_unref (list);
  g_free (text);
}

/**
 * vogue_layout_get_unknown_glyphs_count:
 * @layout: a #VogueLayout
 *
 ...
 */
int
vogue_layout_get_unknown_glyphs_count (VogueLayout *layout)
{
    VogueLayoutLine *line;
    VogueLayoutRun *run;
    GSList *lines_list;
...
static void
check_context_changed (VogueLayout *layout)
{
  guint old_serial = layout->context_serial;

  layout->context_serial = vogue_context_get_serial (layout->context);
...
static void
layout_changed (VogueLayout *layout)
{
  layout->serial++;
  if (layout->serial == 0)
    layout->serial++;
...
/**
 * vogue_layout_context_changed:
 * @layout: a #VogueLayout
 *
 * Forces recomputation of any state in the #VogueLayout that
 * might depend on the layout's context. This function should
 * be called if you make changes to the context subsequent
 * to creating the layout.
 **/
void
vogue_layout_context_changed (VogueLayout *layout)
{
  g_return_if_fail (PANGO_IS_LAYOUT (layout));
 ...
 * of the setter functions, or the #VogueContext it uses has changed.
 * The serial may wrap, but will never have the value 0. Since it
 * can wrap, never compare it with "less than", always use "not equals".
 *
 * This can be used to automatically detect changes to a #VogueLayout, and
 * is useful for example to decide whether a layout needs redrawing.
 * To force the serial to be increased, use vogue_layout_context_changed().
 *
 ...
 **/
guint
vogue_layout_get_serial (VogueLayout *layout)
{
 ...
 **/
void
vogue_layout_get_log_attrs (VogueLayout    *layout,
			    VogueLogAttr  **attrs,
			    gint           *n_attrs)
{
  g_return_if_fail (layout != NULL);
  ...
  if (attrs)
    {
      *attrs = g_new (VogueLogAttr, layout->n_chars + 1);
      memcpy (*attrs, layout->log_attrs, sizeof(VogueLogAttr) * (layout->n_chars + 1));
    }
...
/**
 * vogue_layout_get_log_attrs_readonly:
 * @layout: a #VogueLayout
 ...
 * the @layout.
 *
 * This is a faster alternative to vogue_layout_get_log_attrs().
 ...
 */
const VogueLogAttr *
vogue_layout_get_log_attrs_readonly (VogueLayout *layout,
                                     gint        *n_attrs)
{
  if (n_attrs)
    *n_attrs = 0;
 ...
 **/
int
vogue_layout_get_line_count (VogueLayout   *layout)
{
  g_return_val_if_fail (layout != NULL, 0);
...
/**
 * vogue_layout_get_lines:
 * @layout: a #VogueLayout
 *
 * Returns the lines of the @layout as a list.
 *
 * Use the faster vogue_layout_get_lines_readonly() if you do not plan
 * to modify the contents of the lines (glyphs, glyph widths, etc.).
 *
 * Return value: (element-type Vogue.LayoutLine) (transfer none): a #GSList containing
 * the lines in the layout. This points to internal data of the #VogueLayout
 * and must be used with care. It will become invalid on any change to the layout's
 * text or properties.
 **/
GSList *
vogue_layout_get_lines (VogueLayout *layout)
{
  vogue_layout_check_lines (layout);
  ...
  if (layout->lines)
    {
      ...
      while (tmp_list)
	{
	  VogueLayoutLine *line = tmp_list->data;
	  tmp_list = tmp_list->next;

	  vogue_layout_line_leaked (line);
	}
    }
...
/**
 * vogue_layout_get_lines_readonly:
 * @layout: a #VogueLayout
 *
 * Returns the lines of the @layout as a list.
 *
 * This is a faster alternative to vogue_layout_get_lines(),
 * but the user is not expected
 * to modify the contents of the lines (glyphs, glyph widths, etc.).
 *
 * Return value: (element-type Vogue.LayoutLine) (transfer none): a #GSList containing
 * the lines in the layout. This points to internal data of the #VogueLayout and
 ...
 **/
GSList *
vogue_layout_get_lines_readonly (VogueLayout *layout)
{
  vogue_layout_check_lines (layout);
 ...
 *        <literal>vogue_layout_get_line_count(layout) - 1</literal>, inclusive.
 *
 * Retrieves a particular line from a #VogueLayout.
 *
 * Use the faster vogue_layout_get_line_readonly() if you do not plan
 * to modify the contents of the line (glyphs, glyph widths, etc.).
 *
 * Return value: (transfer none) (nullable): the requested
 *               #VogueLayoutLine, or %NULL if the index is out of
		       ...
		       int          line)
{
  ...
  if (list_item)
    {
      VogueLayoutLine *line = list_item->data;
...
/**
 * vogue_layout_get_line_readonly:
 * @layout: a #VogueLayout
 * @line: the index of a line, which must be between 0 and
 *        <literal>vogue_layout_get_line_count(layout) - 1</literal>, inclusive.
 *
 * Retrieves a particular line from a #VogueLayout.
 *
 * This is a faster alternative to vogue_layout_get_line(),
 * but the user is not expected
 * to modify the contents of the line (glyphs, glyph widths, etc.).
 *
 * Return value: (transfer none) (nullable): the requested
 ...
 *               range. This layout line can be ref'ed and retained,
 *               but will become invalid if changes are made to the
 *               #VogueLayout.  No changes should be made to the line.
 *
 * Since: 1.16
 **/
VogueLayoutLine *
vogue_layout_get_line_readonly (VogueLayout *layout,
				int          line)
{
  GSList *list_item;
  g_return_val_if_fail (layout != NULL, NULL);
...
/**
 * vogue_layout_line_index_to_x:
 * @line:     a #VogueLayoutLine
 * @index_:   byte offset of a grapheme within the layout
 * @trailing: an integer indicating the edge of the grapheme to retrieve
 *            the position of. If > 0, the trailing edge of the grapheme,
 *            if 0, the leading of the grapheme.
 * @x_pos: (out): location to store the x_offset (in Vogue unit)
 *
 * Converts an index within a line to a X position.
 *
 **/
void
vogue_layout_line_index_to_x (VogueLayoutLine  *line,
			      int               index,
			      int               trailing,
			      int              *x_pos)
{
  VogueLayout *layout = line->layout;
  GSList *run_list = line->runs;
  int width = 0;

  while (run_list)
    {
      VogueLayoutRun *run = run_list->data;
      ItemProperties properties;

      vogue_layout_get_item_properties (run->item, &properties);
      ...
      if (run->item->offset <= index && run->item->offset + run->item->length > index)
	{
      ...
      width += vogue_glyph_string_get_width (run->glyphs);
...
static VogueLayoutLine *
vogue_layout_index_to_line (VogueLayout      *layout,
			    int               index,
			    int              *line_nr,
			    VogueLayoutLine **line_before,
			    VogueLayoutLine **line_after)
{
  GSList *tmp_list;
  GSList *line_list;
  VogueLayoutLine *line = NULL;
  VogueLayoutLine *prev_line = NULL;
  int i = -1;

  line_list = tmp_list = layout->lines;
  while (tmp_list)
    {
      VogueLayoutLine *tmp_line = tmp_list->data;
...
static VogueLayoutLine *
vogue_layout_index_to_line_and_extents (VogueLayout     *layout,
					int              index,
					VogueRectangle  *line_rect)
{
  ...
  if (!ITER_IS_INVALID (&iter))
    while (TRUE)
      {
	VogueLayoutLine *tmp_line = _vogue_layout_iter_get_line (&iter);
	...
	vogue_layout_iter_get_line_extents (&iter, NULL, line_rect);

	if (line->start_index + line->length > index)
	  break;

	if (!vogue_layout_iter_next_line (&iter))
 ...
 *               between 0 and vogue_layout_get_line_count(layout) - 1), or %NULL
 ...
 */
void
vogue_layout_index_to_line_x (VogueLayout *layout,
			      int          index,
			      gboolean     trailing,
			      int         *line,
			      int         *x_pos)
{
  int line_num;
  VogueLayoutLine *layout_line = NULL;
  ...
  layout_line = vogue_layout_index_to_line (layout, index,
					    &line_num, NULL, NULL);
  ...
  if (layout_line)
	...
	*line = line_num;
      ...
      vogue_layout_line_index_to_x (layout_line, index, trailing, x_pos);
    }
  else
    {
      if (line)
...
/**
 * vogue_layout_move_cursor_visually:
 * @layout:       a #VogueLayout.
 ...
 * single call to vogue_layout_move_cursor_visually() may move the
 * cursor over multiple characters when multiple characters combine
 * to form a single grapheme.
 **/
void
vogue_layout_move_cursor_visually (VogueLayout *layout,
				   gboolean     strong,
				   int          old_index,
				   int          old_trailing,
				   int          direction,
...
{
  VogueLayoutLine *line = NULL;
  VogueLayoutLine *prev_line;
  VogueLayoutLine *next_line;
  ...
  line = vogue_layout_index_to_line (layout, old_index,
  ...
  log2vis_map = vogue_layout_line_get_log2vis_map (line, strong);
  n_vis = vogue_utf8_strlen (layout->text + line->start_index, line->length);
  ...
  vis2log_map = vogue_layout_line_get_vis2log_map (line, strong);
...
/**
 * vogue_layout_xy_to_index:
 * @layout:    a #VogueLayout
 * @x:         the X offset (in Vogue units)
 *             from the left edge of the layout.
 * @y:         the Y offset (in Vogue units)
 ...
 * end of the line is chosen as described for vogue_layout_line_x_to_index().
 ...
 **/
gboolean
vogue_layout_xy_to_index (VogueLayout *layout,
			  int          x,
			  int          y,
			  int         *index,
			  gint        *trailing)
{
  ...
  do
    {
      VogueRectangle line_logical;
      int first_y, last_y;

      vogue_layout_iter_get_line_extents (&iter, NULL, &line_logical);
      vogue_layout_iter_get_line_yrange (&iter, &first_y, &last_y);
      ...
      if (y < first_y)
	{
	  if (prev_line && y < (prev_last + (first_y - prev_last) / 2))
		...
		outside = TRUE; /* off the top */
	      ...
	      found = _vogue_layout_iter_get_line (&iter);
	       ...
	       y < last_y)
	{
      ...
      prev_line = _vogue_layout_iter_get_line (&iter);
      prev_last = last_y;
  ...
  retval = vogue_layout_line_x_to_index (found,
					 found_line_x,
					 index, trailing);
 ...
 **/
void
vogue_layout_index_to_pos (VogueLayout    *layout,
			   int             index,
			   VogueRectangle *pos)
{
  VogueRectangle logical_rect;
  ...
  if (!ITER_IS_INVALID (&iter))
      ...
      while (TRUE)
	{
      ...
      vogue_layout_line_index_to_x (layout_line, index, 0, &x_pos);
      pos->x = logical_rect.x + x_pos;

      if (index < layout_line->start_index + layout_line->length)
	{
	  vogue_layout_line_index_to_x (layout_line, index, 1, &x_pos);
...
static void
vogue_layout_line_get_range (VogueLayoutLine *line,
			     char           **start,
			     char           **end)
{
  char *p;
...
static int *
vogue_layout_line_get_vis2log_map (VogueLayoutLine *line,
				   gboolean         strong)
{
  VogueLayout *layout = line->layout;
  VogueDirection prev_dir;
  VogueDirection cursor_dir;
  GSList *tmp_list;
  gchar *start, *end;
  int *result;
  int pos;
  ...
  vogue_layout_line_get_range (line, &start, &end);
  n_chars = vogue_utf8_strlen (start, end - start);
  ...
  while (tmp_list)
    {
      ...
      VogueDirection run_dir = (run->item->analysis.level % 2) ? PANGO_DIRECTION_RTL : PANGO_DIRECTION_LTR;
...
static int *
vogue_layout_line_get_log2vis_map (VogueLayoutLine *line,
				   gboolean         strong)
{
  gchar *start, *end;
  int *reverse_map;
  ...
  reverse_map = vogue_layout_line_get_vis2log_map (line, strong);
...
static VogueDirection
vogue_layout_line_get_char_direction (VogueLayoutLine *layout_line,
				      int              index)
{
  GSList *run_list;
 ...
 **/
void
vogue_layout_get_cursor_pos (VogueLayout    *layout,
			     int             index,
			     ...
			     VogueRectangle *weak_pos)
{
  VogueDirection dir1;
  VogueRectangle line_rect;
  VogueLayoutLine *layout_line = NULL; /* Quiet GCC */
  int x1_trailing;
  int x2;
  ...
  layout_line = vogue_layout_index_to_line_and_extents (layout, index,
							&line_rect);
  ...
  if (index == layout_line->start_index)
  ...
  else
    {
      ...
      dir1 = vogue_layout_line_get_char_direction (layout_line, prev_index);
      vogue_layout_line_index_to_x (layout_line, prev_index, TRUE, &x1_trailing);
    }
  ...
  if (index >= layout_line->start_index + layout_line->length)
    ...
    {
      vogue_layout_line_index_to_x (layout_line, index, FALSE, &x2);
    }
...
static inline int
direction_simple (VogueDirection d)
{
  switch (d)
    {
...
static VogueAlignment
get_alignment (VogueLayout     *layout,
	       VogueLayoutLine *line)
{
  VogueAlignment alignment = layout->alignment;

  if (alignment != PANGO_ALIGN_CENTER && line->layout->auto_dir &&
      direction_simple (line->resolved_dir) ==
      -direction_simple (vogue_context_get_base_dir (layout->context)))
    {
...
static void
get_x_offset (VogueLayout     *layout,
	      VogueLayoutLine *line,
	      int              layout_width,
	      int              line_width,
	      int             *x_offset)
{
  VogueAlignment alignment = get_alignment (layout, line);
...
static void
vogue_layout_line_get_extents_and_height (VogueLayoutLine *line,
                                          VogueRectangle *ink,
                                          VogueRectangle *logical,
                                          int            *height);
static void
get_line_extents_layout_coords (VogueLayout     *layout,
				VogueLayoutLine *line,
				int              layout_width,
				int              y_offset,
				int             *baseline,
				VogueRectangle  *line_ink_layout,
				VogueRectangle  *line_logical_layout)
{
  int x_offset;
  /* Line extents in line coords (origin at line baseline) */
  VogueRectangle line_ink;
  VogueRectangle line_logical;
  gboolean first_line;
  int new_baseline;
  int height;
  ...
  vogue_layout_line_get_extents_and_height (line, line_ink_layout ? &line_ink : NULL,
                                            &line_logical,
                                            &height);
 ...
 */
static void
vogue_layout_get_extents_internal (VogueLayout    *layout,
				   VogueRectangle *ink_rect,
				   VogueRectangle *logical_rect,
                                   Extents        **line_extents)
{
  ...
  if (layout->auto_dir)
	...
	{
	  VogueLayoutLine *line = line_list->data;
  ...
  if (width == -1 && need_width && (ink_rect || line_extents))
    {
      VogueRectangle overall_logical;

      vogue_layout_get_extents_internal (layout, NULL, &overall_logical, NULL);
 ...
 * a set width.
 *
 * The extents are given in layout coordinates and in Vogue units; layout
 * coordinates begin at the top left corner of the layout.
 */
void
vogue_layout_get_extents (VogueLayout    *layout,
			  VogueRectangle *ink_rect,
			  VogueRectangle *logical_rect)
{
  g_return_if_fail (layout != NULL);

  vogue_layout_get_extents_internal (layout, ink_rect, logical_rect, NULL);
}

/**
 * vogue_layout_get_pixel_extents:
 * @layout:   a #VogueLayout
 * @ink_rect: (out) (allow-none): rectangle used to store the extents of the
 *                   layout as drawn or %NULL to indicate that the result is
 *                   not needed.
 ...
 * This function just calls vogue_layout_get_extents() followed by
 * two vogue_extents_to_pixels() calls, rounding @ink_rect and @logical_rect
 * such that the rounded rectangles fully contain the unrounded one (that is,
 * passes them as first argument to vogue_extents_to_pixels()).
 **/
void
vogue_layout_get_pixel_extents (VogueLayout *layout,
				VogueRectangle *ink_rect,
				VogueRectangle *logical_rect)
{
  g_return_if_fail (PANGO_IS_LAYOUT (layout));

  vogue_layout_get_extents (layout, ink_rect, logical_rect);
  vogue_extents_to_pixels (ink_rect, NULL);
  vogue_extents_to_pixels (logical_rect, NULL);
}

/**
 * vogue_layout_get_size:
 * @layout: a #VogueLayout
 ...
 *
 * Determines the logical width and height of a #VogueLayout
 * in Vogue units (device units scaled by %PANGO_SCALE). This
 * is simply a convenience function around vogue_layout_get_extents().
 **/
void
vogue_layout_get_size (VogueLayout *layout,
		       int         *width,
		       int         *height)
{
  VogueRectangle logical_rect;

  vogue_layout_get_extents (layout, NULL, &logical_rect);
...
/**
 * vogue_layout_get_pixel_size:
 * @layout: a #VogueLayout
 * @width: (out) (allow-none): location to store the logical width, or %NULL
 * @height: (out) (allow-none): location to store the logical height, or %NULL
 *
 * Determines the logical width and height of a #VogueLayout
 * in device units. (vogue_layout_get_size() returns the width
 * and height scaled by %PANGO_SCALE.) This
 * is simply a convenience function around
 * vogue_layout_get_pixel_extents().
 **/
void
vogue_layout_get_pixel_size (VogueLayout *layout,
			     int         *width,
			     int         *height)
{
  VogueRectangle logical_rect;
 ...
 **/
int
vogue_layout_get_baseline (VogueLayout    *layout)
{
  int baseline;
  Extents *extents = NULL;

  /* XXX this is kinda inefficient */
  vogue_layout_get_extents_internal (layout, NULL, NULL, &extents);
...
static void
vogue_layout_clear_lines (VogueLayout *layout)
{
  if (layout->lines)
    {
      GSList *tmp_list = layout->lines;
...
static void
vogue_layout_line_leaked (VogueLayoutLine *line)
{
  VogueLayoutLinePrivate *private = (VogueLayoutLinePrivate *)line;
...
static void shape_tab (VogueLayoutLine  *line,
                       VogueItem        *item,
		       VogueGlyphString *glyphs);

static void
free_run (VogueLayoutRun *run, gpointer data)
{
  gboolean free_item = data != NULL;
  if (free_item)
    vogue_item_free (run->item);

  vogue_glyph_string_free (run->glyphs);
  g_slice_free (VogueLayoutRun, run);
}

static VogueItem *
uninsert_run (VogueLayoutLine *line)
{
  VogueLayout *layout = line->layout;
  VogueLayoutRun *run, *prev;
  VogueItem *item;
...
static void
ensure_tab_width (VogueLayout *layout)
{
  if (layout->tab_width == -1)
    {
      ...
      GList *items;
      VogueAttribute *attr;
      VogueAttrList *layout_attrs;
      VogueAttrList *tmp_attrs;
      VogueAttrIterator *iter;
      VogueFontDescription *font_desc = vogue_font_description_copy_static (vogue_context_get_font_description (layout->context));
      VogueLanguage *language;

      layout_attrs = vogue_layout_get_effective_attributes (layout);
      iter = vogue_attr_list_get_iterator (layout_attrs);
      vogue_attr_iterator_get_font (iter, font_desc, &language, NULL);

      tmp_attrs = vogue_attr_list_new ();

      attr = vogue_attr_font_desc_new (font_desc);
      vogue_font_description_free (font_desc);
      vogue_attr_list_insert_before (tmp_attrs, attr);

      if (language)
	{
	  attr = vogue_attr_language_new (language);
	  vogue_attr_list_insert_before (tmp_attrs, attr);
	}

      items = vogue_itemize (layout->context, " ", 0, 1, tmp_attrs, NULL);

      vogue_attr_iterator_destroy (iter);
      if (layout_attrs != layout->attrs)
        {
	  vogue_attr_list_unref (layout_attrs);
	  layout_attrs = NULL;
	}
      vogue_attr_list_unref (tmp_attrs);

      item = items->data;
      vogue_shape ("        ", 8, &item->analysis, glyphs);

      vogue_item_free (item);
      g_list_free (items);

      layout->tab_width = vogue_glyph_string_get_width (glyphs);
 ...
 */
static int
get_tab_pos (VogueLayout *layout, int index, gboolean *is_default)
{
  gint n_tabs;
  gboolean in_pixels;
  ...
  if (layout->tabs)
    {
      n_tabs = vogue_tab_array_get_size (layout->tabs);
      in_pixels = vogue_tab_array_get_positions_in_pixels (layout->tabs);
      if (is_default)
	*is_default = FALSE;
    }
  else
  ...
  if (index < n_tabs)
    {
      ...
      vogue_tab_array_get_tab (layout->tabs, index, NULL, &pos);
  ...
  if (n_tabs > 0)
    {
      ...
      vogue_tab_array_get_tab (layout->tabs, n_tabs - 1, NULL, &last_pos);

      if (n_tabs > 1)
	vogue_tab_array_get_tab (layout->tabs, n_tabs - 2, NULL, &next_to_last_pos);
      else
	next_to_last_pos = 0;
...
static int
line_width (VogueLayoutLine *line)
{
  GSList *l;
  int i;
  int width = 0;
...
static gboolean
showing_space (const VogueAnalysis *analysis)
{
  GSList *l;
  ...
  for (l = analysis->extra_attrs; l; l = l->next)
    {
      ...
      if (attr->klass->type == PANGO_ATTR_SHOW &&
          (((VogueAttrInt*)attr)->value & PANGO_SHOW_SPACES) != 0)
        return TRUE;
    }
...
static void
shape_tab (VogueLayoutLine  *line,
           VogueItem        *item,
	   VogueGlyphString *glyphs)
{
  int i, space_width;
  ...
  vogue_glyph_string_set_size (glyphs, 1);
...
can_break_at (VogueLayout *layout,
	      gint         offset,
	      gboolean     always_wrap_char)
{
  ...
  if (offset == layout->n_chars)
    ...
    {
      g_warning (G_STRLOC": broken VogueLayout");
      return TRUE;
    }
}

static inline gboolean
can_break_in (VogueLayout *layout,
	      int          start_offset,
	      int          num_chars,
...
struct _ParaBreakState
{
  ...
  VogueAttrList *attrs;		/* Attributes being used for itemization */
  GList *items;			/* This paragraph turned into items */
  VogueDirection base_dir;	/* Current resolved base direction */
  int line_of_par;		/* Line of the paragraph, starting at 1 for first line */

  VogueGlyphString *glyphs;	/* Glyphs for the first item in state->items */
...
static gboolean
should_ellipsize_current_line (VogueLayout    *layout,
			       ParaBreakState *state);

static VogueGlyphString *
shape_run (VogueLayoutLine *line,
	   ParaBreakState  *state,
	   VogueItem       *item)
{
  VogueLayout *layout = line->layout;
  ...
  if (layout->text[item->offset] == '\t')
    ...
    {
      VogueShapeFlags shape_flags = PANGO_SHAPE_NONE;

      if (vogue_context_get_round_glyph_positions (layout->context))
        shape_flags |= PANGO_SHAPE_ROUND_POSITIONS;

      if (state->properties.shape_set)
	_vogue_shape_shape (layout->text + item->offset, item->num_chars,
			    state->properties.shape_ink_rect, state->properties.shape_logical_rect,
			    glyphs);
      else
        vogue_shape_with_flags (layout->text + item->offset, item->length,
      ...
      if (state->properties.letter_spacing)
	{
	  ...
	  vogue_glyph_item_letter_space (&glyph_item,
					 layout->text,
...
static void
insert_run (VogueLayoutLine *line,
	    ParaBreakState  *state,
	    VogueItem       *run_item,
	    gboolean         last_run)
{
  VogueLayoutRun *run = g_slice_new (VogueLayoutRun);
  ...
  if (last_run)
	...
	vogue_glyph_string_free (state->glyphs);
      state->glyphs = NULL;
...
static void
get_need_hyphen (VogueItem  *item,
                 const char *text,
                 int        *need_hyphen)
{
  int i;
  ...
  for (l = item->analysis.extra_attrs; l; l = l->next)
    {
      ...
      if (attr->klass->type == PANGO_ATTR_INSERT_HYPHENS)
        vogue_attr_list_change (attrs, vogue_attribute_copy (attr));
    }
  ...
  for (i = 0, p = text + item->offset; i < item->num_chars; i++, p = g_utf8_next_char (p))
    {
      ...
      if (start <= pos && pos < end)
        {
          VogueAttribute *attr;
          attr = vogue_attr_iterator_get (iter, PANGO_ATTR_INSERT_HYPHENS);
          if (attr)
            insert_hyphens = ((VogueAttrInt*)attr)->value;
        }
...
static gboolean
break_needs_hyphen (VogueLayout    *layout,
                    ParaBreakState *state,
                    int             pos)
{
...
static int
find_hyphen_width (VogueItem *item)
{
  hb_font_t *hb_font;
   ...
   */
  hb_font = vogue_font_get_hb_font (item->analysis.font);
...
static int
find_break_extra_width (VogueLayout    *layout,
	                ParaBreakState *state,
                        int             pos)
{
  ...
  if (break_needs_hyphen (layout, state, pos))
    {
      if (state->hyphen_width < 0)
        {
          VogueItem *item = state->items->data;
...
# define DEBUG debug
void
debug (const char *where, VogueLayoutLine *line, ParaBreakState *state)
{
  int line_width = vogue_layout_line_get_width (line);
 ...
 */
static BreakResult
process_item (VogueLayout     *layout,
	      VogueLayoutLine *line,
	      ParaBreakState  *state,
	      gboolean         force_fit,
...
#define LINE_SEPARATOR 0x2028
  ...
  if (!state->glyphs)
    {
      vogue_layout_get_item_properties (item, &state->properties);
  ...
  if (processing_new_item)
    {
      width = vogue_glyph_string_get_width (state->glyphs);
    }
  else
    {
  ...
  if ((width <= state->remaining_width || (item->num_chars == 1 && !line->runs)) &&
  ...
  else
    {
      ...
      if (processing_new_item)
	{
	  VogueGlyphItem glyph_item = {item, state->glyphs};
	  state->log_widths = g_new (int, item->num_chars);
	  vogue_glyph_item_get_logical_widths (&glyph_item, layout->text, state->log_widths);
      ...
      if (force_fit || break_width + break_extra_width <= state->remaining_width)	/* Successfully broke the item */
	{
	  ...
	  if (break_num_chars == item->num_chars)
	  ...
	  else
	    {
	      ...
	      new_item = vogue_item_split (item, length, break_num_chars);
	      ...
	      break_width = vogue_glyph_string_get_width (((VogueGlyphItem *)(line->runs->data))->glyphs);
	      state->remaining_width -= break_width;
 ...
 */
static void
line_set_resolved_dir (VogueLayoutLine *line,
		       VogueDirection   direction)
{
   ...
   *
   * A similar dance is performed in vogue-context.c:
   * itemize_state_add_character().  Keep in synch.
   */
  switch (vogue_context_get_gravity (line->layout->context))
    {
    default:
	  ...
	  ParaBreakState  *state)
{
  ...
  if (layout->height >= 0)
    {
      VogueRectangle logical_rect;
      vogue_layout_line_get_extents (line, NULL, &logical_rect);
...
static void
process_line (VogueLayout    *layout,
	      ParaBreakState *state)
{
  VogueLayoutLine *line;
  ...
  line = vogue_layout_line_new (layout);
  ...
  vogue_layout_line_postprocess (line, state, wrapped);
  add_line (line, state);
  state->line_of_par++;
                     ...
                     int           log_attrs_len)
{
  ...
  vogue_default_break (text, length, NULL, log_attrs, log_attrs_len);

  for (l = items; l; l = l->next)
    {
      VogueItem *item = l->data;

      vogue_tailor_break (text + item->offset,
                          item->length,
                          &item->analysis,
                          item->offset,
                          log_attrs + offset,
...
static VogueAttrList *
vogue_layout_get_effective_attributes (VogueLayout *layout)
{
  VogueAttrList *attrs;

  if (layout->attrs)
    attrs = vogue_attr_list_copy (layout->attrs);
  else
    attrs = vogue_attr_list_new ();

  if (layout->font_desc)
    {
      VogueAttribute *attr = vogue_attr_font_desc_new (layout->font_desc);
      vogue_attr_list_insert_before (attrs, attr);
    }

  if (layout->single_paragraph)
    {
      VogueAttribute *attr = vogue_attr_show_new (PANGO_SHOW_LINE_BREAKS);
      vogue_attr_list_insert_before (attrs, attr);
    }
...
static void
apply_attributes_to_runs (VogueLayout   *layout,
                          VogueAttrList *attrs)
{
  GSList *ll;
  ...
  for (ll = layout->lines; ll; ll = ll->next)
    {
      ...
      for (rl = old_runs; rl; rl = rl->next)
        {
          VogueGlyphItem *glyph_item = rl->data;
          GSList *new_runs;

          new_runs = vogue_glyph_item_apply_attrs (glyph_item,
                                                   layout->text,
                                                   attrs);
...
static void
vogue_layout_check_lines (VogueLayout *layout)
{
  const char *start;
  gboolean done = FALSE;
  int start_offset;
  ...
  VogueAttrIterator *iter;
  VogueDirection prev_base_dir = PANGO_DIRECTION_NEUTRAL, base_dir = PANGO_DIRECTION_NEUTRAL;
  ParaBreakState state;
   ...
   */
  if (G_UNLIKELY (!layout->text))
    vogue_layout_set_text (layout, NULL, 0);

  attrs = vogue_layout_get_effective_attributes (layout);
  ...
  if (itemize_attrs)
    iter = vogue_attr_list_get_iterator (itemize_attrs);
  else
    iter = NULL;

  layout->log_attrs = g_new (VogueLogAttr, layout->n_chars + 1);
  ...
  if (layout->auto_dir)
    {
      prev_base_dir = vogue_find_base_dir (layout->text, layout->length);
      if (prev_base_dir == PANGO_DIRECTION_NEUTRAL)
	prev_base_dir = vogue_context_get_base_dir (layout->context);
    }
  else
    base_dir = vogue_context_get_base_dir (layout->context);
  ...
  if (layout->height >= 0)
    {
      VogueRectangle logical;
      vogue_layout_get_empty_extents_at_index (layout, 0, &logical);
  ...
  do
	...
	{
	  vogue_find_paragraph_boundary (start,
      ...
      if (layout->auto_dir)
	{
	  base_dir = vogue_find_base_dir (start, delimiter_index);
      ...
      state.items = vogue_itemize_with_base_dir (layout->context,
						 base_dir,
						 layout->text,
						 start - layout->text,
						 end - start,
	...
	{
	  VogueLayoutLine *empty_line;

	  empty_line = vogue_layout_line_new (layout);
      ...
      if (!done)
	start_offset += vogue_utf8_strlen (start, (end - start) + delim_len);
...
/**
 * vogue_layout_line_ref:
 * @line: (nullable): a #VogueLayoutLine, may be %NULL
 *
 * Increase the reference count of a #VogueLayoutLine by one.
 *
 ...
 **/
VogueLayoutLine *
vogue_layout_line_ref (VogueLayoutLine *line)
{
  VogueLayoutLinePrivate *private = (VogueLayoutLinePrivate *)line;
...
/**
 * vogue_layout_line_unref:
 * @line: a #VogueLayoutLine
 *
 * Decrease the reference count of a #VogueLayoutLine by one.
 * If the result is zero, the line and all associated memory
 * will be freed.
 **/
void
vogue_layout_line_unref (VogueLayoutLine *line)
{
  VogueLayoutLinePrivate *private = (VogueLayoutLinePrivate *)line;
  ...
  if (g_atomic_int_dec_and_test ((int *) &private->ref_count))
    {
      ...
      g_slice_free (VogueLayoutLinePrivate, private);
    }
}

G_DEFINE_BOXED_TYPE (VogueLayoutLine, vogue_layout_line,
                     vogue_layout_line_ref,
                     vogue_layout_line_unref);

/**
 * vogue_layout_line_x_to_index:
 * @line:      a #VogueLayoutLine
 ...
 **/
gboolean
vogue_layout_line_x_to_index (VogueLayoutLine *line,
			      int              x_pos,
			      int             *index,
			      int             *trailing)
{
  ...
  if (tmp_list->next &&
      line->start_index + line->length == ((VogueLayoutLine *)tmp_list->next->data)->start_index)
  ...
  while (tmp_list)
    {
      ...
      logical_width = vogue_glyph_string_get_width (run->glyphs);
      ...
      if (x_pos >= start_pos && x_pos < start_pos + logical_width)
	{
	  ...
	  vogue_glyph_string_x_to_index (run->glyphs,
...
static int
vogue_layout_line_get_width (VogueLayoutLine *line)
{
  int width = 0;
...
/**
 * vogue_layout_line_get_x_ranges:
 * @line:        a #VogueLayoutLine
 ...
 *               to the layout and are in Vogue units.
 ...
 **/
void
vogue_layout_line_get_x_ranges (VogueLayoutLine  *line,
				int               start_index,
				int               end_index,
				int             **ranges,
				int              *n_ranges)
{
  ...
  if (width == -1 && alignment != PANGO_ALIGN_LEFT)
    {
      VogueRectangle logical_rect;
      vogue_layout_get_extents (line->layout, NULL, &logical_rect);
      width = logical_rect.width;
    }
  ...
  while (tmp_list)
    {
      VogueLayoutRun *run = (VogueLayoutRun *)tmp_list->data;
	   ...
	   end_index > run->item->offset))
	{
      ...
      if (tmp_list->next)
	accumulated_width += vogue_glyph_string_get_width (run->glyphs);
...
static void
vogue_layout_get_empty_extents_at_index (VogueLayout    *layout,
					 int             index,
					 VogueRectangle *logical_rect)
{
  if (logical_rect)
    {
      VogueFont *font;
      VogueFontDescription *font_desc = NULL;
      gboolean free_font_desc = FALSE;

      font_desc = vogue_context_get_font_description (layout->context);

      if (layout->font_desc)
        {
	  font_desc = vogue_font_description_copy_static (font_desc);
	  vogue_font_description_merge (font_desc, layout->font_desc, TRUE);
	  free_font_desc = TRUE;
	}
      ...
      if (layout->attrs)
	{
	  VogueAttrIterator *iter = vogue_attr_list_get_iterator (layout->attrs);
	  int start, end;
	  ...
	  do
	      ...
	      if (start <= index && index < end)
		{
		  ...
		  vogue_attr_iterator_get_font (iter,
						font_desc,
						NULL,
						NULL);
      ...
      font = vogue_context_load_font (layout->context, font_desc);
      if (font)
	{
	  VogueFontMetrics *metrics;

	  metrics = vogue_font_get_metrics (font,
					    vogue_context_get_language (layout->context));

	  if (metrics)
	    {
	      logical_rect->y = - vogue_font_metrics_get_ascent (metrics);
	      logical_rect->height = - logical_rect->y + vogue_font_metrics_get_descent (metrics);
...
static void
vogue_layout_line_get_empty_extents (VogueLayoutLine *line,
				     VogueRectangle  *logical_rect)
{
  vogue_layout_get_empty_extents_at_index (line->layout, line->start_index, logical_rect);
}

static void
vogue_layout_run_get_extents_and_height (VogueLayoutRun *run,
                                         VogueRectangle *run_ink,
                                         VogueRectangle *run_logical,
                                         int            *height)
{
  VogueRectangle logical;
  ItemProperties properties;
  VogueFontMetrics *metrics = NULL;
  ...
  if (properties.shape_set)
    _vogue_shape_get_extents (run->item->num_chars,
  ...
  if (run_ink && (properties.uline != PANGO_UNDERLINE_NONE || properties.strikethrough))
    {
      ...
      underline_thickness = vogue_font_metrics_get_underline_thickness (metrics);
      underline_position = vogue_font_metrics_get_underline_position (metrics);
      strikethrough_thickness = vogue_font_metrics_get_strikethrough_thickness (metrics);
      strikethrough_position = vogue_font_metrics_get_strikethrough_position (metrics);
  ...
  if (height)
    {
      if (!metrics)
        metrics = vogue_font_get_metrics (run->item->analysis.font,
				          run->item->analysis.language);
      *height = vogue_font_metrics_get_height (metrics);
    }
                                          ...
                                          int             *height)
{
  ...
  while (tmp_list)
    {
      ...
      vogue_layout_run_get_extents_and_height (run,
  ...
  if (logical_rect && !line->runs)
    vogue_layout_line_get_empty_extents (line, logical_rect);
 ...
 * vogue_font_get_glyph_extents() for details about the interpretation
 * of the rectangles.
 */
void
vogue_layout_line_get_extents (VogueLayoutLine *line,
			       VogueRectangle  *ink_rect,
			       VogueRectangle  *logical_rect)
{
  vogue_layout_line_get_extents_and_height (line, ink_rect, logical_rect, NULL);
}

/**
 * vogue_layout_line_get_height:
 * @line:     a #VogueLayoutLine
 ...
 */
void
vogue_layout_line_get_height (VogueLayoutLine *line,
			      int             *height)
{
  vogue_layout_line_get_extents_and_height (line, NULL, NULL, height);
}

static VogueLayoutLine *
vogue_layout_line_new (VogueLayout *layout)
{
  VogueLayoutLinePrivate *private = g_slice_new (VogueLayoutLinePrivate);
  ...
  return (VogueLayoutLine *) private;
}

/**
 * vogue_layout_line_get_pixel_extents:
 * @layout_line: a #VogueLayoutLine
 * @ink_rect: (out) (allow-none): rectangle used to store the extents of
 *                   the glyph string as drawn, or %NULL
 * @logical_rect: (out) (allow-none): rectangle used to store the logical
 *                       extents of the glyph string, or %NULL
 ...
 * This function just calls vogue_layout_line_get_extents() followed by
 * two vogue_extents_to_pixels() calls, rounding @ink_rect and @logical_rect
 * such that the rounded rectangles fully contain the unrounded one (that is,
 * passes them as first argument to vogue_extents_to_pixels()).
 **/
void
vogue_layout_line_get_pixel_extents (VogueLayoutLine *layout_line,
				     VogueRectangle  *ink_rect,
				     VogueRectangle  *logical_rect)
{
  g_return_if_fail (LINE_IS_VALID (layout_line));

  vogue_layout_line_get_extents (layout_line, ink_rect, logical_rect);
  vogue_extents_to_pixels (ink_rect, NULL);
  vogue_extents_to_pixels (logical_rect, NULL);
}
 ...
 *     reorder-items.c:vogue_reorder_items().
 */
...
static void
vogue_layout_line_reorder (VogueLayoutLine *line)
{
...
static int
get_item_letter_spacing (VogueItem *item)
{
  ItemProperties properties;

  vogue_layout_get_item_properties (item, &properties);

  return properties.letter_spacing;
}

static void
pad_glyphstring_right (VogueGlyphString *glyphs,
		       ParaBreakState   *state,
		       int               adjustment)
{
...
static void
pad_glyphstring_left (VogueGlyphString *glyphs,
		      ParaBreakState   *state,
		      int               adjustment)
{
  int glyph = 0;
...
static void
zero_line_final_space (VogueLayoutLine *line,
		       ParaBreakState  *state,
		       VogueLayoutRun  *run)
{
  VogueLayout *layout = line->layout;
  VogueItem *item = run->item;
  VogueGlyphString *glyphs = run->glyphs;
 ...
 */
static void
adjust_line_letter_spacing (VogueLayoutLine *line,
			    ParaBreakState  *state)
{
  VogueLayout *layout = line->layout;
  gboolean reversed;
  VogueLayoutRun *last_run;
  int tab_adjustment;
  GSList *l;
  ...
  for (l = line->runs; l; l = l->next)
    {
      VogueLayoutRun *run = l->data;
      VogueLayoutRun *next_run = l->next ? l->next->data : NULL;
	...
	{
	  VogueLayoutRun *visual_next_run = reversed ? last_run : next_run;
	  VogueLayoutRun *visual_last_run = reversed ? next_run : last_run;
...
static void
justify_clusters (VogueLayoutLine *line,
		  ParaBreakState  *state)
{
  const gchar *text = line->layout->text;
  const VogueLogAttr *log_attrs = line->layout->log_attrs;
  ...
  for (mode = MEASURE; mode <= ADJUST; mode++)
    {
      ...
      VogueGlyphString *rightmost_glyphs = NULL;
	...
	{
	  VogueLayoutRun *run = run_iter->data;
	  VogueGlyphString *glyphs = run->glyphs;
	  ...
	  offset = state->line_start_offset
		 + vogue_utf8_strlen (text + state->line_start_index,
				      run->item->offset - state->line_start_index);

	  for (have_cluster = dir > 0 ?
		 vogue_glyph_item_iter_init_start (&cluster_iter, run, text) :
		 vogue_glyph_item_iter_init_end   (&cluster_iter, run, text);
	       have_cluster;
	       have_cluster = dir > 0 ?
	         vogue_glyph_item_iter_next_cluster (&cluster_iter) :
	         vogue_glyph_item_iter_prev_cluster (&cluster_iter))
	    {
	      int i;
	      int width = 0;
...
static void
justify_words (VogueLayoutLine *line,
	       ParaBreakState  *state)
{
  const gchar *text = line->layout->text;
  const VogueLogAttr *log_attrs = line->layout->log_attrs;
  ...
  for (mode = MEASURE; mode <= ADJUST; mode++)
      ...
      for (run_iter = line->runs; run_iter; run_iter = run_iter->next)
	{
	  ...
	  for (have_cluster = vogue_glyph_item_iter_init_start (&cluster_iter, run, text);
	       have_cluster;
...
static void
vogue_layout_line_postprocess (VogueLayoutLine *line,
			       ParaBreakState  *state,
			       gboolean         wrapped)
{
  ...
  if (G_UNLIKELY (state->line_width >= 0 &&
    ...
    {
      ellipsized = _vogue_layout_line_ellipsize (line, state->attrs, state->line_width);
    }
   ...
   */
  vogue_layout_line_reorder (line);
  ...
  if (line->layout->justify && (wrapped || ellipsized))
    {
      ...
      if (state->remaining_width < 0)
	state->remaining_width = state->line_width - vogue_layout_line_get_width (line);
...
static void
vogue_layout_get_item_properties (VogueItem      *item,
				  ItemProperties *properties)
{
  ...
  while (tmp_list)
	...
	{
	case PANGO_ATTR_UNDERLINE:
	  properties->uline = ((VogueAttrInt *)attr)->value;
	  break;

	case PANGO_ATTR_STRIKETHROUGH:
	  properties->strikethrough = ((VogueAttrInt *)attr)->value;
	  break;

	case PANGO_ATTR_RISE:
	  properties->rise = ((VogueAttrInt *)attr)->value;
	  break;

	case PANGO_ATTR_LETTER_SPACING:
	  properties->letter_spacing = ((VogueAttrInt *)attr)->value;
	  break;

	case PANGO_ATTR_SHAPE:
	  properties->shape_set = TRUE;
	  properties->shape_logical_rect = &((VogueAttrShape *)attr)->logical_rect;
	  properties->shape_ink_rect = &((VogueAttrShape *)attr)->ink_rect;
	  break;
...
static int
next_cluster_start (VogueGlyphString *gs,
		    int               cluster_start)
{
  int i;
...
static int
cluster_width (VogueGlyphString *gs,
	       int               cluster_start)
{
  int i;
  int width;
 ...
 */
static void
update_cluster (VogueLayoutIter *iter,
		int              cluster_start_index)
{
  ...
  iter->cluster_num_chars = vogue_utf8_strlen (cluster_text, cluster_length);
...
static void
update_run (VogueLayoutIter *iter,
	    int              run_start_index)
{
  ...
  if (iter->run)
    {
      iter->run_width = vogue_glyph_string_get_width (iter->run->glyphs);
    }
  else
    {
...
/**
 * vogue_layout_iter_copy:
 * @iter: (nullable): a #VogueLayoutIter, may be %NULL
 *
 * Copies a #VogueLayoutIter.
 *
 * Return value: (nullable): the newly allocated #VogueLayoutIter,
 *               which should be freed with vogue_layout_iter_free(),
 *               or %NULL if @iter was %NULL.
 *
 * Since: 1.20
 **/
VogueLayoutIter *
vogue_layout_iter_copy (VogueLayoutIter *iter)
{
  VogueLayoutIter *new;
...
G_DEFINE_BOXED_TYPE (VogueLayoutIter, vogue_layout_iter,
                     vogue_layout_iter_copy,
                     vogue_layout_iter_free);

/**
 * vogue_layout_get_iter:
 * @layout: a #VogueLayout
 *
 * Returns an iterator to iterate over the visual extents of the layout.
 *
 * Return value: the new #VogueLayoutIter that should be freed using
 *               vogue_layout_iter_free().
 **/
VogueLayoutIter*
vogue_layout_get_iter (VogueLayout *layout)
{
  VogueLayoutIter *iter;

  g_return_val_if_fail (PANGO_IS_LAYOUT (layout), NULL);

  iter = g_slice_new (VogueLayoutIter);
...
void
_vogue_layout_get_iter (VogueLayout    *layout,
                        VogueLayoutIter*iter)
{
  int run_start_index;
  VogueRectangle logical_rect;
  ...
  vogue_layout_line_ref (iter->line);
  ...
  vogue_layout_get_extents_internal (layout,
				     NULL,
				     &logical_rect,
				     &iter->line_extents);
...
void
_vogue_layout_iter_destroy (VogueLayoutIter *iter)
{
  if (iter == NULL)
    return;

  g_free (iter->line_extents);
  vogue_layout_line_unref (iter->line);
 ...
 **/
void
vogue_layout_iter_free (VogueLayoutIter *iter)
{
  if (iter == NULL)
    return;

  _vogue_layout_iter_destroy (iter);
  g_slice_free (VogueLayoutIter, iter);
}

/**
 * vogue_layout_iter_get_index:
 * @iter: a #VogueLayoutIter
 *
 * Gets the current byte index. Note that iterating forward by char
 * moves in visual order, not logical order, so indexes may not be
 * sequential. Also, the index may be equal to the length of the text
 * in the layout, if on the %NULL run (see vogue_layout_iter_get_run()).
 *
 * Return value: current byte index.
 **/
int
vogue_layout_iter_get_index (VogueLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return 0;
 ...
 *
 * Use the faster vogue_layout_iter_get_run_readonly() if you do not plan
 * to modify the contents of the run (glyphs, glyph widths, etc.).
 *
 * Return value: (transfer none) (nullable): the current run.
 **/
VogueLayoutRun*
vogue_layout_iter_get_run (VogueLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return NULL;

  vogue_layout_line_leaked (iter->line);

  return iter->run;
}

/**
 * vogue_layout_iter_get_run_readonly:
 * @iter: a #VogueLayoutIter
 *
 * Gets the current run. When iterating by run, at the end of each
 * line, there's a position with a %NULL run, so this function can return
 ...
 * at least one run, even lines consisting of only a newline.
 *
 * This is a faster alternative to vogue_layout_iter_get_run(),
 * but the user is not expected
 * to modify the contents of the run (glyphs, glyph widths, etc.).
 *
 ...
 **/
VogueLayoutRun*
vogue_layout_iter_get_run_readonly (VogueLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return NULL;
...
static VogueLayoutLine*
_vogue_layout_iter_get_line (VogueLayoutIter *iter)
{
  return iter->line;
}

/**
 * vogue_layout_iter_get_line:
 * @iter: a #VogueLayoutIter
 *
 * Gets the current line.
 *
 * Use the faster vogue_layout_iter_get_line_readonly() if you do not plan
 * to modify the contents of the line (glyphs, glyph widths, etc.).
 *
 * Return value: (transfer none): the current line.
 **/
VogueLayoutLine*
vogue_layout_iter_get_line (VogueLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return NULL;
...
/**
 * vogue_layout_iter_get_line_readonly:
 * @iter: a #VogueLayoutIter
 *
 * Gets the current line for read-only access.
 *
 * This is a faster alternative to vogue_layout_iter_get_line(),
 * but the user is not expected
 * to modify the contents of the line (glyphs, glyph widths, etc.).
 *
 ...
 **/
VogueLayoutLine*
vogue_layout_iter_get_line_readonly (VogueLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return NULL;
...
/**
 * vogue_layout_iter_at_last_line:
 * @iter: a #VogueLayoutIter
 *
 ...
 **/
gboolean
vogue_layout_iter_at_last_line (VogueLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return FALSE;
...
/**
 * vogue_layout_iter_get_layout:
 * @iter: a #VogueLayoutIter
 *
 * Gets the layout associated with a #VogueLayoutIter.
 *
 ...
 **/
VogueLayout*
vogue_layout_iter_get_layout (VogueLayoutIter *iter)
{
...
static gboolean
line_is_terminated (VogueLayoutIter *iter)
{
  ...
  if (iter->line_list_link->next)
    {
      VogueLayoutLine *next_line = iter->line_list_link->next->data;
 ...
 */
static gboolean
next_nonempty_line (VogueLayoutIter *iter,
		    gboolean         include_terminators)
{
  ...
  while (TRUE)
    {
      result = vogue_layout_iter_next_line (iter);
      if (!result)
	break;
 ...
 */
static gboolean
next_nonempty_run (VogueLayoutIter *iter,
		    gboolean         include_terminators)
{
  gboolean result;
  ...
  while (TRUE)
    {
      result = vogue_layout_iter_next_run (iter);
      if (!result)
	break;
...
/* Like vogue_layout_next_cluster(), but if @include_terminators
 * is set, includes the fake runs/clusters for empty lines.
 * (But not positions introduced by line wrapping).
 */
static gboolean
next_cluster_internal (VogueLayoutIter *iter,
		       gboolean         include_terminators)
{
  VogueGlyphString *gs;
  int               next_start;
 ...
 **/
gboolean
vogue_layout_iter_next_char (VogueLayoutIter *iter)
{
  const char *text;
...
/**
 * vogue_layout_iter_next_cluster:
 * @iter: a #VogueLayoutIter
 *
 ...
 **/
gboolean
vogue_layout_iter_next_cluster (VogueLayoutIter *iter)
{
  return next_cluster_internal (iter, FALSE);
}

/**
 * vogue_layout_iter_next_run:
 * @iter: a #VogueLayoutIter
 *
 ...
 **/
gboolean
vogue_layout_iter_next_run (VogueLayoutIter *iter)
{
  ...
  if (iter->run == NULL)
    return vogue_layout_iter_next_line (iter);
 ...
 **/
gboolean
vogue_layout_iter_next_line (VogueLayoutIter *iter)
{
  GSList *next_link;
...
/**
 * vogue_layout_iter_get_char_extents:
 * @iter: a #VogueLayoutIter
 ...
 **/
void
vogue_layout_iter_get_char_extents (VogueLayoutIter *iter,
				    VogueRectangle  *logical_rect)
{
  VogueRectangle cluster_rect;
  int            x0, x1;
  ...
  vogue_layout_iter_get_cluster_extents (iter, NULL, &cluster_rect);
...
/**
 * vogue_layout_iter_get_cluster_extents:
 * @iter: a #VogueLayoutIter
 ...
 **/
void
vogue_layout_iter_get_cluster_extents (VogueLayoutIter *iter,
				       VogueRectangle  *ink_rect,
				       VogueRectangle  *logical_rect)
{
  if (ITER_IS_INVALID (iter))
  ...
  if (iter->run == NULL)
    {
       ...
       * same extents
       */
      vogue_layout_iter_get_run_extents (iter, ink_rect, logical_rect);
      return;
    }

  vogue_glyph_string_extents_range (iter->run->glyphs,
				    iter->cluster_start,
...
/**
 * vogue_layout_iter_get_run_extents:
 * @iter: a #VogueLayoutIter
 * @ink_rect: (out) (allow-none): rectangle to fill with ink extents, or %NULL
 * @logical_rect: (out) (allow-none): rectangle to fill with logical extents, or %NULL
 *
 ...
 **/
void
vogue_layout_iter_get_run_extents (VogueLayoutIter *iter,
				   VogueRectangle  *ink_rect,
				   VogueRectangle  *logical_rect)
{
  if (G_UNLIKELY (!ink_rect && !logical_rect))
  ...
  if (iter->run)
    {
      vogue_layout_run_get_extents_and_height (iter->run, ink_rect, logical_rect, NULL);
  ...
  else
    {
      ...
      vogue_layout_iter_get_line_extents (iter, ink_rect, logical_rect);
...
/**
 * vogue_layout_iter_get_line_extents:
 * @iter: a #VogueLayoutIter
 * @ink_rect: (out) (allow-none): rectangle to fill with ink extents, or %NULL
 * @logical_rect: (out) (allow-none): rectangle to fill with logical extents, or %NULL
 *
 ...
 * #VogueLayout).  Thus the extents returned by this function will be
 * the same width/height but not at the same x/y as the extents
 * returned from vogue_layout_line_get_extents().
 *
 **/
void
vogue_layout_iter_get_line_extents (VogueLayoutIter *iter,
				    VogueRectangle  *ink_rect,
				    VogueRectangle  *logical_rect)
{
  const Extents *ext;
...
/**
 * vogue_layout_iter_get_line_yrange:
 * @iter: a #VogueLayoutIter
 * @y0_: (out) (allow-none): start of line, or %NULL
 * @y1_: (out) (allow-none): end of line, or %NULL
 *
 * Divides the vertical space in the #VogueLayout being iterated over
 * between the lines in the layout, and returns the space belonging to
 * the current line.  A line's range includes the line's logical
 * extents, plus half of the spacing above and below the line, if
 * vogue_layout_set_spacing() has been called to set layout spacing.
 * The Y positions are in layout coordinates (origin at top left of the
 * entire layout).
 *
 * Note: Since 1.44, Vogue uses line heights for placing lines,
 * and there may be gaps between the ranges returned by this
 * function.
 */
void
vogue_layout_iter_get_line_yrange (VogueLayoutIter *iter,
				   int             *y0,
				   int             *y1)
{
  const Extents *ext;
...
/**
 * vogue_layout_iter_get_baseline:
 * @iter: a #VogueLayoutIter
 *
 ...
 **/
int
vogue_layout_iter_get_baseline (VogueLayoutIter *iter)
{
  if (ITER_IS_INVALID (iter))
    return 0;
...
/**
 * vogue_layout_iter_get_layout_extents:
 * @iter: a #VogueLayoutIter
 ...
 *                extents, or %NULL
 *
 * Obtains the extents of the #VogueLayout being iterated
 * over. @ink_rect or @logical_rect can be %NULL if you
 * aren't interested in them.
 *
 **/
void
vogue_layout_iter_get_layout_extents  (VogueLayoutIter *iter,
				       VogueRectangle  *ink_rect,
				       VogueRectangle  *logical_rect)
{
  if (ITER_IS_INVALID (iter))
  ...
  vogue_layout_get_extents (iter->layout, ink_rect, logical_rect);
}

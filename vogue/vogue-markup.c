/* Vogue
 * vogue-markup.c: Parse markup into attributed text
 *
...
#include "vogue-utils-internal.h"
 ...
 * italicized words). With the base Vogue interfaces, you could create a
 * #VogueAttrList and apply it to the text; the problem is that you'd
 ...
 * translated. Vogue provides this feature with a small markup language.
 * You can parse a marked-up string into the string text plus a
 * #VogueAttrList using either of vogue_parse_markup() or
 * vogue_markup_parser_new().
 *
 ...
 * ]|
 *
 * Vogue uses #GMarkup to parse this language, which means that XML
 * features such as numeric character entities such as &amp;#169; for
 * © can be used too.
 *
 * The root tag of a marked-up document is &lt;markup&gt;, but
 * vogue_parse_markup()allows you to omit this tag, so you will most
 ...
 *   See vogue_font_description_from_string() for a description of the
 ...
 *
 * rise
 * : Vertical displacement, in Vogue units. Can be negative for
...
{
  g_slist_foreach (ot->attrs, (GFunc) vogue_attribute_destroy, NULL);
...
markup_data_close_tag (MarkupData *md)
{
  ...
  while (tmp_list != NULL)
    {
      VogueAttribute *a = tmp_list->data;
  ...
  if (ot->scale_level_delta != 0)
      ...
      if (ot->has_base_font_size)
	{
	   ...
	   */
	  a = vogue_attr_size_new (scale_factor (ot->scale_level,
						 1.0) *
				   ot->base_font_size);
	}
      else
	{
	   ...
	   * as the base size to be scaled from
	   */
	  a = vogue_attr_scale_new (scale_factor (ot->scale_level,
			...
			GError             **error G_GNUC_UNUSED)
{
  ...
  if (md->accel_marker == 0)
      ...
      if (md->attr_list != NULL && uline_index >= 0)
	{
	  ...
	  attr = vogue_attr_underline_new (PANGO_UNDERLINE_LOW);

	  attr->start_index = uline_index;
	  attr->end_index = uline_index + uline_len;

	  vogue_attr_list_change (md->attr_list, attr);
	}
    }
}
...
static const GMarkupParser vogue_markup_parser = {
...
destroy_markup_data (MarkupData *md)
{
  ...
  g_slist_free_full (md->to_apply, (GDestroyNotify) vogue_attribute_destroy);
  if (md->text)
      g_string_free (md->text, TRUE);

  if (md->attr_list)
    vogue_attr_list_unref (md->attr_list);

  g_slice_free (MarkupData, md);
}

static GMarkupParseContext *
vogue_markup_parser_new_internal (char       accel_marker,
				  GError   **error,
				  gboolean   want_attr_list)
{
   ...
   */
  if (want_attr_list)
    md->attr_list = vogue_attr_list_new ();
  else
    md->attr_list = NULL;
  ...
  context = g_markup_parse_context_new (&vogue_markup_parser,
					0, md,
...
/**
 * vogue_parse_markup:
 * @markup_text: markup to parse (see <link linkend="VogueMarkupFormat">markup format</link>)
 * @length: length of @markup_text, or -1 if nul-terminated
 * @accel_marker: character that precedes an accelerator, or 0 for none
 * @attr_list: (out) (allow-none): address of return location for a #VogueAttrList, or %NULL
 ...
 * <link linkend="VogueMarkupFormat">markup format</link>) to create
 ...
 *
 * To parse a stream of vogue markup incrementally, use vogue_markup_parser_new().
 *
 ...
 **/
gboolean
vogue_parse_markup (const char                 *markup_text,
		    int                         length,
		    gunichar                    accel_marker,
		    VogueAttrList             **attr_list,
		    char                      **text,
		    ...
		    GError                    **error)
{
  ...
  context = vogue_markup_parser_new_internal (accel_marker,
                                              error,
                                              (attr_list != NULL));
  if (context == NULL)
    goto out;
  ...
  if (!vogue_markup_parser_finish (context,
                                   attr_list,
                                   text,
                                   accel_char,
                                   error))
 ...
 * to the parser, use vogue_markup_parser_finish() to get the data out
 * of it, and then use g_markup_parse_context_free() to free it.
 *
 * This function is designed for applications that read vogue markup
 * from streams. To simply parse a string containing vogue markup,
 * the simpler vogue_parse_markup() API is recommended instead.
 *
 ...
 **/
GMarkupParseContext *
vogue_markup_parser_new (gunichar               accel_marker)
{
  GError *error = NULL;
  GMarkupParseContext *context;
  context = vogue_markup_parser_new_internal (accel_marker, &error, TRUE);
...
/**
 * vogue_markup_parser_finish:
 * @context: A valid parse context that was returned from vogue_markup_parser_new()
 * @attr_list: (out) (allow-none): address of return location for a #VogueAttrList, or %NULL
 * @text: (out) (allow-none): address of return location for text with tags stripped, or %NULL
 * @accel_char: (out) (allow-none): address of return location for accelerator char, or %NULL
 * @error: address of return location for errors, or %NULL
 *
 * After feeding a vogue markup parser some data with g_markup_parse_context_parse(),
 * use this function to get the list of vogue attributes and text out of the
 ...
 */
gboolean
vogue_markup_parser_finish (GMarkupParseContext   *context,
                            VogueAttrList        **attr_list,
                            char                 **text,
                            gunichar              *accel_char,
                            GError               **error)
{
  ...
  if (md->attr_list)
      ...
      while (tmp_list != NULL)
	{
	  ...
	  vogue_attr_list_insert (md->attr_list, attr);
...
{
  CHECK_NO_ATTRS("b");
  add_attribute (tag, vogue_attr_weight_new (PANGO_WEIGHT_BOLD));
  return TRUE;
}
		     ...
		     const char            *size)
{
  ...
  add_attribute (tag, vogue_attr_scale_new (factor));
  if (tag)
		...
		GError **error)
{
  ...
  if (!_vogue_scan_int (&end, val) || *end != '\0')
    {
      g_set_error (error,
		   G_MARKUP_ERROR,
...
span_parse_color (const char *attr_name,
		  ...
		  GError **error)
{
  if (!_vogue_color_parse_with_alpha (color, alpha, attr_val))
    {
      g_set_error (error,
		   G_MARKUP_ERROR,
		   G_MARKUP_ERROR_INVALID_CONTENT,
                  ...
                  GError     **error)
{
  ...
  if (_vogue_scan_int (&end, &int_val))
    {
		 ...
		 GError **error)
{
  ...
  if (!_vogue_parse_enum (type, attr_val, val, FALSE, &possible_values))
    {
      g_set_error (error,
		   G_MARKUP_ERROR,
		   G_MARKUP_ERROR_INVALID_CONTENT,
                  ...
                  GError     **error)
{
  ...
  if (!vogue_parse_flags (type, attr_val, val, &possible_values))
    {
      g_set_error (error,
                   G_MARKUP_ERROR,
                   G_MARKUP_ERROR_INVALID_CONTENT,
...
#define CHECK_ATTRIBUTE(var) CHECK_ATTRIBUTE2 (var, G_STRINGIFY (var))
  ...
  if (G_UNLIKELY (desc))
    {
      VogueFontDescription *parsed;

      parsed = vogue_font_description_from_string (desc);
      if (parsed)
	{
	  add_attribute (tag, vogue_attr_font_desc_new (parsed));
	  if (tag)
	    open_tag_set_absolute_font_size (tag, vogue_font_description_get_size (parsed));
	  vogue_font_description_free (parsed);
	}
    }

  if (G_UNLIKELY (family))
    {
      add_attribute (tag, vogue_attr_family_new (family));
    }
  ...
  if (G_UNLIKELY (size))
      ...
      if (g_ascii_isdigit (*size))
	{
	  ...
	  if ((end = size, !_vogue_scan_int (&end, &n)) || *end != '\0' || n < 0)
	    {
	      g_set_error (error,
			   G_MARKUP_ERROR,
			   G_MARKUP_ERROR_INVALID_CONTENT,
	  ...
	  add_attribute (tag, vogue_attr_size_new (n));
	  if (tag)
  ...
  if (G_UNLIKELY (style))
    {
      VogueStyle vogue_style;

      if (vogue_parse_style (style, &vogue_style, FALSE))
	add_attribute (tag, vogue_attr_style_new (vogue_style));
      else
	{
	  g_set_error (error,
		       G_MARKUP_ERROR,
  ...
  if (G_UNLIKELY (weight))
    {
      ...
      if (vogue_parse_weight (weight, &vogue_weight, FALSE))
	add_attribute (tag,
		       vogue_attr_weight_new (vogue_weight));
      else
	{
	  g_set_error (error,
		       G_MARKUP_ERROR,
  ...
  if (G_UNLIKELY (variant))
    {
      VogueVariant vogue_variant;

      if (vogue_parse_variant (variant, &vogue_variant, FALSE))
	add_attribute (tag, vogue_attr_variant_new (vogue_variant));
      else
	{
	  g_set_error (error,
		       G_MARKUP_ERROR,
  ...
  if (G_UNLIKELY (stretch))
    {
      VogueStretch vogue_stretch;

      if (vogue_parse_stretch (stretch, &vogue_stretch, FALSE))
	add_attribute (tag, vogue_attr_stretch_new (vogue_stretch));
      else
	{
	  g_set_error (error,
		       G_MARKUP_ERROR,
  ...
  if (G_UNLIKELY (foreground))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_foreground_new (color.red, color.green, color.blue));
      if (alpha != 0)
        add_attribute (tag, vogue_attr_foreground_alpha_new (alpha));
    }
  ...
  if (G_UNLIKELY (background))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_background_new (color.red, color.green, color.blue));
      if (alpha != 0)
        add_attribute (tag, vogue_attr_background_alpha_new (alpha));
    }
  ...
  if (G_UNLIKELY (alpha))
    {
      ...
      add_attribute (tag, vogue_attr_foreground_alpha_new (val));
    }
  ...
  if (G_UNLIKELY (background_alpha))
    {
      ...
      add_attribute (tag, vogue_attr_background_alpha_new (val));
    }

  if (G_UNLIKELY (underline))
    {
      VogueUnderline ul = PANGO_UNDERLINE_NONE;

      if (!span_parse_enum ("underline", underline, PANGO_TYPE_UNDERLINE, (int*)(void*)&ul, line_number, error))
	goto error;

      add_attribute (tag, vogue_attr_underline_new (ul));
    }
  ...
  if (G_UNLIKELY (underline_color))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_underline_color_new (color.red, color.green, color.blue));
    }

  if (G_UNLIKELY (gravity))
    {
      VogueGravity gr = PANGO_GRAVITY_SOUTH;
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_gravity_new (gr));
    }

  if (G_UNLIKELY (gravity_hint))
    {
      VogueGravityHint hint = PANGO_GRAVITY_HINT_NATURAL;

      if (!span_parse_enum ("gravity_hint", gravity_hint, PANGO_TYPE_GRAVITY_HINT, (int*)(void*)&hint, line_number, error))
	goto error;

      add_attribute (tag, vogue_attr_gravity_hint_new (hint));
    }
  ...
  if (G_UNLIKELY (strikethrough))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_strikethrough_new (b));
    }
  ...
  if (G_UNLIKELY (strikethrough_color))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_strikethrough_color_new (color.red, color.green, color.blue));
    }
  ...
  if (G_UNLIKELY (fallback))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_fallback_new (b));
    }

  if (G_UNLIKELY (show))
    {
      VogueShowFlags flags;

      if (!span_parse_flags ("show", show, PANGO_TYPE_SHOW_FLAGS, (int*)(void*)&flags, line_number, error))
	goto error;

      add_attribute (tag, vogue_attr_show_new (flags));
    }
  ...
  if (G_UNLIKELY (rise))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_rise_new (n));
    }
  ...
  if (G_UNLIKELY (letter_spacing))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_letter_spacing_new (n));
    }

  if (G_UNLIKELY (lang))
    {
      add_attribute (tag,
		     vogue_attr_language_new (vogue_language_from_string (lang)));
    }

  if (G_UNLIKELY (font_features))
    {
      add_attribute (tag, vogue_attr_font_features_new (font_features));
    }
  ...
  if (G_UNLIKELY (allow_breaks))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_allow_breaks_new (b));
    }
  ...
  if (G_UNLIKELY (insert_hyphens))
	...
	goto error;
      ...
      add_attribute (tag, vogue_attr_insert_hyphens_new (b));
    }
...
{
  CHECK_NO_ATTRS("i");
  add_attribute (tag, vogue_attr_style_new (PANGO_STYLE_ITALIC));
...
{
  CHECK_NO_ATTRS("s");
  add_attribute (tag, vogue_attr_strikethrough_new (TRUE));
		     ...
		     GError               **error)
{
  ...
  add_attribute (tag, vogue_attr_rise_new (-SUPERSUB_RISE));
		     ...
		     GError               **error)
{
  ...
  add_attribute (tag, vogue_attr_family_new ("Monospace"));
...
{
  CHECK_NO_ATTRS("u");
  add_attribute (tag, vogue_attr_underline_new (PANGO_UNDERLINE_SINGLE));

/* Vogue
 * vogue-impl-utils.h: Macros for get_type() functions
...
void _vogue_shape_shape (const char       *text,
			 unsigned int      n_chars,
			 VogueRectangle   *shape_ink,
			 VogueRectangle   *shape_logical,
			 VogueGlyphString *glyphs);

void _vogue_shape_get_extents (gint              n_chars,
			       VogueRectangle   *shape_ink,
			       VogueRectangle   *shape_logical,
...
vogue_unichar_width (gunichar c)
{
...
vogue_utf8_strwidth (const gchar *p)
{
  glong len = 0;
  ...
  while (*p)
    {
      len += vogue_unichar_width (g_utf8_get_char (p));
 ...
 * Wrap it here. */
static G_GNUC_UNUSED glong
vogue_utf8_strlen (const gchar *p, gssize max)
{
  glong len = 0;
  const gchar *start = p;
...
vogue_glyph_string_reverse_range (VogueGlyphString *glyphs,
				  int start, int end)
{
  int i, j;
...
vogue_get_ignorable (gunichar ch)
{
  int i;
...
static inline G_GNUC_UNUSED const char *
vogue_get_ignorable_size (gunichar  ch,
                          int      *rows,
                          int      *cols)
{
  const char *nick;

...
struct _RunInfo
{
  ...
  int width;			/* Width of run in Vogue units */
};

/* Iterator to a position within the ellipsized line */
struct _LineIter
{
  VogueGlyphItemIter run_iter;
  int run_index;
};
...
{
  VogueLayout *layout;		/* Layout being ellipsized */
  VogueAttrList *attrs;		/* Attributes used for itemization/shaping */

  RunInfo *run_info;		/* Array of information about each run */
  int n_runs;

  int total_width;		/* Original width of line in Vogue units */
  int gap_center;		/* Goal for center of gap */

  VogueGlyphItem *ellipsis_run;	/* Run created to hold ellipsis */
  int ellipsis_width;		/* Width of ellipsis, in Vogue units */
  ...
  VogueAttrIterator *line_start_attr; /* Cached VogueAttrIterator for the start of the run */

  LineIter gap_start_iter;	/* Iteratator pointig to the first cluster in gap */
  int gap_start_x;		/* x position of start of gap, in Vogue units */
  VogueAttrIterator *gap_start_attr; /* Attribute iterator pointing to a range containing
				      * the first character in gap */

  LineIter gap_end_iter;	/* Iterator pointing to last cluster in gap */
  int gap_end_x;		/* x position of end of gap, in Vogue units */
};
	    ...
	    VogueAttrList   *attrs)
{
  ...
  if (attrs)
    state->attrs = vogue_attr_list_ref (attrs);
  else
    state->attrs = vogue_attr_list_new ();

  state->n_runs = g_slist_length (line->runs);
  state->run_info = g_new (RunInfo, state->n_runs);

  start_offset = vogue_utf8_strlen (line->layout->text,
				line->start_index);
  ...
  for (l = line->runs, i = 0; l; l = l->next, i++)
    {
      ...
      int width = vogue_glyph_string_get_width (run->glyphs);
...
{
  vogue_attr_list_unref (state->attrs);
  if (state->line_start_attr)
    vogue_attr_iterator_destroy (state->line_start_attr);
  if (state->gap_start_attr)
    vogue_attr_iterator_destroy (state->gap_start_attr);
...
{
  VogueGlyphItemIter *run_iter = &iter->run_iter;
  VogueGlyphString *glyphs = run_iter->glyph_item->glyphs;
  int width = 0;
  int i;
...
line_iter_next_cluster (EllipsizeState *state,
			LineIter       *iter)
{
  if (!vogue_glyph_item_iter_next_cluster (&iter->run_iter))
    {
	...
	{
	  iter->run_index++;
	  vogue_glyph_item_iter_init_start (&iter->run_iter,
...
line_iter_prev_cluster (EllipsizeState *state,
			LineIter       *iter)
{
  if (!vogue_glyph_item_iter_prev_cluster (&iter->run_iter))
    {
      if (iter->run_index == 0)
	return FALSE;
      else
	{
	  iter->run_index--;
	  vogue_glyph_item_iter_init_end (&iter->run_iter,
					  state->run_info[iter->run_index].run,
					  state->layout->text);
	}
    }
 ...
 * This should be an addition to #VogueGlyphVisAttr
 *
 */
	      ...
	      VogueAttrList  *attrs)
{
  ...
  items = vogue_itemize (state->layout->context, text, 0, strlen (text), attrs, NULL);
...
{
  VogueAttrList *attrs = vogue_attr_list_new ();
  GSList *run_attrs;
  VogueItem *item;
  VogueGlyphString *glyphs;
  GSList *l;
  VogueAttribute *fallback;
  ...
  if (!state->ellipsis_run)
    {
      state->ellipsis_run = g_slice_new (VogueGlyphItem);
      state->ellipsis_run->glyphs = vogue_glyph_string_new ();
      state->ellipsis_run->item = NULL;
    }

  if (state->ellipsis_run->item)
    {
      vogue_item_free (state->ellipsis_run->item);
      state->ellipsis_run->item = NULL;
    }

  /* Create an attribute list
   */
  run_attrs = vogue_attr_iterator_get_attrs (state->gap_start_attr);
  ...
  fallback = vogue_attr_fallback_new (FALSE);
  fallback->start_index = 0;
  fallback->end_index = G_MAXINT;
  vogue_attr_list_insert (attrs, fallback);
  ...
  if (!item->analysis.font ||
      !vogue_font_has_char (item->analysis.font,
                            g_utf8_get_char (ellipsis_text)))
    {
      vogue_item_free (item);

      /* Modify the fallback iter while it is inside the VogueAttrList; Don't try this at home
       */
      ((VogueAttrInt *)fallback)->value = TRUE;
  ...
  vogue_shape (ellipsis_text, strlen (ellipsis_text),
...
/* Helper function to advance a VogueAttrIterator to a particular
 * byte index.
 */
static void
advance_iterator_to (VogueAttrIterator *iter,
		     int                new_index)
{
  int start, end;
...
update_ellipsis_shape (EllipsizeState *state)
{
  ...
  /* Unfortunately, we can only advance VogueAttrIterator forward; so each
  ...
  if (!state->line_start_attr)
    {
      state->line_start_attr = vogue_attr_list_get_iterator (state->attrs);
  ...
  if (state->gap_start_attr)
    {
      ...
      vogue_attr_iterator_range (state->gap_start_attr, &start, &end);
  ...
  if (!state->gap_start_attr)
    {
      state->gap_start_attr = vogue_attr_iterator_copy (state->line_start_attr);
...
find_initial_span (EllipsizeState *state)
{
  ...
  for (have_cluster = vogue_glyph_item_iter_init_start (run_iter, glyph_item, state->layout->text);
       have_cluster;
       have_cluster = vogue_glyph_item_iter_next_cluster (run_iter))
    {
...
{
  VogueGlyphString *glyphs = state->ellipsis_run->glyphs;
  VogueItem *item = state->ellipsis_run->item;
  int level;
  int i;
  ...
  item->num_chars = vogue_utf8_strlen (state->layout->text + item->offset, item->length);
...
{
  VogueGlyphItem *partial_start_run = NULL;
  VogueGlyphItem *partial_end_run = NULL;
  GSList *result = NULL;
  RunInfo *run_info;
  VogueGlyphItemIter *run_iter;
  int i;
  ...
  if (run_iter->end_char != run_info->run->item->num_chars)
    {
      ...
      run_info->run = vogue_glyph_item_split (run_info->run, state->layout->text,
  ...
  if (run_iter->start_char != 0)
    {
      partial_start_run = vogue_glyph_item_split (run_info->run, state->layout->text,
   ...
   */
  for (i = state->gap_start_iter.run_index; i <= state->gap_end_iter.run_index; i++)
    vogue_glyph_item_free (state->run_info[i].run);
...
/**
 * _vogue_layout_line_ellipsize:
 * @line: a #VogueLayoutLine
 * @attrs: Attributes being used for itemization/shaping
 *
 * Given a #VogueLayoutLine with the runs still in logical order, ellipsize
 ...
 **/
gboolean
_vogue_layout_line_ellipsize (VogueLayoutLine *line,
			      VogueAttrList   *attrs,
			      int              goal_width)
{

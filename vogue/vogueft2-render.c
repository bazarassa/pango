/* Vogue
 * vogueft2-render.c: Rendering routines to FT_Bitmap objects
 *
...
/**
 * SECTION:vogue-renderer
 * @short_description:Rendering driver base class
 * @title:VogueRenderer
 *
 * #VogueRenderer is a base class that contains the necessary logic for
 * rendering a #VogueLayout or #VogueLayoutLine. By subclassing
 * #VogueRenderer and overriding operations such as @draw_glyphs and
...
typedef struct _VogueFT2RendererClass VogueFT2RendererClass;

#define PANGO_FT2_RENDERER_CLASS(klass)    (G_TYPE_CHECK_CLASS_CAST ((klass), PANGO_TYPE_FT2_RENDERER, VogueFT2RendererClass))
#define PANGO_IS_FT2_RENDERER_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((klass), PANGO_TYPE_FT2_RENDERER))
#define PANGO_FT2_RENDERER_GET_CLASS(obj)  (G_TYPE_INSTANCE_GET_CLASS ((obj), PANGO_TYPE_FT2_RENDERER, VogueFT2RendererClass))

struct _VogueFT2Renderer
{
...
struct _VogueFT2RendererClass
{
  VogueRendererClass parent_class;
};

static void vogue_ft2_renderer_draw_glyph     (VogueRenderer    *renderer,
					       VogueFont        *font,
					       VogueGlyph        glyph,
					       double            x,
					       double            y);
static void vogue_ft2_renderer_draw_trapezoid (VogueRenderer    *renderer,
...
G_DEFINE_TYPE (VogueFT2Renderer, vogue_ft2_renderer, PANGO_TYPE_RENDERER)

static void
vogue_ft2_renderer_init (VogueFT2Renderer *renderer G_GNUC_UNUSED)
{
}

static void
vogue_ft2_renderer_class_init (VogueFT2RendererClass *klass)
{
  VogueRendererClass *renderer_class = PANGO_RENDERER_CLASS (klass);

  renderer_class->draw_glyph = vogue_ft2_renderer_draw_glyph;
  renderer_class->draw_trapezoid = vogue_ft2_renderer_draw_trapezoid;
}

static void
vogue_ft2_renderer_set_bitmap (VogueFT2Renderer *renderer,
			      FT_Bitmap         *bitmap)
{
...
typedef struct
{
  ...
  int bitmap_left;
  int bitmap_top;
} VogueFT2RenderedGlyph;

static void
vogue_ft2_free_rendered_glyph (VogueFT2RenderedGlyph *rendered)
{
  g_free (rendered->bitmap.buffer);
  g_slice_free (VogueFT2RenderedGlyph, rendered);
}

static VogueFT2RenderedGlyph *
vogue_ft2_font_render_box_glyph (int      width,
				 int      height,
				 int      top,
				 gboolean invalid)
{
  VogueFT2RenderedGlyph *box;
  ...
  box = g_slice_new (VogueFT2RenderedGlyph);
  ...
  if (G_UNLIKELY (!box->bitmap.buffer)) {
    g_slice_free (VogueFT2RenderedGlyph, box);
    return NULL;
  }
...
static VogueFT2RenderedGlyph *
vogue_ft2_font_render_glyph (VogueFont *font,
			     VogueGlyph glyph_index)
{
  FT_Face face;
  ...
  if (glyph_index & PANGO_GLYPH_UNKNOWN_FLAG)
	...
	goto generic_box;
      ...
      metrics = vogue_font_get_metrics (font, NULL);
      if (!metrics)
	goto generic_box;

      box = vogue_ft2_font_render_box_glyph (PANGO_PIXELS (metrics->approximate_char_width),
  ...
  face = vogue_ft2_font_get_face (font);

  if (face)
    {
      VogueFT2RenderedGlyph *rendered;
      VogueFT2Font *ft2font = (VogueFT2Font *) font;

      rendered = g_slice_new (VogueFT2RenderedGlyph);
    ...
    {
generic_box:
      return  vogue_ft2_font_render_box_glyph (PANGO_UNKNOWN_GLYPH_WIDTH,
...
static void
vogue_ft2_renderer_draw_glyph (VogueRenderer *renderer,
			       VogueFont     *font,
			       VogueGlyph     glyph,
			       double         x,
			       double         y)
{
  ...
  VogueFT2RenderedGlyph *rendered_glyph;
  ...
  rendered_glyph = _vogue_ft2_font_get_cache_glyph_data (font, glyph);
  add_glyph_to_cache = FALSE;
  if (rendered_glyph == NULL)
    {
      rendered_glyph = vogue_ft2_font_render_glyph (font, glyph);
      if (rendered_glyph == NULL)
        return;
  ...
  if (add_glyph_to_cache)
    {
      _vogue_ft2_font_set_glyph_cache_destroy (font,
					       (GDestroyNotify) vogue_ft2_free_rendered_glyph);
      _vogue_ft2_font_set_cache_glyph_data (font,
...
static void
draw_simple_trap (VogueRenderer *renderer,
		  Position      *t,
		  Position      *b)
{
  FT_Bitmap *bitmap = PANGO_FT2_RENDERER (renderer)->bitmap;
 ...
 */
static void
vogue_ft2_renderer_draw_trapezoid (VogueRenderer   *renderer,
				   VogueRenderPart  part G_GNUC_UNUSED,
				   double           y1,
				   double           x11,
				   double           x21,
				   double           y2,
...
/**
 * vogue_ft2_render_layout_subpixel:
 ...
 *
 * Render a #VogueLayout onto a FreeType2 bitmap, with he
 * location specified in fixed-point Vogue units rather than
 ...
 */
void
vogue_ft2_render_layout_subpixel (FT_Bitmap   *bitmap,
				  VogueLayout *layout,
				  int          x,
				  int          y)
{
  ...
  renderer = _vogue_ft2_font_map_get_renderer (PANGO_FT2_FONT_MAP (fontmap));

  vogue_ft2_renderer_set_bitmap (PANGO_FT2_RENDERER (renderer), bitmap);

  vogue_renderer_draw_layout (renderer, layout, x, y);
}

/**
 * vogue_ft2_render_layout:
 * @bitmap:    a <type>FT_Bitmap</type> to render the layout onto
 * @layout:    a #VogueLayout
 * @x:         the X position of the left of the layout (in pixels)
 * @y:         the Y position of the top of the layout (in pixels)
 *
 * Render a #VogueLayout onto a FreeType2 bitmap
 */
void
vogue_ft2_render_layout (FT_Bitmap   *bitmap,
			 VogueLayout *layout,
			 int          x,
			 int          y)
{
  vogue_ft2_render_layout_subpixel (bitmap, layout, x * PANGO_SCALE, y * PANGO_SCALE);
}

/**
 * vogue_ft2_render_layout_line_subpixel:
 ...
 *
 * Render a #VogueLayoutLine onto a FreeType2 bitmap, with he
 * location specified in fixed-point Vogue units rather than
 * pixels. (Using this will avoid extra inaccuracies from
 * rounding to integer pixels multiple times, even if the
 * final glyph positions are integers.)
 ...
 */
void
vogue_ft2_render_layout_line_subpixel (FT_Bitmap       *bitmap,
...
/**
 * vogue_ft2_render_layout_line:
 * @bitmap:    a <type>FT_Bitmap</type> to render the line onto
 * @line:      a #VogueLayoutLine
 * @x:         the x position of start of string (in pixels)
 * @y:         the y position of baseline (in pixels)
 *
 * Render a #VogueLayoutLine onto a FreeType2 bitmap
 */
void
vogue_ft2_render_layout_line (FT_Bitmap       *bitmap,
			      VogueLayoutLine *line,
			      int              x,
			      int              y)
{
  vogue_ft2_render_layout_line_subpixel (bitmap, line, x * PANGO_SCALE, y * PANGO_SCALE);
}

/**
 * vogue_ft2_render_transformed:
 ...
 *
 * Renders a #VogueGlyphString onto a FreeType2 bitmap, possibly
 ...
 **/
void
vogue_ft2_render_transformed (FT_Bitmap         *bitmap,
 ...
 *
 * Renders a #VogueGlyphString onto a FreeType2 bitmap.
 **/
void
vogue_ft2_render (FT_Bitmap        *bitmap,
		  VogueFont        *font,
		  VogueGlyphString *glyphs,
		  int               x,
		  int               y)
{
  vogue_ft2_render_transformed (bitmap, NULL, font, glyphs, x * PANGO_SCALE, y * PANGO_SCALE);
}

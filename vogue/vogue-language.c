/* Vogue
 * vogue-language.c: Language handling routines
 *
...
/* We embed a private struct right *before* a where a VogueLanguage *
 * points to.
 */
...
} VogueLanguagePrivate;

#define PANGO_LANGUAGE_PRIVATE_MAGIC 0x0BE4DAD0

static void
vogue_language_private_init (VogueLanguagePrivate *priv)
{
...
static VogueLanguagePrivate * vogue_language_get_private (VogueLanguage *language) G_GNUC_CONST;

static VogueLanguagePrivate *
vogue_language_get_private (VogueLanguage *language)
{
  VogueLanguagePrivate *priv;

  if (!language)
    return NULL;

  priv = (VogueLanguagePrivate *) ((char *)language - sizeof (VogueLanguagePrivate));

  if (G_UNLIKELY (priv->magic != PANGO_LANGUAGE_PRIVATE_MAGIC))
    {
      g_critical ("Invalid VogueLanguage.  Did you pass in a straight string instead of calling vogue_language_from_string()?");
      return NULL;
    }
...
vogue_language_copy (VogueLanguage *language)
{
  return language; /* language tags are const */
}

static void
vogue_language_free (VogueLanguage *language G_GNUC_UNUSED)
{
  return; /* nothing */
}
 ...
 * VogueLanguage:
 *
 * The #VogueLanguage structure is used to
 * represent a language.
 *
 * #VogueLanguage pointers can be efficiently
 * copied and compared with each other.
 */
G_DEFINE_BOXED_TYPE (VogueLanguage, vogue_language,
                     vogue_language_copy,
                     vogue_language_free);
...
_vogue_get_lc_ctype (void)
{
   ...
   * kicks, do obey LC_ALL, LC_CTYPE and LANG in Vogue. (This also makes
   * it easier to test GTK and Vogue in various default languages, you
 ...
 *
 * Returns the #VogueLanguage for the current locale of the process.
 ...
 *               #VogueLanguage, must not be freed.
 *
 * Since: 1.16
 **/
VogueLanguage *
vogue_language_get_default (void)
{
  static VogueLanguage *result = NULL; /* MT-safe */
  ...
  if (g_once_init_enter (&result))
    {
      ...
      lc_ctype = _vogue_get_lc_ctype ();
      lang = vogue_language_from_string (lc_ctype);
      g_free (lc_ctype);
...
/**
 * vogue_language_from_string:
 * @language: (allow-none): a string representing a language tag, or %NULL
 *
 * Take a RFC-3066 format language tag as a string and convert it to a
 * #VogueLanguage pointer that can be efficiently copied (copy the
 ...
 * than letters and '-'.
 *
 * Use vogue_language_get_default() if you want to get the #VogueLanguage for
 * the current locale of the process.
 *
 * Return value: (transfer none) (nullable): an opaque pointer to a
 *               #VogueLanguage structure, or %NULL if @language was
 *               %NULL.  The returned pointer will be valid forever
 *               after, and should not be freed.
 **/
VogueLanguage *
vogue_language_from_string (const char *language)
{
  ...
  len = strlen (language);
  result = g_malloc0 (sizeof (VogueLanguagePrivate) + len + 1);
  g_assert (result);

  priv = (VogueLanguagePrivate *) result;
  result += sizeof (*priv);

  vogue_language_private_init (priv);
...
out:
  ...
  return (VogueLanguage *)result;
}

/**
 * vogue_language_to_string:
 * @language: a language tag.
 *
 * Gets the RFC-3066 format string representing the given language tag. 
 *
 * Returns: a string representing the language tag.  This is owned by
 *          Vogue and should not be freed.
 */
const char *
(vogue_language_to_string) (VogueLanguage *language)
{
  return vogue_language_to_string (language);
}

/**
 * vogue_language_matches:
 * @language: (nullable): a language tag (see vogue_language_from_string()),
 *            %NULL is allowed and matches nothing but '*'
 * @range_list: a list of language ranges, separated by ';', ':',
 *   ',', or space characters.
 *   Each element must either be '*', or a RFC 3066 language range
 *   canonicalized as by vogue_language_from_string()
 *
 ...
 **/
gboolean
vogue_language_matches (VogueLanguage *language,
			const char    *range_list)
{
  const char *lang_str = vogue_language_to_string (language);
...
find_best_lang_match (VogueLanguage *language,
		      ...
		      guint          record_size)
{
  ...
  lang_str = vogue_language_to_string (language);
  ...
  while (start <= record &&
    ...
    {
      if (vogue_language_matches (language, record))
        return record;
...
static gconstpointer
find_best_lang_match_cached (VogueLanguage *language,
			     gconstpointer *cache,
			     gconstpointer  records,
			     guint          num_records,
			     guint          record_size)
...
#define FIND_BEST_LANG_MATCH_CACHED(language, cache_key, records) \
	find_best_lang_match_cached ((language), \
				     vogue_language_get_private (language) ? \
				       &(vogue_language_get_private (language)->cache_key) : NULL, \
				     records, \
...
#include "vogue-language-sample-table.h"
#undef LANGUAGE
};
...
/**
 * vogue_language_get_sample_string:
 ...
 * vogue_language_get_default() is used.
 *
 * If Vogue does not have a sample string for @language, the classic
 * "The quick brown fox..." is returned.  This can be detected by
 * comparing the returned pointer value to that returned for (non-existent)
 * language code "xx".  That is, compare to:
 * <informalexample><programlisting>
 * vogue_language_get_sample_string (vogue_language_from_string ("xx"))
 * </programlisting></informalexample>
 *
 * Return value: the sample string. This value is owned by Vogue
 *   and should not be freed.
 **/
const char *
vogue_language_get_sample_string (VogueLanguage *language)
{
  const LangInfo *lang_info;

  if (!language)
    language = vogue_language_get_default ();
...
#include "vogue-script-lang-table.h"

/**
 * vogue_language_get_scripts:
 * @language: (allow-none): a #VogueLanguage, or %NULL
 ...
 *
 * The vogue_language_includes_script() function uses this function
 * internally.
 *
 * Note: while the return value is declared as VogueScript, the
 * returned values are from the GUnicodeScript enumeration, which
 * may have more values. Callers need to handle unknown values.
 *
 * Return value: (array length=num_scripts) (nullable): An array of
 * #VogueScript values, with the number of entries in the array stored
 * in @num_scripts, or %NULL if Vogue does not have any information
 * about this particular language tag (also the case if @language is
 * %NULL).  The returned array is owned by Vogue and should not be
 * modified or freed.
 
 * Since: 1.22
 **/
const VogueScript *
vogue_language_get_scripts (VogueLanguage *language,
			    int           *num_scripts)
{
  const VogueScriptForLang *script_for_lang;
  unsigned int j;

  script_for_lang = FIND_BEST_LANG_MATCH_CACHED (language,
						 script_for_lang,
						 vogue_script_for_lang);
  ...
  return (const VogueScript *) script_for_lang->scripts;
}

/**
 * vogue_language_includes_script:
 * @language: (nullable): a #VogueLanguage, or %NULL
 ...
 * if nothing is known about the language tag @language,
 * %TRUE will be returned, since, as far as Vogue knows,
 * @script might be used to write @language.
 *
 * This routine is used in Vogue's itemization process when
 * determining if a supplied language tag is relevant to
 * a particular section of text. It probably is not useful for
 * applications in most circumstances.
 *
 * This function uses vogue_language_get_scripts() internally.
 *
 ...
 **/
gboolean
vogue_language_includes_script (VogueLanguage *language,
				VogueScript    script)
{
  const VogueScript *scripts;
  int num_scripts, j;

/* copied from the one in vogue-script.c */
...
#undef REAL_SCRIPT
  ...
  scripts = vogue_language_get_scripts (language, &num_scripts);
  if (!scripts)
    return TRUE;
...
parse_default_languages (void)
{
  ...
  langs = g_array_new (TRUE, FALSE, sizeof (VogueLanguage *));
  ...
  while (!done)
      ...
      if (!end)
	{
      ...
      if (p != end && !(p + 1 == end && *p == 'C'))
        {
	  VogueLanguage *l = vogue_language_from_string (p);
  ...
  return (VogueLanguage **) g_array_free (langs, FALSE);
}

static VogueLanguage *
_vogue_script_get_default_language (VogueScript script)
{
  G_LOCK_DEFINE_STATIC (languages);
  static gboolean initialized = FALSE; /* MT-safe */
  static VogueLanguage * const * languages = NULL; /* MT-safe */
  static GHashTable *hash = NULL; /* MT-safe */
  VogueLanguage *result, * const * p;
  ...
  for (p = languages; *p; p++)
    if (vogue_language_includes_script (*p, script))
      break;
  result = *p;
...
/**
 * vogue_script_get_sample_language:
 * @script: a #VogueScript
 *
 ...
 * will return the first language in the parsed list that Vogue
 * believes may use @script for writing.  This last predicate
 * is tested using vogue_language_includes_script().  This can
 * be used to control Vogue's font selection for non-primary
 * languages.  For example, a PANGO_LANGUAGE enviroment variable
 * set to "en:fa" makes Vogue choose fonts suitable for Persian (fa) 
 * instead of Arabic (ar) when a segment of Arabic text is found
 * in an otherwise non-Arabic text.  The same trick can be used to
 * choose a default language for %PANGO_SCRIPT_HAN when setting
 * context language is not feasible.
 *
 * Return value: (nullable): a #VogueLanguage that is representative
 * of the script, or %NULL if no such language exists.
 *
 * Since: 1.4
 **/
VogueLanguage *
vogue_script_get_sample_language (VogueScript script)
{
  /* Note that in the following, we want
   * vogue_language_includes_script() for the sample language
  ...
  result = _vogue_script_get_default_language (script);
  if (result)
    return result;
  ...
  if (!sample_language[0])
    ...
    return vogue_language_from_string (sample_language);
}

/* Vogue
 * vogue-bidi-type.c: Bidirectional Character Types
 *
 ...
 *
 * Vogue supports bidirectional text (like Arabic and Hebrew) automatically.
 * Some applications however, need some help to correctly handle bidirectional text.
 *
 * The #VogueDirection type can be used with vogue_context_set_base_dir() to
 * instruct Vogue about direction of text, though in most cases Vogue detects
 * that correctly and automatically.  The rest of the facilities in this section
 * are used internally by Vogue already, and are provided to help applications
...
#include "vogue-bidi-type.h"
#include "vogue-utils.h"
 ...
 */
VogueBidiType
vogue_bidi_type_for_unichar (gunichar ch)
{
...
/**
 * vogue_log2vis_get_embedding_levels:
 ...
 */
guint8 *
vogue_log2vis_get_embedding_levels (const gchar    *text,
				    int             length,
 ...
 * vogue_bidi_type_for_unichar() can be used instead.
 *
 * Return value: the direction of the character.
 */
VogueDirection
vogue_unichar_direction (gunichar ch)
{
  FriBidiCharType fribidi_ch_type;
 ...
 **/
gboolean
vogue_get_mirror_char (gunichar        ch,

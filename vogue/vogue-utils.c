...
/**
 * SECTION:vogue-version
 * @short_description:Tools for checking Vogue version at compile- and run-time.
 * @title:Version Checking
 *
 * The capital-letter macros defined here can be used to check the version of Vogue
 * at compile-time, and to <firstterm>encode</firstterm> Vogue versions into integers.
 *
 * The functions can be used to check the version of the linked Vogue library at run-time.
 */
/**
 * SECTION:utils
 ...
 *
 * The functions and utilities in this section are mostly used from Vogue
 ...
 * it returns the encoded version of Vogue available at run-time,
 * as opposed to the version available at compile-time.
 *
 * A version number can be encoded into an integer using
 * PANGO_VERSION_ENCODE().
 *
 * Returns: The encoded version of Vogue library
 ...
 * it returns the version of Vogue available at run-time, as opposed to
 * the version available at compile-time.
 *
 * Returns: A string containing the version of Vogue library
 *   available at run time.
 *   The returned string is owned by Vogue and should not be modified
 *   or freed.
 *
 * Since: 1.16
 **/
const char *
vogue_version_string (void)
{
 ...
 *
 * Checks that the Vogue library in use is compatible with the
 * given version. Generally you would pass in the constants
 * %PANGO_VERSION_MAJOR, %PANGO_VERSION_MINOR, %PANGO_VERSION_MICRO
 * as the three arguments to this function; that produces
 * a check that the library in use at run-time is compatible with
 * the version of Vogue the application or module was compiled against.
 *
 ...
 *
 * Return value: (nullable): %NULL if the Vogue library is compatible
 *   with the given version, or a string describing the version
 *   mismatch.  The returned string is owned by Vogue and should not
 *   be modified or freed.
 *
 * Since: 1.16
 **/
const gchar*
vogue_version_check (int required_major,
		     int required_minor,
		     int required_micro)
{
  gint vogue_effective_micro = 100 * PANGO_VERSION_MINOR + PANGO_VERSION_MICRO;
  gint required_effective_micro = 100 * required_minor + required_micro;

  if (required_major < PANGO_VERSION_MAJOR)
    return "Vogue version too new (major mismatch)";
  if (required_effective_micro < vogue_effective_micro - PANGO_BINARY_AGE)
    return "Vogue version too new (micro mismatch)";
  if (required_effective_micro > vogue_effective_micro)
    return "Vogue version too old (micro mismatch)";
  return NULL;
}
 ...
 **/
char *
vogue_trim_string (const char *str)
{
  return _vogue_trim_string (str);
}

char *
_vogue_trim_string (const char *str)
{
  int len;
 ...
 **/
char **
vogue_split_file_list (const char *str)
{
  int i = 0;
  int j;
  char **files;
  ...
  while (files[i])
    {
      char *file = _vogue_trim_string (files[i]);
 ...
 **/
gint
vogue_read_line (FILE *stream, GString *str)
{
 ...
 **/
gboolean
vogue_skip_space (const char **pos)
{
  const char *p = *pos;
 ...
 **/
gboolean
vogue_scan_word (const char **pos, GString *out)
{
  const char *p = *pos;
 ...
 **/
gboolean
vogue_scan_string (const char **pos, GString *out)
{
  const char *p = *pos;
 ...
 **/
gboolean
vogue_scan_int (const char **pos, int *out)
{
  return _vogue_scan_int (pos, out);
}

gboolean
_vogue_scan_int (const char **pos, int *out)
{
  char *end;
  long temp;
 ...
 **/
char *
vogue_config_key_get_system (const char *key)
{
  return NULL;
}

/**
 * vogue_config_key_get:
 * @key: Key to look up, in the form "SECTION/KEY".
 *
 * Do not use.  Does not do anything.
 *
 ...
 **/
char *
vogue_config_key_get (const char *key)
{
  return NULL;
}

/**
 * vogue_get_sysconf_subdirectory:
 *
 * Returns the name of the "vogue" subdirectory of SYSCONFDIR
 * (which is set at compile time).
 *
 * Return value: the Vogue sysconf directory. The returned string should
 * not be freed.
 *
 * Deprecated: 1.38
 */
const char *
vogue_get_sysconf_subdirectory (void)
{
  ...
  if (g_once_init_enter (&result))
    {
      ...
      if (sysconfdir != NULL)
	tmp_result = g_build_filename (sysconfdir, "vogue", NULL);
      else
	tmp_result = SYSCONFDIR "/vogue";
...
/**
 * vogue_get_lib_subdirectory:
 *
 * Returns the name of the "vogue" subdirectory of LIBDIR
 * (which is set at compile time).
 *
 * Return value: the Vogue lib directory. The returned string should
 * not be freed.
 *
 * Deprecated: 1.38
 */
const char *
vogue_get_lib_subdirectory (void)
{
  static const gchar *result = NULL; /* MT-safe */
  ...
  if (g_once_init_enter (&result))
    {
      ...
      if (libdir != NULL)
	tmp_result = g_build_filename (libdir, "vogue", NULL);
      else
	tmp_result = LIBDIR "/vogue";
      g_once_init_leave(&result, tmp_result);
    }
  return result;
}
 ...
 **/
gboolean
vogue_parse_enum (GType       type,
		  const char *str,
		  int        *value,
		  gboolean    warn,
		  char      **possible_values)
{
  return _vogue_parse_enum (type, str, value, warn, possible_values);
}

gboolean
_vogue_parse_enum (GType       type,
		   const char *str,
 		   int        *value,
		   gboolean    warn,
		   char      **possible_values)
 ...
 */
void
vogue_lookup_aliases (const char   *fontname,
		      char       ***families,
		      int          *n_families)
{
  *families = NULL;
 ...
 */
VogueDirection
vogue_find_base_dir (const gchar *text,
		     gint         length)
{
  VogueDirection dir = PANGO_DIRECTION_NEUTRAL;
  const gchar *p;
  ...
  while ((length < 0 || p < text + length) && *p)
    {
      ...
      dir = vogue_unichar_direction (wc);
 ...
 */
gboolean
vogue_is_zero_width (gunichar ch)
{
...
/**
 * vogue_quantize_line_geometry:
 * @thickness: (inout): pointer to the thickness of a line, in Vogue units
 ...
 */
void
vogue_quantize_line_geometry (int *thickness,
			      int *position)
{
...
/**
 * vogue_units_from_double:
 * @d: double floating-point value
 *
 * Converts a floating-point number to Vogue units: multiplies
 * it by %PANGO_SCALE and rounds to nearest integer.
 *
 * Return value: the value in Vogue units.
 *
 * Since: 1.16
 */
int
vogue_units_from_double (double d)
{
  return (int)floor (d * PANGO_SCALE + 0.5);
}

/**
 * vogue_units_to_double:
 * @i: value in Vogue units
 *
 * Converts a number in Vogue units to floating-point: divides
 * it by %PANGO_SCALE.
 *
 ...
 */
double
vogue_units_to_double (int i)
{
  return (double)i / PANGO_SCALE;
}

/**
 * vogue_extents_to_pixels:
 ...
 *
 * Converts extents from Vogue units to device units, dividing by the
 ...
 **/
void
vogue_extents_to_pixels (VogueRectangle *inclusive,
...
void
_vogue_shape_shape (const char       *text,
		    unsigned int      n_chars,
		    VogueRectangle   *shape_ink G_GNUC_UNUSED,
		    VogueRectangle   *shape_logical,
...
void
_vogue_shape_get_extents (gint              n_chars,
			  VogueRectangle   *shape_ink,
			  VogueRectangle   *shape_logical,

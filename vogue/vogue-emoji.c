 ...
 *
 * Implementation of vogue_emoji_iter is based on Chromium's Ragel-based
 * parser:
 *
...
#define DEFINE_vogue_Is_(name) \
static gboolean \
_vogue_Is_##name (gunichar ch) \
{ \
 ...
 \
  if (bsearch (GUINT_TO_POINTER (ch), \
               _vogue_##name##_table, \
               G_N_ELEMENTS (_vogue_##name##_table), \
               sizeof _vogue_##name##_table[0], \
	       interval_compare)) \
    return TRUE; \
 \
  return FALSE; \
...
DEFINE_vogue_Is_(Emoji)
DEFINE_vogue_Is_(Emoji_Presentation)
DEFINE_vogue_Is_(Emoji_Modifier)
DEFINE_vogue_Is_(Emoji_Modifier_Base)
DEFINE_vogue_Is_(Extended_Pictographic)

gboolean
_vogue_Is_Emoji_Base_Character (gunichar ch)
{
	return _vogue_Is_Emoji (ch);
}

gboolean
_vogue_Is_Emoji_Extended_Pictographic (gunichar ch)
{
	return _vogue_Is_Extended_Pictographic (ch);
}

static gboolean
_vogue_Is_Emoji_Text_Default (gunichar ch)
{
  return _vogue_Is_Emoji (ch) && !_vogue_Is_Emoji_Presentation (ch);
}

static gboolean
_vogue_Is_Emoji_Emoji_Default (gunichar ch)
{
  return _vogue_Is_Emoji_Presentation (ch);
}

static gboolean
_vogue_Is_Emoji_Keycap_Base (gunichar ch)
{
  return (ch >= '0' && ch <= '9') || ch == '#' || ch == '*';
}

static gboolean
_vogue_Is_Regional_Indicator (gunichar ch)
{
...
enum VogueEmojiScannerCategory {
  EMOJI = 0,
...
_vogue_EmojiSegmentationCategory (gunichar codepoint)
{
  /* Specific ones first. */
  ...
  if (codepoint == 0xE007F)
    return TAG_TERM;
  if (_vogue_Is_Emoji_Modifier_Base (codepoint))
    return EMOJI_MODIFIER_BASE;
  if (_vogue_Is_Emoji_Modifier (codepoint))
    return EMOJI_MODIFIER;
  if (_vogue_Is_Regional_Indicator (codepoint))
    return REGIONAL_INDICATOR;
  if (_vogue_Is_Emoji_Keycap_Base (codepoint))
    return KEYCAP_BASE;

  if (_vogue_Is_Emoji_Emoji_Default (codepoint))
    return EMOJI_EMOJI_PRESENTATION;
  if (_vogue_Is_Emoji_Text_Default (codepoint))
    return EMOJI_TEXT_PRESENTATION;
  if (_vogue_Is_Emoji (codepoint))
    return EMOJI;
			...
			int             length)
{
  ...
  for (i = 0; i < n_chars; i++)
  {
    types[i] = _vogue_EmojiSegmentationCategory (g_utf8_get_char (p));

...
compare_font_family (gconstpointer a,
                     gconstpointer b)
{
  VogueFontFamily **aa = (VogueFontFamily **)a;
  VogueFontFamily **bb = (VogueFontFamily **)b;
  return strcmp (vogue_font_family_get_name (*aa, *bb);
}

int main (int argc, char **argv)
{
  VogueFontMap *fontmap = vogue_win32_font_map_for_display();
  VogueContext *context;
  VogueCoverage * coverage = NULL;
  VogueFont* font = NULL;
  VogueFontFamily** families = NULL;
  VogueFontFace** faces = NULL;
  int nb, i;
  gchar* family_name = NULL;
  VogueLanguage *lang = vogue_language_from_string (g_win32_getlocale ());
  HDC hdc = NULL;
  int line = 0;
  GTimeVal tv0, tv1;
  int my_font_size = 12;

  printf ("# Vogue Font Test\n"
	  "# Language: %s\n"
	  "#\n", vogue_language_to_string (lang));
  ...
  if (argc == 1)		/* No arguments given */
	...
	{
	  VogueFontDescription *desc = vogue_font_description_from_string(std_fonts[i]);
      ...
      for (i = 2; i < argc; i++)
	{
      ...
      desc = vogue_font_description_from_string(s->str);
      family_name = g_strdup (vogue_font_description_get_family (desc));

      font = vogue_font_map_load_font (fontmap, context, desc);

      coverage = vogue_font_get_coverage (font, lang);
  ...
  vogue_font_map_list_families (fontmap, &families, &nb);

  if (!family_name)
    {
      qsort (families, nb, sizeof (VogueFontFamily*), compare_font_family);
    }
  else
    {
      ...
      for (i = 0; i < nb; i++)
	{
	  if (0 == g_ascii_strcasecmp (vogue_font_family_get_name (families[i]), family_name))
	    {
	      vogue_font_family_list_faces (families[i], &faces, &nb);
  ...
  for (i = 0; i < nb; i++)
      ...
      if (families)
	{
	  ...
	  f_name =  vogue_font_family_get_name (families[i]);
	  vogue_font_description_set_family (desc, f_name);
	}
      else
	{
	  desc = vogue_font_face_describe (faces[i]);
	  /* this is _not_ the family name from above */
	  f_name = vogue_font_description_get_family (desc);
	}
      weight = vogue_font_description_get_weight (desc);
      style  = vogue_font_description_get_style  (desc);

      g_print ("%s; Style: %d; Weight: %d\n",
	       f_name, style, weight);

      /* give it an arbitray size to load it */
      vogue_font_description_set_size (desc, my_font_size * PANGO_SCALE);

      g_get_current_time (&tv0);
      font = vogue_font_map_load_font (fontmap, context, desc);
      g_get_current_time (&tv1);
      g_print ("\tvogue_font_map_load_font took %.3f sec\n", calc_duration (&tv1, &tv0));
      ...
      if (font)
	{
	  ...
	  coverage = vogue_font_get_coverage (font, lang);
	  g_get_current_time (&tv1);
	  g_print ("\tvogue_font_get_coverage took %.3f sec\n", calc_duration (&tv1, &tv0));

	  /* ... */
	  vogue_context_set_language (context, lang);
	  vogue_context_set_base_dir (context, PANGO_DIRECTION_LTR);
	  vogue_context_set_font_description (context, desc);

	  glyphs = vogue_glyph_string_new ();
	  item = vogue_item_new ();

	  item->analysis.font = g_object_ref (font);
	  vogue_shape ( s, sizeof(s), &(item->analysis), glyphs);
	  ...
	  if (hdc)
	    {
	      ...
	      g_get_current_time (&tv0);
	      vogue_win32_render (hdc, font, glyphs, 200, line);
	      g_get_current_time (&tv1);
	      g_print ("\tvogue_win32_render took %.3f sec\n",
		       calc_duration (&tv1, &tv0));
  ...
  if (hdc)
    post_render (hdc, "vogue-fonts.bmp");
